...
| `15-input` `16-flight` `15d-domread` | keys/pads/mouse, starfield, autopilot, trail; the DOM-read counter (`?domread`, asleep otherwise — the frame and the pointer handlers must read zero, 0.3) |
| `15c-rec` | `?rec=1` input recording by frame, F8 «bug here» → `drift.rec`; replayed by `T.replay` |
