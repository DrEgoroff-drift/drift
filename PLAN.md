...
- [ ] **[design owed] M492 Космопочта — first pass BUILT 18.09 (Control), `src/26e2-post.js` (prefix `kp`: `post*` belongs to the postcard circle 11e).** A КОСМОПОЧТА block under ДОСКА at every station with a board: hours on the door on a post clock of its own (a post day = half a смена, since a calendar day is one real minute; open 9–10 → 21–22, lunch 13–14, ~45 % of the time), «ЗАКРЫТО · откроется через N мин». The СТАПЕЛЬ hull comes as an извещение and is collected at any open window with a талон (or at the yard as before); it waits 30 post days, then «возвращено отправителю» (the yard only); a day late — the clerk keeps it «ещё денёк, не по правилам», once per parcel. **Open:** a rare part and cooperative goods as parcels, a real queue, M499 попутная посылка. Original: at stations open by the game clock (hours on the door);
