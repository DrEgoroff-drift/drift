...
1. **Merge the six agent branches into main** (each is one commit on top of `15508e6`, all `-Only` green + Node green; they touch different files, expect no conflicts except `drift.html`/`docs/INDEX.md` artefacts — take theirs from a rebuild):
   - `worktree-agent-a42298b22422873b4` `7d6eae8` — helm M410 suite: the test held the dead zone as the literal 12 while `HELM_DEAD` is 22; now `HELM_DEAD+HELM_REACH`. (The phone-stick branch below carries the same fix in `91zzzw-helm.js` — take either.)
   - `worktree-agent-af4c29258093592dd` `b141b8d` — same-hash determinism: `ABIL_ST` (`16c-abil.js`) kept `on/cd` in `G.t` across `resetWorld`/new game, so the special system started on cooldown and ФОРСАЖ fired only in the first run. Real game bug. New `abilStale()` at the head of `abilTick`/`abilFire`.
   - `worktree-agent-a1d571deda27a4d59` `2496494` — detector «карта · A · застой»: `detStuck` in `90c-detect-laws.js` ignored the driver's `DET_MUTE` (map has no flight keys by design); now `&&!c.mute`. Test was stale, game fine.
   - `worktree-agent-aeec4c689c28a9299` `afb8179` — phone stick suites, three causes: (a) test stale — since 0.446 `pointermove` only stores raw x/y and `helmTake()` runs once a frame from `helmSyncPointer()`, the suite now forces a frame; (b) GAME BUG — compass chips grew to the 44 px touch size AFTER placement, so the invisible half sat under the thumb: `CHIP_TOUCH`, `grow()` in `17-mode-system.js`; (c) GAME BUG — `helmLift()` added the current `--helmlift` to a cached `promptRect()` that never invalidated, so the prompt lift flipped 0↔91 every other frame (`HELM.liftRect/liftBase` in `15a`/`15b`). **(c) is a per-frame DOM style write flipping — a prime suspect for the phone jitter below.**
   - `worktree-agent-ae50e84a336595cc4` `3329d4f` — mutant `instr-lies` retargeted at `setPair($fn,"fn",Math.round(G.fuel),` in `27z-telemetry.js` (the old `setTx` line is gone); killed by «сквозной: сцены» (`прибор «бак» показывает 101/100`).
   - `worktree-agent-a40296abe31a7c5e2` — «полный трюм» (СДАТЬ overfills by 8): agent still running when this was written; check `git branch --list 'worktree-agent-*'` and its last commit; if absent, the suite is still open.
2. **Then the release ladder** (memory: Node tier too): `node test-node.js`, `test.ps1 -Full`, `-Mobile`, `-Mutants`, goldens `-Accept` only if a frame changed on purpose; PATCHNOTES 0.451.0; VER; push `helm-layout:main`; deploy status via api.github.com; `docs/live.ps1`.
