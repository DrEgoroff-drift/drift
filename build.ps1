...
$PLAN_KB = 40      # порог для PLAN.md (100 → 40 23.09.2026: в плане только то, что хотим; сделанное — docs/done/)
...
function Bulk($files, $tfiles) {
  ...
  $done = Join-Path (Join-Path $root "docs") "done"
  if (Test-Path $done) {
...
$WAR_MODULES = @("01-core.js", "03a-hull-maker.js", "12al-powers.js", "17z-map-backdrop.js",
