/* ДРЕЙФ — попугай отдельной птицей: тот же код, что в игре (src/12x-12z),
   плюс маленькая заглушка вместо игрового состояния. Живёт на любой странице,
   где есть канва #parrotcv. */
const G={parrot:{name:"Балабол",who:"борта «Стриж-7»"}};
const pick=(a,r)=>a[Math.floor(r()*a.length)];
const HEARD=[
  {kind:"pidgin",ru:["зурта","не́","ка","ва́лли"]},
  {kind:"pidgin",ru:["◇","ку́ро","⌇","та́ш"]},
  {kind:"pidgin",ru:["ва́лли","ва́лли","мо́рэ"]},
  {kind:"yours",note:"частота 118.4, ответа не будет"},
  {kind:"yours",note:"Тихоня знает про третий трюм"},
  {kind:"price",note:"«Тихой Гавани»"}
];
function parrotHas(){return true;}
function heardAll(){return HEARD;}
function heardWordsRu(h){return h.ru;}
/* ══════════════ трепло: жёрдочка ══════════════
...
function parFoot(c,x,dir,grip,up){
  ...
  c.beginPath();c.moveTo(1,-42);c.quadraticCurveTo(0,-20,0,-5);c.stroke();
...
function parrotDraw(c,W,H){
  ...
  parRow(g,4,-10,8,9,1.28,1.92,5,40,15,3.6,.07,PAR_C.amber,PAR_C.amberD,.9);
  g.restore();
  ...
  for(let i=0;i<9;i++){
    ...
    const len=(56+Math.sin(k*3.1)*22)*(1+cr*.30);
    ...
    g.save();g.rotate(w);
    const P=parPlume(g,len,a,.86,5.4-k*1.5,col,colD,4,1.0);
    /* голый стебель за опушкой: бусина висит НА нём, а не приклеена к перу —
       это и даёт антенну, а не блёстку на кончике */
    const sx=P[0]+Math.cos(a+.30)*13, sy=P[1]+Math.sin(a+.30)*13;
    g.strokeStyle="rgba(150,200,235,.55)";g.lineWidth=1.15;
...
let PAR_L=null;
function parLayer(W,H){
  if(!PAR_L||PAR_L.w!==W||PAR_L.h!==H){
    const cv=document.createElement("canvas");
    cv.width=W;cv.height=H;
    PAR_L={cv,g:cv.getContext("2d"),w:W,h:H,ox:W*.5,oy:H*.94};
  }
  PAR_L.g.setTransform(1,0,0,1,0,0);
...
/* ── жёрдочка ──
   Тот же цикл, что в игре, только без корабля вокруг. Заводится сам, если на
   странице есть канва #parrotcv: так одна и та же птица живёт и на главной,
   и на своей странице, и код у неё один. */
(function(){
  const cv=document.getElementById("parrotcv");
  if(!cv)return;
  const cage=document.getElementById("cage")||cv;
  const said=document.getElementById("said");
  const DPR=Math.min(2,window.devicePixelRatio||1);
  const slow=matchMedia("(prefers-reduced-motion: reduce)").matches;

  function size(){cv.width=cv.clientWidth*DPR;cv.height=cv.clientHeight*DPR;PAR_L=null;}
  size();
  addEventListener("resize",()=>{clearTimeout(window._pz);window._pz=setTimeout(size,180)});

  let t0=performance.now();
  function frame(ts){
    const dt=Math.min(.05,(ts-t0)/1000||.016);t0=ts;
    parStep(slow?dt*.4:dt);
    const c=cv.getContext("2d");
    c.setTransform(1,0,0,1,0,0);c.clearRect(0,0,cv.width,cv.height);
    c.setTransform(DPR,0,0,DPR,0,0);
    parrotDraw(c,cv.width/DPR,cv.height/DPR);
  ...
  /* Реплику берём у самой птицы; молчит — так и пишем, молчание тоже ответ. */
  function speak(){
    if(!said)return;
    let line="";
    try{ line=parrotLine&&parrotLine(); }catch(e){}
    if(line&&typeof line==="object")line=line.text||line.ru||"";
    said.textContent=line?String(line):"птица молчит";
    said.style.opacity=line?"1":".55";
  }
  setTimeout(speak,900);
  setInterval(()=>{if(Math.random()<.5)speak();},14000);
})();
