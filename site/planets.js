...
(function(){
  ...
  window.makePlanet=function(o){
    ...
    /* Порог пересборки — по ЭКРАННОМУ сдвигу, а не по пикселям развёртки.
       Три четверти пикселя текстуры на диске в пол-экрана — это больше пикселя
       НА ЭКРАНЕ, и приходил он раз в пятую долю секунды: планета не плыла, а
       щёлкала. Меряем в долях экранного пикселя (у центра диска один оборот
       проходит два диаметра), и пересборок выходит около десяти в секунду на
       любом размере — цена та же, ход непрерывный. */
    const turnStep=Math.min(.75/P.strip.width,.5/(TAU*half));
    ...
    P.draw=function(ctx,x,y,turn){
      ...
      /* дробные координаты, а не целые: планета ползёт по экрану примерно на
         пятую долю пикселя за кадр, и округление копило этот ход, чтобы разом
         прыгнуть на пиксель раз в пять кадров — тот же рывок, что и с
         поворотом. Пересэмплировать один готовый диск дешевле, чем собирать
         его заново, а мягкость кромки на приглушённом фоне не читается */
      ctx.drawImage(disc,x-box/2,y-box/2);
    };
    return P;
  };
})();
