...
# окно, которое ПРОСИЛИ, — в адрес (0.440.0): золотые кадры ключуются по нему, а не по
# измеренному W×H, которое у каждого headless своё (полоса прокрутки, шапка)
$sepw = if ($url -match "\?") { "&" } else { "?" }; $url += "$sepw" + "win=$win"
