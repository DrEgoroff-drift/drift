...
function stapelSheet(o,cw,ch,st){
  ...
  cv.style.width=cw+"px";cv.style.height=ch+"px";cv.className="stp-sheet";
  ...
  stapelSheet.gd={cv,cw,ch,nd:dpr,ship:NPC_SHIPS[STAPEL_PV],by,hull,paint,sig:++STP_G.k};
  return cv;
}
/* рамка корпуса в единицах мира — по вершинам GPU-холста (08ca: GcCtx режет пути на треугольники
   ...
   выпечки на видеокарте (84 корпуса, 26.09) — худшее 0.42 % длины. Раз на сдвиг ползунка, не в кадре */
function stapelHullBox(hl){
  const K=8,g=new GcCtx(1,1,1),prev=ctx;
  ...
  try{hullPart1(hl,STAPEL_PV,0,false);hullPart2(hl);hullPart3(hl,STAPEL_PV);}catch(e){g._ops.length=0;}finally{ctx=prev;}
  for(const q of g._ops){
    if(q.op.startsWith("destination"))continue;
    const al=q.t==="i"?q.a:q.p?(q.p.k===0?q.p.c[3]:q.p.a):0;if(!(al>.094))continue;
    let [a,b,c,d]=bb(q.v,q.t==="i"||q.t==="x"?4:2);
    for(const k of q.clip||[]){const m=bb(k.v,2);a=Math.max(a,m[0]);b=Math.max(b,m[1]);c=Math.min(c,m[2]);d=Math.min(d,m[3]);}
    if(c<a||d<b)continue;
    X0=Math.min(X0,a);Y0=Math.min(Y0,b);X1=Math.max(X1,c);Y1=Math.max(Y1,d);
  }
  if(X1<X0)return {x0:hl.tail,x1:hl.nose,y0:-hl.halfW,y1:hl.halfW};
...
function stapelHullTick(){
  ...
  const W=cv.width,H=cv.height,nd=D.nd;
  ...
  if(!B)return;
  g.bk=[HB,B];
  ovInto(g.T,nd,()=>ovImage(B,D.cw/2,D.ch/2,D.cw,D.ch,0,0,0,1,1,1));
