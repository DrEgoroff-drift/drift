...
/* L1b: пыль — поле (x: >0 внутри тела, в долях порога), мелкая структура (y), узор (zw).
   Три масштаба: 2–3 крупные полосы (гребни вытянутого узора), средние столпы и волокна,
   мелкие глобулы; всё вытянуто по своему направлению системы. Край изъеден мелким шумом */
fn dustAt(p:vec2f,W:f32,H:f32,seed:f32,t:f32)->vec4f{
  let q=((p-vec2f(W,H)*.5)+u.b.xy*PDUST)/H+vec2f(seed*2.3+31.,seed*1.1+7.);
  let th=fract(seed*7.13)*3.1416;let cs=cos(th);let sn=sin(th);
  let qr=vec2f(cs*q.x+sn*q.y,-sn*q.x+cs*q.y);
  let w=vec2f(fb(qr*.8+vec2f(3.,t*.15),3),fb(qr*.8+vec2f(8.,1.),3))-.5;
  let qa=(qr+w*.9)*vec2f(.45,1.5);
  let band=(1.-abs(2.*fb(qa*.9+vec2f(1.,4.),4)-1.)-.74)*4.;
  let med=(fb(qa*2.6+w*1.5+vec2f(6.,2.),4)-.62)*4.;
  let glb=(fb(qr*6.+w*2.+vec2f(13.,2.),3)-.74)*5.;
  let dn=fb(qr*9.+w*3.+vec2f(2.,9.),3);
  let er=(fb(qr*22.+vec2f(5.,5.),3)-.5)*.5+(dn-.5)*.3;
  return vec4f(max(band,max(med,glb))+er,dn,qa);}
...
@fragment fn fs(i:VO)->@location(0) vec4f{
  ...
  let DD=dustAt(p,W,H,seed,t);let Dm=DD.x;let dn=DD.y;let dq=DD.zw;
  /* край — в пикселях: расстояние до порога по градиенту */
  let ex=1.5;
  let gD=vec2f(dustAt(p+vec2f(ex,0.),W,H,seed,t).x-dustAt(p-vec2f(ex,0.),W,H,seed,t).x,
               dustAt(p+vec2f(0.,ex),W,H,seed,t).x-dustAt(p-vec2f(0.,ex),W,H,seed,t).x)/(2.*ex);
  let gl2=max(length(gD),1e-5);
  let dk=clamp(dust,.6,1.3)*(1.-.75*LM.w);
  let thr=.08-.25*(dk-1.)+.6*LM.w;
  let dpx=(thr-Dm)/gl2;                       /* >0 — снаружи тела, в CSS px */
  let body=1.-smoothstep(-1.,1.,dpx);
  /* плотность растёт вглубь: тонкое место просвечивает газом и звёздами, толстое глухое */
  let od=body*(.35+3.*smoothstep(1.,16.,-dpx))*(.8+.4*dn)*dk;
  /* кайма фронта ионизации на стороне к свету: линия 1–3 px светлее газа и свечение наружу */
  let tS=normalize(sp-p+vec2f(1e-3));
  let face=max(dot(-gD/gl2,tS),0.);
  let brk=smoothstep(.3,.65,fb(dq*4.+vec2f(7.,3.),3));
  let o=max(dpx,0.);
  let edge=smoothstep(-1.5,.3,dpx)*(exp(-o/1.3)+.3*exp(-o/8.));
  let ion=edge*face*face*brk*(.3+2.4*lit)*dk;
  let ic=mix(gc*1.5,vec3f(1.,.95,.86),.3)*max(max(sc.r,max(sc.g,sc.b)),.7);
  /* внутри — бурый (тон 15–30°): отражённый свет, прожилки вдоль вытяжки, к сердцевине темнее */
  let vein=1.-abs(2.*fb(dq*vec2f(3.,9.)+vec2f(1.,6.),3)-1.);
  let deep=.35+.65*exp(min(dpx,0.)/18.);
  let brown=vec3f(.11,.07,0.)*(.5+.9*lit)*(.3+1.6*pow(vein,4.))*deep
    *(1.+2.*face*exp(min(dpx,0.)/6.))*body*(1.-exp(-od*2.4))*dk;
  C=C*exp(-od*2.4)+ic*ion*1.1+brown;T=T*exp(-od*2.4);
