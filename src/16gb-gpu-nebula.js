...
@fragment fn fs(i:VO)->@location(0) vec4f{
  ...
  /* стык тонов — не стенка: переход на ~60–150 px, тона перемешаны клочьями (варп мельче
     крупного плана) — тон меняется не быстрее ~2° на px (автор 24.09: «лужа») */
  let sel0=fb(qm*.55+wm*.9+vec2f(11.,3.),3)+near*.3*smoothstep(.05,.45,lit)*son;
  let sel=sel0+(fb(qm*2.6+wm*2.+vec2f(3.,17.),3)-.5)*.16;
  let tone=smoothstep(.38,.62,sel);
  let seam=1.-smoothstep(.0,.07,abs(sel-.5));
  ...
  let cvn=.45+.55*smoothstep(.3,.7,fb(p/H*3.2+vec2f(seed*.3+4.,2.),3));
  ...
  /* край мягкий и наружу, и внутрь: тыл тает на ~60 px, к звезде — на ~30 (там его держит
     кайма); резкая граница читалась лужей (автор, 24.09) */
  let wsoft=mix(60.,34.,smoothstep(-.75,-.25,fd))*mix(1.,.8,fw);
  let body=1.-smoothstep(-wsoft*.6,wsoft*.4,dpx);
  ...
  let od=body*(.3+3.2*smoothstep(0.,26.,-dpx)*(.35+.65*dv))*dk;
  ...
  let o=max(dpx,0.);
  /* кайма — мягкое свечение 8–20 px: ярче у тела, наружу гаснет, без резкой внутренней границы */
  let edge=smoothstep(-8.,2.,dpx)*(.55*exp(-o/4.)+.45*exp(-o/12.));
