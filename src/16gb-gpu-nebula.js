...
/* столпы одного масштаба: ячейки по углу (n на оборот — шов у звезды сходится сам) и по ln r
   (cr); в ячейке с вероятностью pr — голова. Ствол уходит от звезды до стены, ширина — в H,
   а не в угле: у звезды столп не сходится в иглу. Голова глубже стены на gw кадра — нет
   столпа (полость у звезды выметена). gl — доля голов с глобулой впереди: хвост от звезды */
fn pillars(D:DQ,se:f32,rw:f32,n:f32,cr:f32,pr:f32,w0:f32,gw:f32,gl:f32,sd:f32)->f32{
  var m=-1.;let per=6.2832/n;
  let i0=floor(D.q.x/per);let j0=floor(D.q.y/cr);
  for(var di=-2;di<=2;di++){
    let i=i0+f32(di);let im=i-n*floor(i/n);
    for(var dj=-4;dj<=2;dj++){
      let j=j0+f32(dj);
      ...
      m=max(m,-caps(a,d,w,w*1.9,max(rw-rh,2.*w)+.05));
      /* глобула — только вдали от звезды: у самой звезды поперечник по углу схлопывается,
         и глобула размазалась бы в кольцо */
      let g=w*(.14+.14*h3);let gp=w+g*(2.+4.*h2/max(gl,1e-3));
      if(h2<gl&&rh-gp>.12+6.*g){
        m=max(m,-caps(a+gp,d+w*.5*(h3-.5),g,g*.25,g*5.));}
    }}
  return m;}
/* x: поле в H (>0 — внутри тела), z: плотность толщи, w: s */
fn dustAt(p:vec2f,W:f32,H:f32,seed:f32)->vec4f{
  ...
  let se=.975+(nzs(tr,D.q,vec2f(4.,1.),o+vec2f(21.,9.),2)-.5)*.2+(nzs(tr,D.q,vec2f(11.,2.5),o+vec2f(4.,13.),2)-.5)*.16;
  let rw=dR(D,D.r0+se*(D.r1-D.r0));
  var m=D.r-rw;
  /* из стены к звезде — столпы трёх масштабов, у голов — глобулы */
  m=max(m,pillars(D,se,rw,18.,.8,.41,.05,.9,.45,sd));
  m=max(m,pillars(D,se,rw,40.,.55,.26,.026,.55,.3,sd+13.));
  m=max(m,pillars(D,se,rw,70.,.4,.25,.012,.22,0.,sd+29.));
  /* край комковатый: крупные наплывы и мелкие */
  m=m+(nzs(tr,D.q,vec2f(7.,1.8),o+vec2f(8.,8.),2)-.5)*.04+(nzs(tr,D.q,vec2f(14.,7.),o+vec2f(1.,3.),3)-.5)*.022
    +(nzs(tr,D.q,vec2f(40.,18.),o+vec2f(6.,1.),2)-.5)*.01
    -(1.-smoothstep(.0,.2,s))*.04;
  let dn=nzs(tr,D.q,vec2f(12.,3.),o+vec2f(2.,9.),3);
  return vec4f(m,0.,dn,s);}
...
@fragment fn fs(i:VO)->@location(0) vec4f{
  ...
  /* полоса узкая: шум fb держится у .5, и широкий порог клал её на полкадра — звёзд не было */
  let band=1.-smoothstep(.015,.05,abs(bv-.5));
  ...
  /* край несимметричен: к звезде — резкий, изъеденный, с каймой; от звезды — мягкий,
     пыль тает в газ на 10–20 px. Изъеден только освещённый край */
  let DQ0=dpolar(p,W,H);
  /* мелочь края — в слое с параллаксом пыли (.12): у центра кадра она едет вместе с пылью */
  let pl=(p+u.b.xy*.12)/H;
  let ero=(fb(pl*55.+vec2f(5.,5.),3)-.5)*(.005+.012*fw);
  let dk=clamp(dust,.6,1.3)*(1.-.75*LM.w);
  let thr=-.012*(dk-1.)+.08*LM.w;
  let dpx=(thr-DD.x-ero)/gl2;                   /* >0 — снаружи тела, в CSS px */
  /* мягко — только тыл (край смотрит от звезды); бока столпов резкие */
  let wsoft=mix(16.,1.2,smoothstep(-.75,-.25,fd));
  let body=1.-smoothstep(-wsoft,1.,dpx);
  ...
  let od=body*(.35+3.*smoothstep(1.,16.,-dpx)*(.3+.9*dn))*dk;
  ...
  /* отражённый свет звезды и без газа: тёпло-серый на стороне тела к звезде, вглубь гаснет —
     тело читается формой, а не дырой (L1b 5/n) */
  let refl=vec3f(.18,.147,.11)*(.3+.7*fw)*(.12+.88*exp(min(dpx,0.)/20.))*body*(1.-gas)*dk
    *mix(.6,1.,smoothstep(.3,.7,dn));
  C=C*exp(-od*2.4)+ic*ion*1.5+brown+refl;T=T*exp(-odT*2.4);
...
fn field(p:vec2f,uv:vec2f)->vec4f{
  ...
  let fog=V[1].w*(.1+.05/(1.+pow(dl/.4,2.)))*smoothstep(.12,.3,dl)*(1.-body)*(1.-.92*wn)
    *mix(.55,1.,clamp(1.-texCubic(t0,smp,uv).a,0.,1.));
