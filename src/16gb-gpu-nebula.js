/* ══════════════ туманность системы объёмом (L1, docs/DESIGN-gpu.md) ══════════════
   Считается целиком на видеокарте:
   ...
   · мелкий масштаб — в полном разрешении при сведении: гребни шума по тому же
     течению режут газ на волокна, края пыли мягкие; тени тонируются третьим цветом.
...
/* столп — сужающийся ствол (L1b 6/n): одна ячейка по углу (n на оборот) — не больше одного
   ...
   длины, к голове сужается, по длине 2–3 вздутия, изгиб до 15°; голова тупая и шире шеи
   (острая читалась когтем). gl — доля голов с глобулой впереди */
...
@fragment fn fs(i:VO)->@location(0) vec4f{
  ...
  let DD=dustAt(p,W,H,seed);let dn=DD.z;let gb=max(C.r,max(C.g,C.b));
  ...
  let o=max(dpx,0.);
  /* кайма 15–30 px при 760, наружу гаснет; внутрь над тусклым газом — хвост ~22 px, до −70:
     иначе на тёмном контур рыбки (×2) */
  let edge=mix(exp(min(dpx,0.)/22.)*smoothstep(-70.,-30.,dpx),smoothstep(-26.,4.,dpx),smoothstep(.15,.5,gb))*(.45*exp(-o/10.)+.55*exp(-o/28.));
  ...
  let brown=vec3f(.13,.08,.02)*(.55+.35*lit)*(.6+.8*pow(vein,4.))*deep*max(1.+.6*emb,.15)
    *(1.+face*fw*(1.2*exp(min(dpx,0.)/6.)+.8*exp(min(dpx,0.)/18.)))*body*(1.-exp(-od*2.4))*dk*gas;
  /* отражённый свет звезды и без газа — на стороне тела к звезде (L1b 5/n) */
  /* кора: серо-бурый с третью тона газа, внутрь гаснет за 5–8 px; тонкое (глобула) — без коры:
     без газа оно только гасит звёзды, а не светится рыбкой */
  let refl=mix(vec3f(.17,.15,.13),gc*.16,.3)*(.3+.7*fw)*(.06+.94*exp(min(dpx,0.)/6.5))*body*(1.-gas)*dk
    *mix(.6,1.,smoothstep(.3,.7,dn))*tk;
