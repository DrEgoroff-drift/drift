...
@fragment fn fs(i:VO)->@location(0) vec4f{
  ...
  let o=max(dpx,0.);
  /* кайма 15–30 px при 760, наружу гаснет; внутрь тела над тусклым газом — хвост ~22 px:
     обрыв на тёмном читался контуром рыбки (×2) */
  let edge=mix(exp(min(dpx,0.)/22.),smoothstep(-26.,4.,dpx),smoothstep(.15,.5,gb))*(.45*exp(-o/10.)+.55*exp(-o/28.));
