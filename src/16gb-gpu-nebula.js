...
fn lfr(p:vec2f,W:f32,H:f32,sp:vec2f,h:vec4f,i:vec4f,cam:vec2f,par:f32)->vec2f{
  ...
  if(h.w>2.5){Lc=sp;a=1.3208;}
    ...
    /* комета: голова в своём углу (решётка, отступ от HUD — как у всех), хвосты — внутрь
       кадра по диагонали угла ±20° (зерно потока) и гаснут к середине. Громадина на
       бесконечности: проекция хвоста любая, от звезды его не ставим */
    let v=vec2f(W,H)*.5-Lc;a=atan2(v.y,v.x)+(i.x/6.2832-.5)*.7;}
...
fn lmk(p:vec2f,W:f32,H:f32,sp:vec2f)->LK{
  ...
  if(ty<.5){
    ...
    let wi=.0018+.0085*min(xp/.7,1.);
    let st=.7+.3*fb(vec2f(x*5.-u.b.z*4.,ls),2);
    ...
    e=vec3f(.5,.75,1.)*ion*.45+vec3f(1.,.86,.62)*dst*.85
