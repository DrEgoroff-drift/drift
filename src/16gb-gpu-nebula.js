...
const GNB={tex:null,view:null,dev:null,w:0,h:0,last:-99,cx:1e9,cy:1e9,sys:null,U:new Float32Array(28),C:new Float32Array(20)};
const GNB_NOISE=`
...
@fragment fn fs(i:VO)->@location(0) vec4f{
  ...
  let sel=fb(qm*.55+wm*.9+vec2f(11.,3.),3)+near*.12*smoothstep(.02,.5,lit)*son;
  ...
  for(var L=0;L<3;L++){
    ...
    let g=smoothstep(.46-.06*(2.-fl)-.12*fill,.76+.07*(2.-fl),d)*amp*mass*(1.-.9*seam);
  ...
  let wc=mix(sc,gc*max(sc.r,max(sc.g,sc.b)),.8);
  C=C+son*(sc*core+wc*wide*1.6*dsum)*mix(1.,T,.6);
  return vec4f(C,1.-T);}`;
...
/* V[0]: камера x,y, время, зерно · V[1]: звезда x,y, радиус/H, вкл · V[2]: цвет звезды · V[3]: цвет теней */
...
const GNB_PAL=[
  ...
  {a:[206,58,164],b:[64,186,118],c:[40,26,110]},    // маджента + зелень, тени индиго
