...
function hinTalk(f){
  const S=G.hin;
  let line="";
  /* ── дома она говорит домашнее (25.08.2026) ──
     Здесь стоял набор С БОРТА: в собственной кухне Вега сообщала «летаешь как
     баржа» и «укачивает, сколько ещё прыжков». А написанный для дома
     `VEGA_DREAM` — про оставленный завтрак, про заштопанный локоть, про свет,
     который она не станет гасить, — не звучал НИ РАЗУ ни в одной сборке.
     Наборы просто стояли не на своих местах. */
  if(f.who==="vega"&&VEGA_DREAM.length)
    line=VEGA_DREAM[Math.abs(hashi(Math.round(G.t/300),7,0x40F3))%VEGA_DREAM.length];
  if(!line){
    const pool={
