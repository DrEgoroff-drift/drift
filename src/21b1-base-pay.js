...
/* ── лёд в баки: своё топливо из своего льда ──
   Было два топлива за лёд, и это оказалось не «по себестоимости», а вдвое
   дешевле любого прилавка НАВСЕГДА (разбор 0.409.1): лёд на прилавке стоит
   семь, топливо — пять-двенадцать за единицу, а лёд на базу можно и привезти
   (`baseSupply` кладёт его в тот же склад). Получался насос: купил лёд, слил
   в баки, вышел вдвое дешевле — и блокада тут была ни при чём.

   Один к одному — это и есть себестоимость: свой лёд стоит своих семи, бак
   выходит по цене сырья и без наценки прилавка. Ценность базы в блокаду от
   этого не меняется — там прилавка просто нет, и в этом вся мысль §43. */
const FUEL_PER_ICE=1;
