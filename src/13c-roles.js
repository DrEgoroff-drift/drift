...
function roleFire(p,d,want,range,cool){
  ...
  /* по игроку за экраном не стреляют (R0, 12.09): держава с целым iff бьёт
     чужой борт, и эта война идёт своим ходом */
  if(!p.iff&&worldCovered())return false;
...
function pirateRoleTick(p,dt,d,want){
  ...
  if(rank===0){
    ...
    /* очередь барона закрывается не ракетой, а паузой: пусковая по таблице
       §5 у капитана, у барона рельса, кассетник, зенитка и мины (M368) */
