...
/* ── улика, которую называет сама игра ──
   До 0.419 на сервер уходил КАЖДЫЙ «warn» судового журнала (хук жил в 28-loop):
   «просто пиши всё, потом разберём» (автор, 05.09). Разобрали: за четверо суток
   в crash.log легло сто строк, и все сто — новости игры. Дрон встал в док,
   смотритель ворчит про голый процент, пираты закрепились в секторе — это не
   сбой, это игра, а настоящую улику в такой стене не найти. Теперь письмо шлёт
   тот, кто и правда нашёл неладное: отказ записи, отказ облака, расхождение
   летописи. Игрок при этом видит ту же строку в журнале, что и раньше. */
function logShip(text,extra){
  try{if(typeof logAdd==="function")logAdd("warn",text);}catch(_){}
  crashShip("journal",text,"",extra);
}
