...
const WAN_C={
  wall:[42,40,40], wall2:[78,72,66], floor:[36,33,32], ceil:[26,25,28],   /* второй проход: стены теплее, дерево и латунь вместо стали */
...
const WAN_ICON={
  sextant(s){ctx.beginPath();ctx.arc(0,s*.1,s*.42,Math.PI*1.1,Math.PI*1.9);ctx.lineWidth=s*.09;ctx.strokeStyle="#c9a24a";ctx.stroke();
    ctx.lineWidth=s*.06;ctx.strokeStyle="#6a5221";ctx.beginPath();ctx.moveTo(0,s*.1);ctx.lineTo(-s*.3,-s*.28);ctx.moveTo(0,s*.1);ctx.lineTo(s*.34,-s*.2);ctx.stroke();
    ctx.fillStyle="#2b2f36";ctx.beginPath();ctx.arc(0,s*.1,s*.06,0,TAU);ctx.fill();ctx.fillStyle="rgba(255,240,200,.8)";ctx.fillRect(-s*.2,-s*.28,s*.12,s*.03);},
  pencil(s){ctx.save();ctx.rotate(-.5);ctx.fillStyle="#d9a43a";ctx.fillRect(-s*.45,-s*.06,s*.75,s*.12);ctx.fillStyle="#2b2f36";ctx.beginPath();ctx.moveTo(s*.3,-s*.06);ctx.lineTo(s*.48,0);ctx.lineTo(s*.3,s*.06);ctx.closePath();ctx.fill();
    ctx.fillStyle="rgba(255,240,200,.6)";ctx.fillRect(-s*.45,-s*.05,s*.75,s*.025);ctx.restore();},
  gyro(s){ctx.lineWidth=s*.07;ctx.strokeStyle="#c9a24a";ctx.beginPath();ctx.arc(0,0,s*.4,0,TAU);ctx.stroke();ctx.beginPath();ctx.ellipse(0,0,s*.4,s*.16,.6,0,TAU);ctx.stroke();
    ctx.fillStyle="#2b2f36";ctx.beginPath();ctx.arc(0,0,s*.08,0,TAU);ctx.fill();ctx.strokeStyle="#e8e0d0";ctx.lineWidth=s*.04;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(s*.22,-s*.22);ctx.stroke();
    ctx.fillStyle="rgba(255,240,200,.7)";ctx.beginPath();ctx.arc(-s*.22,-s*.28,s*.05,0,TAU);ctx.fill();},
  blanket(s){ctx.fillStyle="#b8bcc2";ctx.beginPath();ctx.roundRect(-s*.45,-s*.25,s*.9,s*.5,s*.05);ctx.fill();ctx.fillStyle="#d8dce2";ctx.fillRect(-s*.45,-s*.25,s*.9,s*.22);
    ctx.strokeStyle="rgba(0,0,0,.35)";ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(-s*.45,-s*.03);ctx.lineTo(s*.45,-s*.03);ctx.stroke();ctx.fillStyle="rgba(255,255,255,.7)";ctx.fillRect(-s*.4,-s*.22,s*.25,s*.03);},
  valve(s){ctx.fillStyle="#6a5221";ctx.fillRect(-s*.08,-s*.05,s*.16,s*.45);ctx.lineWidth=s*.08;ctx.strokeStyle="#c9a24a";ctx.beginPath();ctx.arc(0,-s*.12,s*.3,0,TAU);ctx.stroke();
    ctx.lineWidth=s*.05;for(let i=0;i<3;i++){const a=i/3*TAU;ctx.beginPath();ctx.moveTo(0,-s*.12);ctx.lineTo(Math.cos(a)*s*.3,-s*.12+Math.sin(a)*s*.3);ctx.stroke();}
    ctx.fillStyle="rgba(255,240,200,.7)";ctx.beginPath();ctx.arc(-s*.2,-s*.3,s*.045,0,TAU);ctx.fill();},
  needle(s){ctx.save();ctx.rotate(-.7);ctx.strokeStyle="#d8dce2";ctx.lineWidth=s*.05;ctx.beginPath();ctx.moveTo(-s*.45,0);ctx.lineTo(s*.45,0);ctx.stroke();
    ctx.lineWidth=s*.03;ctx.beginPath();ctx.ellipse(-s*.38,0,s*.06,s*.03,0,0,TAU);ctx.stroke();ctx.strokeStyle="rgba(200,60,50,.8)";ctx.beginPath();ctx.moveTo(-s*.44,0);ctx.quadraticCurveTo(-s*.6,s*.3,-s*.2,s*.36);ctx.stroke();ctx.restore();},
  bell(s){ctx.fillStyle="#c9a24a";ctx.beginPath();ctx.moveTo(-s*.32,s*.28);ctx.quadraticCurveTo(-s*.3,-s*.3,0,-s*.36);ctx.quadraticCurveTo(s*.3,-s*.3,s*.32,s*.28);ctx.closePath();ctx.fill();
    ctx.fillStyle="#6a5221";ctx.fillRect(-s*.36,s*.26,s*.72,s*.06);ctx.beginPath();ctx.arc(0,s*.36,s*.06,0,TAU);ctx.fill();ctx.fillStyle="rgba(255,240,200,.8)";ctx.fillRect(-s*.14,-s*.22,s*.05,s*.35);},
  notebook(s){ctx.fillStyle="#3a3128";ctx.beginPath();ctx.roundRect(-s*.32,-s*.4,s*.64,s*.8,s*.04);ctx.fill();ctx.fillStyle="#6a5221";ctx.fillRect(-s*.32,-s*.1,s*.64,s*.1);
    ctx.fillStyle="#d9cfae";ctx.fillRect(s*.26,-s*.36,s*.05,s*.72);ctx.fillStyle="rgba(255,240,200,.35)";ctx.fillRect(-s*.28,-s*.36,s*.04,s*.72);},
  sign(s){ctx.fillStyle="#3a3128";ctx.fillRect(-s*.5,-s*.22,s,s*.44);ctx.strokeStyle="#6a5221";ctx.lineWidth=1;ctx.strokeRect(-s*.5,-s*.22,s,s*.44);
    ctx.fillStyle="rgba(232,220,190,.85)";ctx.font=Math.max(6,s*.16)+"px ui-monospace,monospace";ctx.textAlign="center";ctx.fillText("НЕ КУПЛЕНО",0,s*.06);},
  hand(s){ctx.fillStyle="#8a9299";ctx.fillRect(-s*.12,s*.05,s*.24,s*.38);ctx.fillStyle="#b8bcc2";ctx.beginPath();ctx.roundRect(-s*.3,-s*.2,s*.6,s*.3,s*.06);ctx.fill();
    ctx.lineWidth=s*.07;ctx.strokeStyle="#b8bcc2";for(let i=-1;i<=1;i++){ctx.beginPath();ctx.moveTo(i*s*.18,-s*.18);ctx.lineTo(i*s*.22,-s*.44);ctx.stroke();}
    ctx.fillStyle="rgba(255,255,255,.6)";ctx.fillRect(-s*.26,-s*.17,s*.2,s*.03);},
  trumpet(s){ctx.save();ctx.rotate(.4);ctx.fillStyle="#c9a24a";ctx.beginPath();ctx.moveTo(-s*.45,-s*.05);ctx.lineTo(s*.2,-s*.3);ctx.lineTo(s*.42,-s*.34);ctx.lineTo(s*.42,s*.34);ctx.lineTo(s*.2,s*.3);ctx.lineTo(-s*.45,s*.05);ctx.closePath();ctx.fill();
    ctx.fillStyle="#2b2f36";ctx.beginPath();ctx.ellipse(s*.42,0,s*.05,s*.34,0,0,TAU);ctx.fill();ctx.fillStyle="rgba(255,240,200,.7)";ctx.fillRect(-s*.4,-s*.06,s*.5,s*.03);ctx.restore();},
  key(s){ctx.save();ctx.rotate(-.35);ctx.lineWidth=s*.07;ctx.strokeStyle="#c9a24a";ctx.beginPath();ctx.arc(-s*.28,0,s*.14,0,TAU);ctx.stroke();
    ctx.fillStyle="#c9a24a";ctx.fillRect(-s*.14,-s*.04,s*.6,s*.08);ctx.fillRect(s*.3,0,s*.06,s*.16);ctx.fillRect(s*.42,0,s*.05,s*.12);ctx.fillStyle="rgba(255,240,200,.7)";ctx.fillRect(-s*.1,-s*.035,s*.4,s*.02);ctx.restore();},
  pricelist(s){ctx.fillStyle="#d9cfae";ctx.fillRect(-s*.3,-s*.42,s*.6,s*.84);ctx.strokeStyle="rgba(90,70,40,.7)";ctx.lineWidth=1;
    for(let i=0;i<6;i++){ctx.beginPath();ctx.moveTo(-s*.22,-s*.3+i*s*.12);ctx.lineTo(s*.05+(i%2)*s*.1,-s*.3+i*s*.12);ctx.stroke();}
    ctx.fillStyle="rgba(120,40,30,.6)";ctx.fillRect(s*.12,-s*.3,s*.1,s*.03);ctx.fillRect(s*.12,-s*.06,s*.1,s*.03);},
  shelfwide(s){ctx.fillStyle="#6a5221";ctx.fillRect(-s*.48,-s*.06,s*.96,s*.1);ctx.fillStyle="#3a3128";ctx.beginPath();ctx.moveTo(-s*.4,s*.04);ctx.lineTo(-s*.4,s*.3);ctx.lineTo(-s*.14,s*.04);ctx.closePath();ctx.fill();
    ctx.beginPath();ctx.moveTo(s*.4,s*.04);ctx.lineTo(s*.4,s*.3);ctx.lineTo(s*.14,s*.04);ctx.closePath();ctx.fill();ctx.fillStyle="rgba(255,240,200,.5)";ctx.fillRect(-s*.48,-s*.06,s*.96,s*.02);}
};
function wanItemIcon(lot,cx,cy,s){
  ...
  }else if(WAN_ICON[lot.id]){
    /* у каждого инструмента своё лицо (второй проход): латунь — тело, один блик сверху-слева */
    ctx.fillStyle="#c9a24a";ctx.strokeStyle="#6a5221";ctx.lineWidth=Math.max(1,s*.05);ctx.lineCap="round";ctx.lineJoin="round";
    WAN_ICON[lot.id](s);
  }else{
