...
function updateSystem(dt){
  ...
  /* гравитационный якорь: за краем системы уход от звезды сходит на нет,
     ...
     Поэтому потолка больше нет вовсе — есть только сила. Притяжение к звезде
     нарастает от нуля на кромке до полутора тяг за 700 единиц: у самой кромки
     двигатель сильнее и игрок волен висеть где хочет, дальше гравитация
     пересиливает и сносит обратно. Никаких порогов, ничего не отнимается —
     корабль остаётся телом, которое всегда куда-то летит. */
  {
    ...
    if(d>rEdge){
      ...
      const pull=.082*st.thr*1.6*k;
      sh.vx-=nx*pull*dt;sh.vy-=ny*pull*dt;
      atEdge=true;
