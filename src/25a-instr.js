...
function instrRead(sx,sy){
  ...
  return INSTR.map(I=>{
    const base=I.base();
    /* разрешение прибора: завод, износ и профессия корпуса (05b-instr-kit,
       03f-hull-role). На изыскателе с «Вехой» отклонение видно раньше, на
       рудовозе с артельным хламом — почти не видно. Сам мир при этом не
       меняется: меняется прибор */
    const q=(typeof instrQuality==="function")?instrQuality(I.id)
...
/* положение показания на шкале: одна формула на стрелку и на перо
   самописца (25b-tape). Держать её в двух местах значило бы завести панель и
   ленту, которые расходятся между собой. */
function instrTrack(r){
  return clamp(Math.log10(1+Math.abs(r.val))/2.6+r.dev*.42,0,1);
}
...
/* место панели на блоке — одно на 2D и на мастер кабины (24bc): ширина, левый край, ячейка,
   высота шкалы, ось. Панель обязана уместиться В блоке, а не свешивать корытце на остекление:
   высота шкалы считается от того, сколько осталось до кромки проёма */
function instrPanelGeo(P){
  const w=Math.min(P.BW*.52,420),h=clamp((P.brow-12)/2.7,5,14);
  return {w,x0:(W-w)/2,cw:w/5,h,y:5+h};
}
/* part "base" — только то, что не движется (корытце, дуги, деления, подписи): мастер кабины 24bc,
   стрелки и невязку он кладёт сам */
function instrPanel(P,FS,part){
  const brow=P.brow;
  if(brow<26)return;                       // на низком потолке панели нет места
  const base=part==="base", R=base?INSTR:instrRead();
  const {w,x0,cw,h,y}=instrPanelGeo(P);
  ...
  for(let i=0;i<R.length;i++){
    ...
    ctx.fillText(ctx.measureText(r.ru).width<=cw-4?r.ru:(r.ab||r.ru),cx,y+h*1.66);
  }
