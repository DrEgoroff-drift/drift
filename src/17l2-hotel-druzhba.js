/* ══════════════ турбаза «Дружба» — РАССВЕТ (ra) ══════════════
   ...
   На крыше — надстройка, антенны, тарелка и щит с крашеной вывеской: буквы краской, не неоном
   (sign:null; неон — примета ГЛАВТРАССЫ, DESIGN-gpu). Над щитом — гирлянда лампочек, она же и
   освещает щит тёплым спадом сверху вниз (в слое свечения, буквы из него вырезаны: тёмное на
   светлом читается на зуме 2.2 в любой час); по кромке крыши — ещё гирлянда, часть перегорела
   (зерно), лампы горят ровно. Свет —
...
const HD_HALO=[.6,.8],HD_SIGN="ТУРБАЗА «ДРУЖБА»";
let HD_WINS=null;
...
function* hdPaint(c,e,sd,lit,Lt){
  ...
  /* ── крыша: диск под звездой, кромка, дорожки; надстройка, антенны с огнями, тарелка;
     щит с крашеной вывеской под гирляндой и гирлянда по кромке ── */
  {const g=c.createLinearGradient(sg>0?CX+RX:CX-RX,0,sg>0?CX-RX:CX+RX,0);g.addColorStop(0,C(HD_WALL,sU+.2));g.addColorStop(1,C(HD_WALL,sU*.55));c.fillStyle=g;hotelPoly(c,roof);c.fill();
   ...
   /* щит: крашеный, буквы краской; над ним провод с лампочками, часть перегорела */
   ...
   c.font="bold 11px ui-monospace,monospace";c.textAlign="center";c.textBaseline="alphabetic";
   c.fillStyle=sh(.3);c.fillText(HD_SIGN,CX+.4,45.2);c.fillStyle=C([184,40,30],sF*.5+.45);c.fillText(HD_SIGN,CX,44.8);
   /* свет гирлянды на щите: тёплый спад от ламп вниз — в слое свечения, поэтому щит светел и в тени
      звезды, и ночью; буквы из света вырезаны (чёрное в слое свечения = ничего не прибавить) —
      краска остаётся тёмной, и слово читается контрастом, без ореола */
   {const lg=e.createLinearGradient(0,34,0,47.5);lg.addColorStop(0,"rgba(255,214,150,.62)");lg.addColorStop(.45,"rgba(255,200,140,.34)");lg.addColorStop(1,"rgba(255,190,130,.14)");
    e.fillStyle=lg;hotelPoly(e,board);e.fill();
    e.font=c.font;e.textAlign="center";e.textBaseline="alphabetic";e.fillStyle="#000";e.fillText(HD_SIGN,CX,44.8);
    e.fillStyle="#000";e.fillRect(CX-64,34,128,1.1);e.fillRect(CX-64,46.4,128,1.1);}
...
HOTEL_T.ra={W:280,H:200,PX:2,ax:185,ay:100,sign:null,sheen:[185,40,30],halo:HD_HALO,wins:hdWins,paint:hdPaint};
