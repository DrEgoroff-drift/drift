/* ══════════════ турбаза «Дружба» — РАССВЕТ (ra) ══════════════
   ...
   (sign:null), над щитом и по кромке крыши — гирлянда лампочек, часть перегорела (зерно). Свет —
...
function* hdPaint(c,e,sd,lit,Lt){
  ...
  /* ── крыша: диск под звездой, кромка, дорожки; надстройка, антенны с огнями, тарелка;
     щит с крашеной вывеской и гирлянда над ним и по кромке ── */
  {const g=c.createLinearGradient(sg>0?CX+RX:CX-RX,0,sg>0?CX-RX:CX+RX,0);g.addColorStop(0,C(HD_WALL,sU+.2));g.addColorStop(1,C(HD_WALL,sU*.55));c.fillStyle=g;hotelPoly(c,roof);c.fill();
   ...
   c.font="bold 10px ui-monospace,monospace";c.textAlign="center";c.textBaseline="alphabetic";
   c.fillStyle=sh(.25);c.fillText(HD_SIGN,CX+.4,44.9);c.fillStyle=C(HD_RED,sF*.6+.55);c.fillText(HD_SIGN,CX,44.5);
