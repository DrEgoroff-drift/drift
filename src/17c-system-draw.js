...
const RL_DL="3.",RL_KD="6.",RL_SIDE=".5",RL_AMB=".46",RL_GAS=".9",RL_LIT="1.1",RL_EM="2.3",RL_GD="4.",RL_GF="1.5",RL_GL="1.5",RL_GN="8.",RL_GP="12.",RL_FL="4.",RL_FLM=".1",RL_RIM=".7",RL_GK=".85";
...
fn fieldL(p:vec2f,uv0:vec2f)->vec4f{
  ...
  if(hm&&mo){
    ...
    let key=smoothstep(-.35,.35,dot(gd,sd)*${RL_KD}+side*${RL_SIDE});
    var fl=vec3f(${RL_AMB});
    if((u32(fu.v[3].w+.5)&16u)!=0u){let rq=fu.res.zw;var gz=vec3f(0.);
      ...
      fl=fl*(1.-smoothstep(.0,.25,gm)*.35)+gc*${RL_GAS};}
    fl*=.55+.45*sk;let dir=vec3f(${RL_LIT}-${RL_AMB})*key*sk;
    let e=1.-own;
    /* d) блики по счёту: только голый металл (маска), рельеф панелей (мип грубее швов — иначе по
       каждому шву рябь в пиксель) на куполе тела в высокой степени — 2–4 жёстких блика на хребте и
    ...
    let gs=smoothstep(.15,.9,pow(max(dot(nm,Hs),0.),${RL_GP}))*smoothstep(.35,.7,mg)*(1.-mk.z)*own*key*sk*a;
