...
const HELM_NUDGE=0;          /* px увода корабля от пальца. Было 70 (M422); автор на плейтесте
                                13.09: «экран уезжает и корабль из-под пальца уезжает. Вот это
                                не надо» (P8, правило 4) — корабль стоит под пальцем */
const HELM_NUDGE_R=120;      /* ближе этого палец считается «поверх корабля» */
const HELM_DEAD=22;          /* мёртвая зона стика, px. Было 12 ≈ 2 мм на S23, и «палец лёг и
                                стоит = стой» сработал на 15 кадрах из 3 177: большой палец
                                уплывает дальше (P8, плейтест 13.09 §2.2). 22 ≈ 3.7 мм */
const HELM_REACH=70;         /* px хода стика до полной тяги */
const HELM_EDGE_BAND=240;    /* за сколько до кромки стик наружу уже ведёт вдоль неё */
...
/* лента (M422): она идёт от центра стика к пальцу и говорит две вещи разом —
   ...
   0.439.0 сняла эти четыре как «мёртвые» — их читают 15b-helm-draw и
   helmTrail, и стик на телефоне падал на каждом касании. */
...
cvs.addEventListener("pointermove",e=>{
  ...
  /* Палец — только сырые координаты, без чтения канвы: перевод в её пиксели,
     helmDrag и helmTrail идут РАЗ ЗА КАДР, в helmSyncPointer() ниже, а не на
     каждое событие. Сенсор шлёт движение чаще кадра (телефон — 120 Гц против
     60 отрисовки), и раньше каждое такое событие звало cvsRect(): само по
     себе дёшево (кэш), но не в кадре, где чуть раньше уже писали в стиль
     (helmLift → --helmlift) — тогда чтение прямоугольника вынуждает браузер
     пересчитать вёрстку синхронно, и это повторялось на КАЖДОЕ движение
     пальца. Отсюда каденция, линейно падающая с частотой событий, при том что
     неподвижный палец ничего не стоит (Контроль и Тестировщик, 18.09) —
     теперь на движение приходится не более одного такого пересчёта за кадр,
     а не по одному на событие. */
  if(HELM.S&&HELM.S.id===e.pointerId){HELM.S.rawX=e.clientX;HELM.S.rawY=e.clientY;return;}
  /* палец ещё не решил, кто он: сдвинулся дальше HELM_TAKE — значит стик (M422) */
  if(HELM.P&&HELM.P.id===e.pointerId){HELM.P.rawX=e.clientX;HELM.P.rawY=e.clientY;}
});
/* сырые координаты пальца → пиксели канвы, раз за кадр (см. комментарий у
   pointermove выше). Зовётся из helmTick, до физики. Ручные HELM.S/HELM.P из
   тестов не трогает: у них нет rawX/rawY, и это правильно — тесты подают уже
   переведённые координаты напрямую. */
function helmSyncPointer(){
  const p=HELM.P;
  ...
  if(s&&s.rawX!==undefined){
    const xy=helmCanvasXY({clientX:s.rawX,clientY:s.rawY});
    s.x=xy[0];s.y=xy[1];
    helmDrag(s);helmTrail(s);
  }
}
/* ── бегущий центр (M422) ──
...
/* ── стик за кромкой системы (0b, «якорь и стик», ролик 12.09) ──
   За кромкой якорь (17-mode-system) заворачивает курс к звезде. Стик, тянущий
   наружу, с ним боролся: помощь каждый кадр гнала скорость обратно наружу, а
   правило торможения читало загнутую якорем скорость как «игрок тянет против
   хода» и включало тормоз. Равновесие было: корабль полз вдоль кромки десятой
   долей крейсерской, нос на 90° в сторону, топливо горело (сим в 20 строк
   совпал с роликом). Теперь якорь спорит не с состоянием, а с ВВОДОМ: кромка —
   стена. За ней (и в полосе HELM_EDGE_BAND перед ней) наружная часть желания
   убирается, касательная остаётся: стик, клонящий вдоль кромки, ведёт вдоль
   неё; стик строго наружу доводит корабль до стены и держит у неё — нос туда,
   куда тянут, топливо не горит, нижняя строка называет якорь. Мягкая тяга к
   самой линии кромки держит корабль на ней, а не в полосе. Пока стик ведёт
   так, заворот курса в 17-mode-system молчит (c.edge): два руля на одной
   кромке и давали ползок. Накатом, без стика, якорь заворачивает дугой домой.
   Отвергнутое по трассе теста 91a2: (1) отрезать наружу и пустить накатом —
   внутри кромки правило торможения видело «стик наружу, летим внутрь» и гасило
   ход до нуля, выходил маятник; (2) поворачивать упор наружу в скольжение —
   стик смотрит в стену экрана, а не в кромку, и по мере хода у него
   появлялась касательная часть назад: скольжение переворачивалось. */
function helmEdgeInput(c,sh){
  if(G.mode!=="system"||!G.sys||typeof sysEdge!=="function")return;
  const d=Math.hypot(sh.x,sh.y)||1,edge=sysEdge(G.sys);
  if(d<=edge-HELM_EDGE_BAND)return;
  const rx=sh.x/d,ry=sh.y/d,out=c.ax*rx+c.ay*ry;
  if(out<=0)return;                     /* домой — как обычно */
  const am=Math.hypot(c.ax,c.ay);
  const hold=clamp((edge-d)/HELM_EDGE_BAND,-1,1)*.35*am;
  c.ax+=rx*(hold-out);c.ay+=ry*(hold-out);
  c.slow=false;c.edge=true;
  const left=Math.hypot(c.ax,c.ay);
  if(left>am*.25)c.head=Math.atan2(c.ay,c.ax);   /* вдоль стены — нос по ходу */
}
...
function helmTick(dt){
  ...
  c.assist=false;c.ax=0;c.ay=0;c.slow=false;c.edge=false;
  let headBusy=false,input=false;
  /* сырые координаты пальца → пиксели канвы, раз за кадр, не на каждое
     событие движения (18.09, см. helmSyncPointer) */
  helmSyncPointer();
...
function helmApply(dt,st,sh,maxSp){
  ...
  /* маршевый на поддержании (D2, 18.09): помощь держит заданный ход, тяга в
     нуле — но корабль под пальцем без факела читался буксируемым. Пока стик
     в руке и ход выше четверти крейсерской, сопла тлеют слабым ровным факелом */
  o.idle=!!(c.assist&&!o.main&&G.fuel>0&&sp0>maxSp*.25);
  return o;
}
