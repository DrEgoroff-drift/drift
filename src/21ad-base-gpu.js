...
fn field(p:vec2f,uv:vec2f)->vec4f{
  ...
  if(rock>.004||cav>.004){
    /* порода: холодная, в зерне материала, тёплая у выработки.
       Цвет и зерно материала — МНОЖИТЕЛЕМ по нарисованной породе, как задумано в 21ab1:
       у main зерно, валуны и жилы ложились ПОВЕРХ материала, а плёнка сложением сверху
       (поле воздуха) топила их — кратеры теряли обод и тень (Контроль 26.09).
       .393 — яркость бурого тона rockAlb; в среднем множитель ≈1.85 — порода та же по яркости */
    var fr=rockLight(w,lit)*(.9+.2*vn(w/2.2,5u))*1.85
      *(.45+1.3*dot(rockAlb(w),vec3f(.3,.59,.11))/.393)*vec3f(1.10,.98,.853);
    /* крупный слой материала — как второй проход fillMaterial у main (overlay .26, ×3.7):
       на тёмном камне overlay — это множитель .74+.52·s, в среднем единица */
    if(fu.v[2].z>.5){fr*=.74+.52*matL((w+91.)/3.7);}
...
fn field(p:vec2f,uv:vec2f)->vec4f{
  ...
  /* камень: здесь только тёплая кайма и пятна отсеков. Материал — в поле света
     множителем: плёнка сложением поверх зерна и валунов стирала их (Контроль 26.09) */
  let rk=m.r*(1.-m.g);
  if(rk>.004){o+=rk*mspill(w,lit);}
  if(m.g>.02){
    let ci=i32(floor((w.x-OX)/CW));let ri=i32(floor((w.y-OY)/CH));
    let code=cellCode(ci,ri);
