...
function winFigBox(g){
  const m=g.man,x0=Math.floor((g.manx-m*0.27)*DPR),y0=Math.floor((g.flo-m*1.06)*DPR);
  const x1=Math.ceil((g.manx+m*0.27)*DPR),y1=Math.ceil((g.flo+m*0.06)*DPR);
  return {x:x0/DPR,y:y0/DPR,w:(x1-x0)/DPR,h:(y1-y0)/DPR};
}
/* зимовщик — своя выпечка «win.fig», в кадре поверх света комнаты, как у main: печь и лампа
   светят на пол и стены под ним, а не пеленой по ватнику (Контроль 26.09: пальто рыжело,
   валенки теряли контраст, пятно печи на полу смывало тень под ногами) */
function winFigure(g,W0){
  /* ── зимовщик ──
     ...
     мире ночью силуэт совпадал по светлоте с грунтом и пропадал совсем. */
  {
    ...
    /* ореол кадра (08b) кладёт на фигуру у светлой комнаты два-восемь уровней тёплого
       света — пелену, которую Контроль 26.09 видел рыжим пальто. Краска фигуры и тень
       под ногами плотнее на эту добавку: в кадре ватник и валенки не светлее main */
    const DEN=0.93;
    ...
    silh(0,0,wcol(body,(0.52+k*0.52)*DEN));
    ...
    ctx.fillStyle=wcol(pcMix([92,88,80],WIN_C.warm,0.10),(0.42+k*0.44)*(DEN-0.03));
    for(const s3 of [-1,1]){
