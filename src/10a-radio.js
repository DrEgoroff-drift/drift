...
function radioLoad(t){
  ...
  RADIO.tempoK=1;RADIO.keyShift=0;RADIO.modeNow=null;RADIO.lv={pad:1,lead:1,arp:1};
  radioMix(t,true);
...
function radioStep(t){
  ...
  if(s===0){
    ...
    if(RADIO.sec!==w.name){
      ...
      }else{RADIO.modeNow=null;RADIO.keyShift=0;}
  ...
  const key=RADIO.trk.key-12+RADIO.keyShift+(w.last&&w.name!=="out"?(radioA("modUp")|0):0);   // 0 = A2 → бас в первой октаве
