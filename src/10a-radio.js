...
function radioBuild(){
  ...
  const sf=c.createBiquadFilter();sf.type="lowpass";sf.frequency.value=1500;
...
function radioMix(t,instant){
  ...
  const L={pad:(radioA("pad")==="none"||solo?0:(radioA("epic")?1.1:.9))*m.pad*padIn*Q,bass:.9*m.bass*Q,lead:off?0:m.lead*(solo?1.2:1)*Q,
    harm:(sec==="A2"||sec==="B"?.35*m.lead:0)*Q,arp:(radioA("arp")==="none"||solo?0:.55)*m.arp*arpIn*(sec==="brk"?.5:1)*Q,
