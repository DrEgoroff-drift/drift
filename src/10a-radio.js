...
function radioBuild(){
  ...
  const sf=c.createBiquadFilter();sf.type="lowpass";sf.frequency.value=1700;
...
function radioStep(t){
  ...
  if(s===0){
    ...
    if(RADIO.sec!==w.name){
      ...
      /* новый тон и лад были решены заранее и подготовлены переходным аккордом */
      if(RADIO.pending){RADIO.keyShift=RADIO.pending.keyShift;RADIO.modeNow=RADIO.pending.mode;RADIO.pending=null;}
      RADIO.pivot=null;
      RADIO.lv={pad:.7+rr()*.4,lead:.7+rr()*.4,arp:.5+rr()*.6};
    }
    /* ── модуляция через переходный аккорд ──
       За один шаг гармонии (три такта) до конца части решается, куда уйти: тон на ±2
       полутона, лад — к минорным родственникам. На эти три такта пьеса встаёт на
       ДОМИНАНТУ нового тона в виде sus4 с малой септимой — аккорд, который сам «выводит»
       в новую тонику; мелодия на нём молчит, бас берёт его корень. В первой части в
       чужой тон не уходим, в последней — возвращаемся домой тем же способом. */
    {
      const hold0=radioA("bassA")==="seq"?1:3;
      if(w.barIn===w.len-hold0&&w.name!=="out"&&!RADIO.pending){
        const rr=RADIO.r,nxt=RADIO.form[w.i+1],goHome=!nxt||nxt[0]==="out"||w.i+1>=RADIO.form.length-2;
        let ks=RADIO.keyShift,md=RADIO.modeNow;
        if(goHome||w.i===0){ks=0;md=null;}            // из вступления — только в свой тон
        else{if(rr()<.4)md=RADIO_DRIFT_MODES[Math.floor(rr()*RADIO_DRIFT_MODES.length)];if(rr()<.4)ks=[-2,-1,1,2][Math.floor(rr()*4)];}
        RADIO.pending={keyShift:ks,mode:md};
        if(ks!==RADIO.keyShift){const dom=ks-RADIO.keyShift+7;RADIO.pivot={root:dom,semis:[dom,dom+5,dom+7,dom+10]};}
      }
    }
  ...
  /* гармония идёт в ногу с басом: аккорд держится три такта, как и его дрон — иначе бас
     на корне первого такта спорил с аккордами второго и третьего */
  const hold=radioA("bassA")==="seq"?1:3,hb=Math.floor(w.barIn/hold);
  const cd=prog[hb%prog.length],nextCd=prog[(hb+1)%prog.length];
  /* когда сменится аккорд: голоса режут хвосты к этому моменту, чтобы нота не тянулась
     в чужую гармонию (radioCap) */
  RADIO.chordEnd=t+((hold-w.barIn%hold)*B-s)*radioS16();
  const PV=RADIO.pivot;
  ...
  if(s===0&&!drop)radioPad(t,key,cd,radioS16()*B,(w.name==="B"&&radioA("padB"))||radioA("pad"),PV&&PV.semis);
  if(PV){if(s===0&&bar%3===0)radioBassVoice(t,key+PV.root-12,radioS16()*B*3*.9,1,"drone",0);}
  else radioBass(t,s,key,cd,w,nextCd);
  if(!drop&&!PV)radioArp(t,s,key,cd);
  if(!drop)radioDrums(t,s,w);
  if(!PV)radioMelody(t,s,w,key,cd);
  if(s===0&&r()<.6&&!PV)radioBell(t+radioS16()*Math.floor(r()*B),key,cd);
  /* маяк: раз в несколько тактов — одна звонкая высокая нота с эхом, как позывной */
  if(s===0&&bar%2===1&&r()<.6&&!PV)radioBeacon(t+radioS16()*Math.floor(r()*8),key,cd);   // примерно раз в четыре такта
  if(s===0&&bar%8===0&&radioA("waves"))radioWave(t,radioS16()*B*2);
  /* обертоновое облако: раз в минуту-полторы, на ~20 с — см. radioSpectral */
  if(s===0&&!PV&&(RADIO.specNext==null||t>=RADIO.specNext)&&w.name!=="out"){RADIO.specNext=t+50+r()*50;radioSpectral(t,key,cd);}
}
...
function radioMelody(t,s,w,key,cd){
  ...
  const mel=RADIO.mel[k],n0=mel[Math.floor(w.barIn/2)%4][w.barIn%2].find(x=>x.st===s);
  if(!n0)return;
  const dur=n0.dur*radioS16()*.92;
  /* длинная нота (от четверти такта) — только звук аккорда: проходящая ступень, повисшая
     на секунды поверх аккорда, и есть та фальшь; короткие могут пройти мимо */
  let d=n0.d;
  if(n0.dur>=4){const q=((d%7)+7)%7;if(q!==0&&q!==2&&q!==4)d+=(q===1||q===3)?1:(q===5?-1:1);}
  /* мелодия на октаву выше тоники, не на две: ниже — темнее */
  radioLead(t,key+12+radioDeg(cd+d),dur,"lead");
  /* подголосок — ближайший звук аккорда снизу, а не «на две ступени ниже» */
  if(w.name!=="A"){const q=((d%7)+7)%7,dn=q===0?-3:q<=2?0:q<=4?2:4;radioLead(t,key+12+radioDeg(cd+d-q+dn),dur,"harm");}
}

/* ── голоса ── */
/* нота и смена аккорда: если хвост (rel) не дотухает через полсекунды после смены —
   нота укорачивается, но не больше чем на треть; иначе она не начинается вовсе (-1):
   сдавленный обрубок в 0.15 с слышался как «захлебнулось» */
function radioCap(t,dur,rel){
  const lim=(RADIO.chordEnd||0)-t;
  if(lim<=0||dur+rel<=lim+.5)return dur;
  const need=lim+.5-rel;
  return need>=dur*.66?need:-1;
}
/* для подложки: не пропускать, а поджать хвост — не короче 0.8 с */
function radioCapRel(t,hold,rel){
  const lim=(RADIO.chordEnd||0)-t;
  if(lim<=0||hold+rel<=lim+.5)return rel;
  return Math.max(.8,lim+.5-hold);
}
...
function radioPad(t,key,cd,dur,kind,semis){
  ...
  if(kind==="cluster"){
    ...
    for(let i=0;i<5;i++){
      ...
      o.type="sine";o.frequency.value=midiHz(key+12+(semis?semis[i%semis.length]+12*(i>>2):radioDeg(cd+[0,2,4,7,9,6,8][i])))*(1+(RADIO.r()-.5)*.006);   // расстройка в 5 центов, не в четверть тона
      lg.connect(g.gain);p.pan.value=(i/4)*1.4-.7;
      const end=radioEnv(g.gain,t,2,.016,dur-.5,radioCapRel(t,dur-.5,3));last=Math.max(last,end);
  ...
  {const c=SND.ctx,o=c.createOscillator(),g=c.createGain(),p=c.createStereoPanner();
    o.type="sine";o.frequency.value=midiHz(key+36+(semis?semis[0]:radioDeg(cd+[0,4,7][Math.floor(RADIO.r()*3)])));
    p.pan.value=RADIO.r()*1.2-.6;const end=radioEnv(g.gain,t+.5,2.5,.004,dur-1,radioCapRel(t+.5,dur-1,3));
    o.connect(g);g.connect(p);p.connect(RADIO.layers.bell);o.start(t+.5);o.stop(end+.1);}
  (semis?[0,1,2,3]:VO[Math.floor(RADIO.r()*VO.length)]).forEach((iv,i)=>{
    t+=i?.04+RADIO.r()*.05:0;                    // голоса вступают вразнобой, а не залпом
    const f=midiHz(key+(kind==="choirhi"?12:0)+(semis?semis[iv]:radioDeg(cd+iv))),g=c.createGain(),p=c.createStereoPanner();   // октавой ниже солиста: середина — ему
    p.pan.value=-.4+i*.27;
    const org=kind==="organ",hold=dur-.1,rel=radioCapRel(t,hold,org?1:2.6);
    const end=radioEnv(g.gain,t,org?.3:1.4,org?.012:.018,hold,rel);   // растушёвка: наплыв полторы секунды, хвост две с половиной
    let dst=g;
...
function radioBassVoice(t,midi,dur,acc,pat,s){
  ...
  if(tone==="drone"){
    ...
    const tail=Math.min(2,Math.max(.8,(RADIO.chordEnd||t+dur+2)-t-dur+.5));   // хвост дотухает к смене аккорда
    g.gain.exponentialRampToValueAtTime(.0001,t+dur+tail);const end=t+dur+tail;
...
function radioArp(t,s,key,cd){
  ...
  if(kind==="drip"){        // капля: короткий синус с падением высоты, редко
    ...
    const c=SND.ctx,o=c.createOscillator(),g=c.createGain(),p=c.createStereoPanner();
    const dh=midiHz(key+48+radioDeg(cd+[0,2,4,7][Math.floor(RADIO.r()*4)]));
  ...
  else{if(RADIO.r()>.28)return;d=[0,2,4,7,9,11][Math.floor(RADIO.r()*6)];}
...
function radioLead(t,midi,dur,layer){
  ...
  const rel=fl?.9:du?1.4:1.2;dur=radioCap(t,dur,rel);
  if(dur<0)return;                                            // не успеет дозвучать до смены аккорда — пауза
  const end=radioEnv(g.gain,t,fl?.3:du?.4:br?.3:th?.5:.4,harm?.016:fl?.034:du?.03:br?.028:.026,dur,rel);
...
function radioBellVoice(t,fr,len,dst,peak){
  {const lim=(RADIO.chordEnd||0)-t;if(lim>0)len=Math.min(len,Math.max(.6,(lim+.5)/1.5));}   // колокол укорачивается, не пропадает
...
function radioBeacon(t,key,cd){
  ...
  const K=Math.max(2,Math.min(6,Math.floor(((RADIO.chordEnd||t+9)-t)/gap)));   // эхо не заходит в следующий аккорд
  const wob=c.createOscillator(),wg=c.createGain();wob.frequency.value=.25+r()*.2;wg.gain.value=fr*.004;wob.connect(wg);
  const dry=c.createGain();dry.gain.value=.4;dry.connect(RADIO.bus);
  for(let k=0;k<K;k++){
    const tk=t+k*gap,v=[.056,.032,.019,.011,.006,.004][k];
...
function radioBell(t,key,cd){
  radioBellVoice(t,midiHz(key+48+radioDeg(cd+[0,2,4][Math.floor(RADIO.r()*3)])),2.2,RADIO.layers.bell,.032);
}
/* ── обертоновое облако ──
...
function radioSpectral(t,key,cd){
  const c=SND.ctx,r=RADIO.r,base=midiHz(key-12+radioDeg(cd));
  const minor=(radioDeg(cd+2)-radioDeg(cd))%12===3;      // 19-й партиал — малая терция: только над минорным аккордом
  const P=[2,3,4,6,8,9,12,16,19,24].filter(n=>(n!==19||minor)&&r()<.6);
  ...
  for(const n of P.slice(0,7)){
    ...
    const v=(n<6?.03:.045)/Math.sqrt(n);    // низкие партиалы тише: они в середине, где и так тесно
    lfo.frequency.value=.05+r()*.12;lg.gain.value=v*.5;lfo.connect(lg);lg.connect(g.gain);
    p.pan.value=r()*1.6-.8;
    const room=Math.max(8,(RADIO.chordEnd||t+20)-t),a=Math.min(4+r()*2,room*.25),rel=Math.min(6+r()*2,room*.3),h=Math.max(1,room-a-rel-1),t0=t+r()*.5;
