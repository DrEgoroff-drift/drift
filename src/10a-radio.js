...
function radioStep(t){
  ...
  if(s===0&&bar%2===0&&r()<.5&&!PV)radioBell(t,key,cd);
  /* маяк: раз в несколько тактов — одна звонкая высокая нота с эхом, как позывной */
  if(s===0&&bar%2===1&&r()<.45&&!PV)radioBeacon(t+radioS16()*Math.floor(r()*8),key,cd);   // примерно раз в четыре такта
...
/* ── голоса ── */
/* укоротить ноту так, чтобы её хвост (rel) дотух через полсекунды после смены аккорда */
function radioCap(t,dur,rel){
  const lim=(RADIO.chordEnd||0)-t;
  if(lim<=0)return dur;
  return dur+rel>lim+.5?Math.max(.15,lim+.5-rel):dur;
}
...
function radioPad(t,key,cd,dur,kind,semis){
  ...
  if(kind==="cluster"){
    ...
    for(let i=0;i<5;i++){
      ...
      const end=radioEnv(g.gain,t,2,.016,radioCap(t,dur-.5,3),3);last=Math.max(last,end);
  ...
  {const c=SND.ctx,o=c.createOscillator(),g=c.createGain(),p=c.createStereoPanner();
    ...
    p.pan.value=RADIO.r()*1.2-.6;const end=radioEnv(g.gain,t+.5,2.5,.004,radioCap(t+.5,dur-1,3),3);
  ...
  (semis?[0,1,2,3]:VO[Math.floor(RADIO.r()*VO.length)]).forEach((iv,i)=>{
    ...
    p.pan.value=-.4+i*.27;
    const org=kind==="organ",rel=org?1:2.6,hold=radioCap(t,dur-.1,rel);
...
function radioBassVoice(t,midi,dur,acc,pat,s){
  ...
  if(tone==="drone"){
    ...
    const tail=Math.min(2,Math.max(.4,(RADIO.chordEnd||t+dur+2)-t-dur+.5));   // хвост дотухает к смене аккорда
