...
const HULL_CLASS={
  ...
  scout:  {ru:"разведчик",     bw:.85,len:1.00,wing:[1,2],nac:.40,notch:.55,dish:1,boom:1,wsp:.95},
  ...
  warship:{ru:"фрегат",        bw:1.16,len:1.12,wing:[2,2],nac:.74,notch:.50,guns:1,armor:1,wsp:.86},
  yacht:  {ru:"яхта",          bw:.74,len:1.34,wing:[0,0],nac:.85,notch:.10,win:1,fin:1,pylon:1,wsp:.7},
  survey: {ru:"исследователь", bw:.92,len:1.14,wing:[0,1],nac:.52,notch:.40,dish:1,panel:1,wsp:.8}
};
...
function drawHullMarks(h){
  ...
  if(M.guns)for(const g of M.guns)for(const s of [1,-1]){
    const y=g[1]*s;
    ctx.strokeStyle=rgba(h.col,.75);ctx.lineWidth=1.4;
    ctx.beginPath();ctx.moveTo(g[0]-1,h.bw*.5*s);ctx.lineTo(g[0]-1,y);ctx.stroke();
    ctx.fillStyle=rgba(h.body,1);
    ctx.strokeStyle=rgba(h.lite,.5);ctx.lineWidth=.8;
    ctx.beginPath();ctx.rect(g[0]-4.5,y-2.2*s-(s>0?0:0),9,4.4*s);
    ctx.fill();ctx.stroke();
    ctx.strokeStyle=rgba(h.lite,.8);ctx.lineWidth=1.9;
    ctx.beginPath();ctx.moveTo(g[0]+1,y);ctx.lineTo(g[0]+g[2],y);ctx.stroke();
    ctx.fillStyle=rgba(h.dark,1);
    ctx.beginPath();ctx.arc(g[0]+g[2],y,1.5,0,TAU);ctx.fill();
  }
  if(M.armor){
  ...
  if(M.panel)for(const s of [1,-1]){
    ...
    ctx.strokeStyle=rgba(h.col,.6);ctx.lineWidth=1.1;
    ctx.beginPath();
    ctx.moveTo(p.x-p.l*.22,h.bw*.5*s);ctx.lineTo(p.x-p.l*.16,y0);
    ctx.moveTo(p.x+p.l*.22,h.bw*.5*s);ctx.lineTo(p.x+p.l*.16,y0);
    ctx.stroke();
    ctx.fillStyle="rgba(34,58,96,.9)";
    ctx.strokeStyle=rgba(h.lite,.55);ctx.lineWidth=.9;
  ...
  if(M.boom){
    const b=M.boom;
    ctx.strokeStyle=rgba(h.lite,.55);ctx.lineWidth=b.r*1.6;
...
function drawHull(id,thrusting,braking,lvl,bank){
  ...
  for(const w of h.wings)for(const s of [1,-1]){
    ...
    ctx.strokeStyle=rgba(h.col,.45);ctx.lineWidth=.7;ctx.stroke();
  ...
  for(const n of h.nacs)for(const s of [1,-1]){
    ...
    /* пилон: у яхты крыла нет вовсе, и гондола держится на тонком кронштейне.
       ...
       остальных там крыло */
    if(h.mark.pylon){
      ctx.strokeStyle=rgba(h.col,.8);ctx.lineWidth=1.5;
      ctx.beginPath();
    ...
    ctx.fillStyle=g;ctx.fill();
    ctx.strokeStyle=rgba(h.col,.7);ctx.lineWidth=.75;ctx.stroke();
  ...
  ctx.restore();
  ctx.strokeStyle=rgba(h.col,.62);ctx.lineWidth=.65;
