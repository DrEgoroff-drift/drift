...
/* FLEET_KEEP — кораблей в кэше (по две выпечки, ~0.6 МБ каждая у «поста»): новый посев каждые 10 минут.
   Обход 15 систем (26.09): до 7 кораблей на систему — 24 держат три системы */
...
function* fleetArtJob(f){
  ...
  const rad=L*.75,side=Math.ceil(rad*2*FLEET_SS);
  /* износ считан заранее: выпечка повторяется при подъёме устройства, а поток r() — один на
     корабль (порядок вызовов прежний — заплаты те же, что у 2D) */
  const wear=[];
  for(let i=0;i<3+Math.floor(r()*4);i++){
    ...
    wear.push("rgba("+(C[q.c][0]-14)+","+(C[q.c][1]-10)+","+(C[q.c][2]-4)+",.9)",
      p[0]+(r()-.5)*L*.06,p[1]+(r()-.5)*hw*.6,L*.03+r()*L*.03,hw*.25+r()*hw*.3);
  }
  /* выпечка на GPU-холсте (25.09): кисть та же, ctx на время выпечки — GPU-холст, текст — v2.
  ...
  const body=()=>{
    ...
    for(let i=0;i<wear.length;i+=5){ctx.fillStyle=wear[i];ctx.fillRect(wear[i+1],wear[i+2],wear[i+3],wear[i+4]);}
...
const FLEET_ENG=1.3,FLEET_HALO=.45,FLEET_LOD=-1.2;   /* мельче экрана на −1.2 ступени и маска по яркости (sharp, 17c): резкость +0.6…+5.2 % против прежних −1.2 без маски, свет −0.8…+0.1 %, рябь на полупикселе 10.3 против 10.4; −1.0 терял до 2.8 % резкости, −.8 — до 5 % (ворота 25.09) */
...
function fleetShipAt(f,art,ma,mb,mc,md,me,mf,al){
  ...
  for(const li of art.lights){const [px,py]=T(li.x,li.y);
    if(li.c==="nav"){const c=li.g?[120,240,150]:[255,90,80];
      const a=(Math.sin(G.t*.05+(li.g?1.6:0))>.6?.95:.2)*al;
      L.push([1,px,py,1.4*s,0,0,0,c[0],c[1],c[2],a]);A.push([1,px,py,.7*s,0,0,2.6*s,c[0],c[1],c[2],a*FLEET_HALO]);}
    else if(li.c==="win"){const a=((Math.sin(G.t*.03+li.x)>-.4)?.9:.4)*al;
      L.push([1,px,py,1.3*s,0,0,0,255,228,170,a]);A.push([1,px,py,.6*s,0,0,2.2*s,255,214,150,a*FLEET_HALO]);}
...
/* корабль флота в текущей матрице ctx (флот, жесты, мирный флот): только проход сцены (25.09) — 2D-ветки нет */
function drawFleetShip(f){fleetShipGpu(f,fleetArtOf(f));}
