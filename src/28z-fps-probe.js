...
function gpuTs(name){
  ...
  const T=GPU.tsQ||(GPU.tsQ={qs:GPU.dev.createQuerySet({type:"timestamp",count:64}),
    res:GPU.dev.createBuffer({size:512,usage:GPUBufferUsage.QUERY_RESOLVE|GPUBufferUsage.COPY_SRC}),rb:[],names:[],n:0});
  if(T.n+2>64)return undefined;
...
function gpuTsResolve(){
  ...
  if(!rb){if(T.rb.length>=4)return null;
    rb=GPU.dev.createBuffer({size:512,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST});T.rb.push(rb);}
