...
function baseEventApply(B,e,n){
  ...
  if(e.k==="vent"){
    /* на базе без людей выброс не отнимает ничего (M391, набор 91zzzw-base):
       запас воздуха — это запас ДЛЯ ЛЮДЕЙ, и заперт он тем же правилом, что
       течь атмосферы в `baseLifeStep` («leak && baseCrewN»). Иначе всякий,
       кто заложил базу и улетел, возвращался бы к развалине, ни разу не
       согласившись на эту игру, — то же, из-за чего пожару на пустой базе в
       0.409.1 вышел срок. Событие не случилось: и в журнал не пишем. */
