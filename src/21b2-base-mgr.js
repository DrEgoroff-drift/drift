...
const BMGR_DUE=4;            /* столько смен без жалованья он терпит */
/* ── бросок (§48.1) ──
...
function bmgrStep(B,n){
  ...
  B._earned=0;
  /* ── долг, а не мгновенный расчёт (разбор 0.409.1) ──
     Списывалось каждую смену догона, и на первой же неоплаченной он уходил:
     вернулся из недельного рейса с пустым счётом — управляющего нет, и узнаёшь
     об этом постфактум. Как у наёмников: неоплаченное копится, и уходит он
     после четырёх смен без денег. */
  if(cost>0){
    if(G.credits>=cost)G.credits-=cost;
    else{
      B.mgr.due=(B.mgr.due|0)+1;
      if((B.mgr.due|0)>=BMGR_DUE){
        B.mgr=null;
        baseLog(B,"mgrgo",n,{who:M.name});
        logAdd("warn",M.name+" ушёл с базы «"+B.name+"»: жалованье не платили четыре смены");
        return 1;
      }
      return 0;
    }
    if(B.mgr)B.mgr.due=0;
  }
