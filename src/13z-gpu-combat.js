  ...
  for(var b=0;b<6;b++){
    ...
    if(a<1.3){
      ...
      for(var i=0;i<26;i++){
        ...
        let wd=select(.3,.65,lg)*mix(.12,1.,q);let tail=q*q;   /* хвост сходит в ноль и по ширине, и по яркости */
        em=em+lad(.3+.45*tt)*(.4+3.*tt*tt)*select(1.1+2.2*h3,2.2,lg)*tail*exp(-ds*ds/wd)*clamp(3./max(ln,1.),select(.35,.6,lg),1.);
