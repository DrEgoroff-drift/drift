...
function applySaveRest(s){
  ...
  G.farTaken=asMap(s.farTaken);   /* выбранное из дальних залежей (M466) */
  G.draft=asMap(s.draft);
  G.smenaAt=asMap(s.smenaAt);   /* где прожита глава и её снимок (P15) */
  G.tapeRoll=s.tapeRoll|0;G.tapes=asMap(s.tapes);   /* изолента (M486) */   /* чертежи кораблей — правка игрока (M477); нет записи — упаковщик */
  ...
  resize();
  if(!G.opts.audio||typeof G.opts.audio!=="object")G.opts.audio={on:true,music:.2,sfx:.6,engine:.4};
  if(typeof G.opts.audio.music!=="number")G.opts.audio.music=.2;   /* 20 %: радио — фон, не концерт */
  ...
  if(!["tracks","gen","ambient"].includes(G.opts.audio.src))G.opts.audio.src="gen";
