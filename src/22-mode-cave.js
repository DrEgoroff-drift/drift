...
function updateCave(dt){
  ...
  /* стена у устья (M210): НЕ в самом устье — там ДЕЙСТВИЕ уже выводит наружу.
    ...
    /* клавиша в скобках — живая (ctlHint): на телефоне её нет вовсе, на
       компьютере после переназначения буква врала */
    ...
    /* ходьба и ранец называются по-разному на разных руках: клавиши — только
       там, где они есть; пэды зовутся своими стрелками */
    const walk=(typeof ctlHint==="function")
      ?ctlHint("left","◀")+" "+ctlHint("right","▶"):"A D";
    const jet=(typeof ctlHint==="function")?ctlHint("thrust","▲"):"W";
    if(C.y>caveGalY(C,C.x)+260)
      G.prompt=walk+" — ИДТИ · "+jet+" — РАНЕЦ · НАХОДКА В ЛЕВОМ КОНЦЕ НИЖНЕЙ ГАЛЕРЕИ";
    ...
    else G.prompt=walk+" — ИДТИ · "+jet+" — РАНЕЦ · ШАХТЫ ВЕДУТ ВНИЗ · НАРУЖУ — ДЕЙСТВИЕ У УСТЬЯ";
  }
}
