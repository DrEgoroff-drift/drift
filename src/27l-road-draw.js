...
function drawRoad(ts){
  ...
  /* ── сияет НИЖНИЙ КРАЙ (M168k, вторая правка по словам автора) ──
     ...
     и справедливо. Из референсов автора («моя волна») видно другое устройство:
     светится САМА КРОМКА по всей ширине, из неё поднимаются несколько цветных
     плюмажей, выше — чернота. Насыщенно, но невысоко: свет не лезет на экран.

     Поэтому:
     1. Кромка — ровная полоса света во всю ширину, тон настроения.
     2. Пять ПЛЮМАЖЕЙ, узких по горизонтали и высоких: круглые пятна шире, чем
        расстояние между ними, и под «lighter» усредняются в белесый ком —
        узкие сходятся только основаниями, а выше держат каждый свой тон.
     3. Веер по спектру — поверх, короткий.
     Тёмной полосы на холсте больше нет: кнопки держит своё стекло подвала
     (`body.road .scr footer`), а не вырезанный из картинки кусок. */
  c.save();c.globalCompositeOperation="lighter";
  const ay=H*1.03;                         /* центры чуть ниже кромки — светится сама кромка */
  const hs=h2=>((h2%360)+360)%360;
  const bd=roadBands(RD.wave);
  RD.flow=(RD.flow||0)+dt*(.09+bd.bass*.34);   /* время шума: бас гонит течение */
  /* 1. ровное свечение по всей кромке */
  {
    const gh=H*(.035+en*.035+bd.bass*.03);
    const eg=c.createLinearGradient(0,H-gh,0,H);
    eg.addColorStop(0,"hsla("+hs(hue)+",92%,52%,0)");
    eg.addColorStop(1,"hsla("+hs(hue)+",92%,54%,"+(.09+en*.10+bd.bass*.07+RD.beat*.07).toFixed(3)+")");
    c.fillStyle=eg;c.fillRect(0,H-gh,W,gh);
  }
  /* 2. плюмажи — «жидкие», а не airbrush-овальные.
     Форму ведёт fbm2 из 01-core: тот же приём, что в шейдерном рецепте даёт
     шум Перлина, только считается на пять чисел в кадре, а не на миллион
     пикселей. Каждый плюмаж — три доли со СВОИМИ смещениями по шуму: у
     объединения нет жёсткой кромки (у каждой доли градиент сам уходит в ноль),
     а силуэт всё время течёт. Полосы разведены по делу:
       бас   — высота и скорость течения (низ двигает массу),
       верх  — быстрая мелкая рябь по краю,
       серёдка — общая плотность.
     Высота растёт с энергией СЛАБО: автору нравится, как это выглядит на
     стоянке, а там света мало и он не заливает кадр. */
  const R0=H*(.24+en*.17+bd.bass*.06+RD.beat*.05);
  const lobe=(bx,by,rw,rh,hb,a0)=>{
    if(a0<.004||rh<2)return;
    c.save();
    c.translate(bx,by);c.scale(rw/rh,1);
    const g2=c.createRadialGradient(0,0,0,0,0,rh);
    g2.addColorStop(0,"hsla("+hs(hb)+",97%,56%,"+a0.toFixed(3)+")");
    g2.addColorStop(.45,"hsla("+hs(hb)+",94%,46%,"+(a0*.44).toFixed(3)+")");
    g2.addColorStop(1,"hsla("+hs(hb)+",90%,40%,0)");
    c.fillStyle=g2;c.fillRect(-rh,-rh,rh*2,rh*2);
    c.restore();
  };
  for(let i=0;i<5;i++){
    const f=(i+.5)/5;
    /* середина тише прочих: по центру идёт шлейф, и два света в одном месте
       гасят друг друга — тот же разбор, что и у ленты */
    const q=i===2?.70:1;
    for(let k=0;k<3;k++){
      const n1=fbm2(i*3.7+k*11.3,RD.flow,0x51A0,3);      /* смещение вдоль кромки */
      const n2=fbm2(i*5.1+k*7.9+40,RD.flow*.8,0x51A1,3); /* полнота доли */
      const rip=Math.sin(RD.flow*9.5+i*2.3+k*1.9)*bd.tre*.22;   /* рябь от верхов */
      const bx=W*f+(n1-.5)*W*.13+(k-1)*W*.045;
      const by=ay+(n2-.5)*H*.03;
      const g=(.70+n2*.55+rip);
      lobe(bx,by,W*.135*g,R0*g*(i===2?.92:1),hue+ROAD_BLOOM_H[i],
           (.17+en*.15+bd.mid*.08)*q);
    }
  }
  /* 3. веер по спектру — короткий, поверх плюмажей */
  const ax=W*.5+Math.sin(t*.07)*W*.04;
  for(let i=0;i<7;i++){
    const wv=RD.wave[Math.min(27,Math.floor(i*27/6))];
    const ang=-Math.PI/2+(i-3)*.28+Math.sin(t*.31+i*2.1)*.05;
    const L=H*(.11+.22*wv)*(1+RD.beat*.18);
    const hr=hs(hue+(i-3)*44);
    c.save();
    c.translate(ax,H*1.005);c.rotate(ang);c.scale(1,Math.max(.04,(.14-.011*Math.abs(i-3))));
    const rg=c.createRadialGradient(0,0,0,0,0,L);
    rg.addColorStop(0,"hsla("+hr+",92%,58%,"+(.12+wv*.20+en*.07).toFixed(3)+")");
    rg.addColorStop(1,"hsla("+hr+",88%,50%,0)");
