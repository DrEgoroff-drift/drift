/* ══════════════ почта: провод ══════════════
   ...
   таймеру и не держит соединение: раз пристыковались — один поход, который
   разом забирает ответы и, если повезло, приносит чужую карточку.
...
function mailAll(){
  if(!G.mail||typeof G.mail!=="object")G.mail={st:[],day:"",sent:0,seen:0};
...
/* ── стыковка: один поход ──
   Забирает ответы и заодно тянет из кучи чужую карточку. С M191 ловля уедет
   в вечерний эфир и станет событием, а не побочным делом стыковки. */
function mailDock(){
  if(!mailOn())return;
  const now=Date.now();
  ...
  mailCall("in").then(j=>{
    ...
    mailPush(String(j.ch),j.card,false);
    mailAll().seen=(mailAll().seen|0)+1;
    logAdd("ether","Из общей почты выпала чужая карточка");
    tell("good","Чужая карточка","ЧУЖАЯ КАРТОЧКА\nиз общей почты\nстопка на столе");
    if(tableOpenNow)tableRender();
  });
}
