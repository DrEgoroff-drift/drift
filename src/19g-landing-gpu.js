...
/* ── корабль: выпечка тела и свет мира на ней ──
   ...
   cos/sin кивка; V2 — к звезде в осях корабля, df, ambK; V3 — звезда, сила факелов;
   V4 — небо; V5 — отсвет земли; V6 — факел (оси корабля), сила, радиус; V7 — стекло
   кабины (оси кивка); V8 — сопло 1 (оси кивка), шаг до второго, радиус; V9 — жар сопел, маяк x,y,
...
const LGF=vec3f(1.,.64,.36);   /* цвет факела */
...
fn lgLander(p:vec2f)->vec4f{
  ...
  if(V[6].z>.002){
    let v=q-V[6].xy;
    em=em+LGF*V[6].z*.10*exp(-dot(v,v)/(V[6].w*V[6].w*.9));
  }
  /* три тормозных факела из брюха (оси кивка): x — доли полудлины V4.w, срез
     сопла V5.w, радиус от сопла кормы, длина V11.w, сила V3.w */
  if(V[3].w>.01){
  ...
  /* ночью корпус не тонет в фон: холодный пол света — звёзды и отсвет неба. С полом
     втрое тише корпус в ночь садился почти чёрным силуэтом — 42 % яркости 2D (пара
     флота 26.09, мерило Контроля — не ниже 60 %); верх борта берёт небо сильнее брюха (sky) */
  let nk=1.-min(V[2].z*1.6,1.);
  li=li+vec3f(.38,.43,.55)*nk*sky;
  if(V[6].z>.002){
    let v=V[6].xy-q;let r2=dot(v,v);
    let fd=max(dot(n,normalize(vec3f(v,5.))),0.);
    li=li+LGF*V[6].z/(1.+r2/(V[6].w*V[6].w))*(.25+.75*fd);
  }
  var c=alb*li*1.25;
  /* кайма: кромка, повёрнутая к звезде, горит узкой нитью её цвета; ночью верхние
     кромки берут холодную нить неба — силуэт читается и без звезды */
  let n2u=n2/max(length(n2),1e-4);
  let rim=tl*pow(max(dot(n2u,normalize(sd)),0.),2.)*V[2].z*.5;
  c=c+V[3].rgb*rim+vec3f(.26,.32,.44)*tl*pow(max(-n2u.y,0.),2.)*nk*.5;
  ...
  /* стекло кабины светится изнутри: ночью — почти своим цветом, днём его перекрывает
     звезда. Голубое в рамке кабины — стекло, борт вокруг него не голубой */
  if(qi.x>V[7].x&&qi.x<V[7].z&&qi.y>V[7].y&&qi.y<V[7].w&&alb.b>alb.r+.06){c=max(c,alb*(.30+.60*nk));}
...
function lgLander(L,tr,camx,camy,p){
  ...
  U[15]=LG_FIRE*lvl;
  /* стекло кабины — рамка, как её рисует drawLander (19f): у диска купол, у плиты
     рубка на носу, у остальных остекление впереди */
  const fm=hullOf(G.shipId).form||"swept",disc=fm==="disc",flat=fm==="slab"||fm==="boxed";
  U[28]=half*(disc?-.22:flat?.46:.34);U[30]=half*(disc?.38:flat?.84:.72);
  U[29]=tY+bodyH*(disc?-.30:flat?-.28:.08);U[31]=tY+bodyH*(disc?.04:flat?.12:.40);
  const er=bodyH*.24;
