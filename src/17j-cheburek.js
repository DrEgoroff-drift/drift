...
function chebBake(){
  ...
  const mk=()=>{const q=document.createElement("canvas");q.width=CHEB_BW*CHEB_PX;q.height=CHEB_BH*CHEB_PX;
    const g=q.getContext("2d");g.setTransform(CHEB_PX,0,0,CHEB_PX,-CHEB_X0*CHEB_PX,-CHEB_Y0*CHEB_PX);return [q,g];};
  const [cv,c]=mk(),[em,e]=mk();
...
function chebSignBake(F,k){
  ...
  g=cv.getContext("2d");g.setTransform(d,0,0,d,0,0);g.clearRect(0,0,cv.width,cv.height);
...
function drawCheburek(zx,zy,Z){
  ...
  if(pass){
    ...
    gpuImage(pass,S.cv,[{x:sl+sw/2,y:st+sh/2,w:sw,h:sh}],{ver:S.key});
    return;
  }
  ctx.save();ctx.translate(x,y);ctx.rotate(C.a);ctx.scale(s,s);
  ctx.drawImage(A.cv,CHEB_X0,CHEB_Y0,CHEB_BW,CHEB_BH);
  ctx.globalCompositeOperation="lighter";ctx.globalAlpha=br;ctx.drawImage(A.em,CHEB_X0,CHEB_Y0,CHEB_BW,CHEB_BH);
  ctx.restore();ctx.save();
  for(const t of steam){ctx.fillStyle=rgba([t[7],t[8],t[9]],t[10]);ctx.beginPath();ctx.arc(t[1],t[2],t[3]+t[6]*.5,0,TAU);ctx.fill();}
  ctx.globalCompositeOperation="lighter";
  for(const t of warm.concat([noz])){ctx.fillStyle=rgba([t[7],t[8],t[9]],t[10]);ctx.beginPath();ctx.arc(t[1],t[2],t[3]+t[6]*.5,0,TAU);ctx.fill();}
  ctx.restore();
  ctx.strokeStyle="rgba(200,190,170,.45)";ctx.lineWidth=.8;ctx.beginPath();
  ctx.moveTo(x-S.tw*.3,by+7.5*k);ctx.lineTo(x-4*s,y-6*s);ctx.moveTo(x+S.tw*.3,by+7.5*k);ctx.lineTo(x+4*s,y-6*s);ctx.stroke();
  ctx.drawImage(S.cv,sl,st,sw,sh);
}
