...
function drawCheburek(zx,zy,Z){
  ...
  const tw=ctx.measureText("ЧЕБУРЕКИ").width+8*k;
  ctx.strokeStyle="rgba(200,190,170,.45)";ctx.lineWidth=.8;ctx.beginPath();
  ctx.moveTo(x-tw*.3,by+5*k);ctx.lineTo(x-4*s,y-6*s);ctx.moveTo(x+tw*.3,by+5*k);ctx.lineTo(x+4*s,y-6*s);ctx.stroke();
  ctx.save();ctx.translate(x,by);ctx.rotate(-.04);
  ctx.fillStyle="rgba(0,0,0,.45)";ctx.fillRect(-tw/2+1.5,-5.5*k+1.5,tw,11*k);
  ctx.fillStyle="#efe3c6";ctx.fillRect(-tw/2,-5.5*k,tw,11*k);
  ctx.fillStyle="#b8322a";ctx.fillText("ЧЕБУРЕКИ",0,.5);
  ctx.restore();
}
