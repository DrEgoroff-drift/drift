...
function roadFinish(){
  ...
  if(cr>0){
    ...
    if(typeof recordAdd==="function")
      /* .toFixed(0), а не .toFixed(1) как у той же цифры строкой выше (П6,
         §4.4): короткая поездка честно заработала cr>0, но округлялась до
         «0 км» — читалось как деньги из ниоткуда */
      recordAdd("дорога","командировочные: "+cr+" кр за "+km.toFixed(1)+" км"+
