...
function drawRoad(ts){
  ...
  c.restore();
  /* звёзды текут навстречу; на экспрессе тянутся вдвое, бит рождает новые */
  const r=rng(0x50AD);
  const streak=tier===2?2.2:tier===3?4:1;
  for(let i=0;i<90;i++){
    const depth=.25+r()*.75,y=r()*H;
    const x=(r()*W-t*(20+Math.min(spd,300)*3)*depth)%W;
    c.globalAlpha=.25+depth*.6;
    c.fillStyle=depth>.8?"#cfe3ea":"#7d8fa0";
    c.fillRect(((x%W)+W)%W,y,1+fast*14*depth*streak,depth>.8?1.6:1);
  }
  c.globalAlpha=1;
  /* гипердрайв (самолёт): звёздный тоннель — росчерки сходятся в точку по
     курсу, и корпус идёт в световом коконе */
  if(tier===3){
    const vx=W*.78,vy=H*.42;
    c.save();c.globalCompositeOperation="lighter";
    const wr=rng(0x3A9F);
    for(let i=0;i<46;i++){
      const a2=wr()*TAU,rr0=(60+wr()*Math.max(W,H))* (1+((t*1.6+wr())%1));
      const x0=vx+Math.cos(a2)*rr0,y0=vy+Math.sin(a2)*rr0*.7;
      const k2=.72;
  ...
  if(tier===3){
    ...
    c.fillStyle=cg;c.beginPath();c.ellipse(0,0,h.len*sc*.75,h.bw*sc*1.6,0,0,TAU);c.fill();
    c.restore();
  }
  c.scale(sc,sc);
