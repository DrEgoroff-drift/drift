...
/* масштаб спрайта в системе (§8, альманах III; п. 2, 12.09): корабли-спрайты
   выше потолка корабля (16c shipScaleCap, растёт с зумом) не растут — мир растёт быстрее. Флот на
   .85 от своего корабля, как и был. Спрайт печётся в ×3: разрешение оплачено */
function fleetScale(Z){return clamp(Z,.5,shipScaleCap(Z))*.85;}   /* потолок растёт с зумом у всех (P8, 18.09) */
...
/* ширина подписи (25.09): мера — раз на текст, дальше из кэша, кадр не зовёт 2D (перепись:
   ~22 вызова #c за кадр на два борта). Мерит ctx, с v2 — мера GPU-холста. Общая: 12ai, 13 */
const LBL_W=new Map();
function labelW(font,s){
  const k=font+"|"+s;let w=LBL_W.get(k);
  if(w==null){if(LBL_W.size>=256)LBL_W.clear();const f0=ctx.font;ctx.font=font;w=ctx.measureText(s).width;ctx.font=f0;LBL_W.set(k,w);}
  return w;
}
