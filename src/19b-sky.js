...
function drawSkyBodies(p,camx,camy){
  ...
  /* осадки и туман глушат небесное — тот же закон, что у клиньев света (19c):
     в ливень далёкого тела не видно, и его светящаяся кромка не должна
     переживать его тело — иначе в небе остаётся голый контур круга (П3) */
  const wp=(atm&&typeof weatherPower==="function")?weatherPower(p):0;
  const dim=(atm?.42:1)*(1-wp*.75);          // воздух гасит небо
  if(dim<.04)return;
...
function skyGiant(p,e,x,y,dim,ringy){
  ...
  /* ── СЕДЬМАЯ переделка: терминатор переживает пол (второй круг марафона) ──
     Пол «не темнее неба» днём поднимал обе стороны к воздуху, и свет с тенью
     схлопывались в тридцать единиц: диск оставался «отличным от воздуха», но
     ПЛОСКИМ — водяной знак, а не шар (крап автора «кругов дохуя» — это он).
     Тело узнаётся не отличием от неба, а СВОИМ терминатором: после всех
     полов день обязан быть светлее ночи хотя бы на сорок. */
  {
    /* порог считает ВНУТРЕННИЙ контраст: тело кладётся с альфой dim (≈.42 в
       воздухе), и сорока внутренних на экране оставалось семнадцать — мало.
       Семьдесят два внутренних ≈ тридцать на экране: шар, а не знак. */
