...
function bhudKey(b,fwd,st,bas){
  ...
  const hd=held();
  k.push(q(G.fuel/st.fuelMax,1024),q(G.hull/st.hullMax,1024),st.fuelMax,st.hullMax,st.cargoMax,hd);
  ...
  k.push(BHUD.pod);
  /* радар: точки в пределах дальности — место 1/16 px на шкале и прозрачность; размер и цвет
     у камня свои навсегда, их несёт его место в списке */
  let h=0;
  const fx=Math.sin(b.yaw),fz=Math.cos(b.yaw),rx=fz,rz=-fx,ast=b.ast;
  for(let i=0;i<ast.length;i++){
    const a=ast[i],dx=a.x-b.x,dz=a.z-b.z,dy=a.y-b.y;
    if(dx*dx+dy*dy+dz*dz>4e6)continue;
    h=(h*31+i)|0;h=(h*31+Math.round((dx*rx+dz*rz)*2.048))|0;h=(h*31+Math.round((dx*fx+dz*fz)*2.048))|0;
    h=(h*31+Math.round(dy*(dy<0?-.25:.25))+(a===L?7:0))|0;
  }
  k.push(h);
