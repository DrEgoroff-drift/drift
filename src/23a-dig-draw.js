...
function drawDigWorld(){
  ...
  /* Порог был `camy<40` — число, не имеющее отношения к тому, видно ли устье:
     стоило отойти от нуля, как небо переставало рисоваться, а порода сверху не
     начиналась, и верх кадра становился ЧЁРНОЙ ДЫРОЙ в пол-экрана (M233).
     Спрашиваем прямо: попадает ли линия поверхности в кадр (с запасом на
     разброс профиля) — если да, над ней небо. */
  if(digSurfY(p,camx+W*.5)-camy>-40){
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(-2,-2);
    ...
    /* ── над устьем стоит НЕБО, а не чернота ──
       У безвоздушного мира палитра неба почти чёрная (4,4,8) — и это верно, но
       на поверхности такую черноту держат звёзды, а в шахте их не рисовали:
       над кромкой висела ровная чёрная плашка в пол-кадра. Те же звёзды, что
       наверху; ночью на мире с воздухом — тоже. */
    const nite=(typeof surfNight==="function")?surfNight(p):0;
    if(p.T.atm==="отсутствует"||nite>.15){
      if(typeof drawStars==="function")drawStars(camx*.1,0,p.T.atm==="отсутствует"?1:nite);
    }
    ctx.restore();
