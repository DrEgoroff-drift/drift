...
fn field(p:vec2f,uv:vec2f)->vec4f{
  ...
  for(var i=0;i<4;i++){
    ...
    if(d.y>0.){
      ...
      let cn=(1.-smoothstep(.74,1.0,abs(d.x)/hw))*(1.1-.75*clamp(d.y/max(fy-35.,1.),0.,1.))*(1.-smoothstep(fy-2.,fy+8.,q.y));
      cone+=cn;
  ...
  /* виньетка: как у старой рубки, но по кривой, а не по прямой. Эллипс по залу, а не
     круг по высоте: круглая в широком зале клала крайние места в полную тень (×.38) —
     левый менеджер стоял в тёмном пятне и его табличка не читалась */
