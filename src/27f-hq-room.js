...
function hqTable(c,W2,H2,fy,seed){
  ...
  pg.addColorStop(0,"rgba(180,230,255,.50)");
  pg.addColorStop(.55,"rgba(85,160,220,.26)");
  pg.addColorStop(1,"rgba(40,100,160,.05)");
  c.fillStyle=pg;c.beginPath();c.arc(cx,py0,pr,0,TAU);c.fill();
  c.save();c.beginPath();c.arc(cx,py0,pr,0,TAU);c.clip();
  c.strokeStyle="rgba(190,235,255,.13)";c.lineWidth=.8;   // параллели внутри шара
  for(let i=-1;i<=1;i++){
    c.beginPath();c.ellipse(cx,py0+i*10,pr,pr*.26,0,0,TAU);c.stroke();
  }
  const mer=(G.t*.2)%1;                                   // меридианы медленно едут
  for(let i=0;i<4;i++){
    const t=((i/4+mer)%1)*2-1;
    c.beginPath();c.ellipse(cx,py0,Math.abs(t)*pr,pr,0,0,TAU);c.stroke();
  }
  c.fillStyle="rgba(0,10,20,.35)";                        // терминатор
  c.beginPath();c.arc(cx+pr*.55,py0+pr*.2,pr,0,TAU);c.fill();
  c.restore();
  c.strokeStyle="rgba(190,235,255,.28)";c.lineWidth=1;
  c.beginPath();c.arc(cx,py0,pr,0,TAU);c.stroke();
  /* одно кольцо, наклонённое: два и больше читались клубком */
  c.strokeStyle="rgba(190,235,255,.42)";c.lineWidth=1.2;
  c.save();c.translate(cx,py0+4);c.rotate(-.22);
  c.beginPath();c.ellipse(0,0,pr*1.6,pr*.34,0,0,TAU);c.stroke();
  c.restore();
  for(let i=0;i<26;i++){                                   // звёзды сектора
    const a=RR()*TAU,rr=.35+RR()*.65;
    const px=cx+Math.cos(a)*rx*.86*rr,py=ty-14-Math.abs(Math.sin(a))*26*rr;
    const tw=.5+.5*Math.sin(G.t*1.4+i);
    c.fillStyle="rgba(150,205,240,"+(.14+tw*.22).toFixed(2)+")";
    c.beginPath();c.arc(px,py,1+RR()*1.5,0,TAU);c.fill();
  }
