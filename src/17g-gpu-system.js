...
fn star(p:vec2f,sv:vec4f,cv:vec4f,t:f32,big:f32,px:f32,hz:f32)->vec4f{
  ...
  let breath=.93+.07*sin(t*.04);
  {
    ...
    let x=max(r-Rd,0.);
    e=e+col*(exp(-x*3.)*.5+exp(-x*.9)*.16*(.6+.8*n))*breath;
  ...
  if(r<Rd+.06){
    ...
    let gs=(5.+6.*(1.-big))/Rd;
    let ga=.7+.2*big;let gr=1.-ga*.5+ga*sf(d/R*gs+vec2f(t*.0015,-t*.001));
    ...
    gl=1.-.5*cov*mu;   /* засветка над серединой диска слабее — цвет поверхности не выцветает */
  }
...
fn field(p:vec2f,uv:vec2f)->vec4f{
  ...
  var acc=vec4f(0.);
  if(kind<.5){let hz=1.-.8*V[7].w;acc=over(acc,star(p,V[0],V[1],t,V[4].w,px,hz));acc=over(acc,star(p,V[2],V[3],t,V[4].w,px,hz));}
