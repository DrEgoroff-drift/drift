...
/* ══════════════ испечённое живёт не дольше, чем нужно (M332) ══════════════
   Кэш систем не имеет предела, и это правильно: система — дешёвый объект с
   числами, а по нему привязано состояние (рынок, аппетит, захват тела). Но на
   планетах этого объекта висит РАСТР: развёртка глобуса (до 512×256), две
   накладки света (256×256) и тринадцать спрайтов облаков. Измерено набором
   «память: вечер прыжков» — сорок систем дают 636 холстов и 28.9 МБ, восемьдесят
   ровно вдвое больше. Вечер — это не сорок систем, а несколько сотен: счёт
   идёт на сотни мегабайт, и вкладка встаёт. Именно так и выглядит «жёсткое
   зависание», которое ищут с M238: ни исключения, ни строки в консоли.
...
function sysRasterTick(){
  const key=G.sx+","+G.sy;
  if(key===SYS_HOME_KEY)return 0;
  SYS_HOME_KEY=key;
  const i=SYS_LRU.indexOf(key);
  if(i>=0)SYS_LRU.splice(i,1);
  SYS_LRU.push(key);
  if(SYS_LRU.length<=SYS_BAKE_KEEP)return 0;
  const keep=new Set(SYS_LRU.slice(-SYS_BAKE_KEEP));
  let n=0;
  for(const [k,s] of SYS_CACHE)if(!keep.has(k))n+=sysRasterDrop(s);
  return n;
}
