...
function renderRoute(){
  ...
  if(R.legs.length<2){
    ...
    $body.appendChild(el("div","row","<div class='nm'><b>Маршрута нет</b><s>"+
      "на карте выберите станцию и нажмите «В МАРШРУТ» — 2–4 плеча.<br>"+
      "Игра посчитает по живым ценам, что и куда везти"+
      (R.legs.length?"<br>сейчас отмечено плеч: "+R.legs.length:"")+"</s></div>"));
    return;
  }
  const S=routeSum();
