...
function enterDig(){
  ...
  mineLoad(G.dig,S.p);
  G.mode="dig";
  /* «Шахта» стоит в сводке места, спуск игрок только что видел глазами, а
     «W A S D — копать» и без того постоянно висит в подсказке у пэдов —
...
function exitDig(){
  /* кнопка ВЫХОД живёт в режиме, а режим и шахта — две разные вещи: кадр и
     апдейт давно проверяют `G.mode==="dig"&&G.dig`, а выход не проверял ничего
     и умирал на `G.dig.p`. Смерть в обработчике нажатия не видно нигде: она
     уходит в window.onerror, игрок остаётся в шахте навсегда (M329). */
  if(!G.dig){G.mode="surface";return;}
