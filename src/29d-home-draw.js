...
function hinRoomStuff(r,fy,ceil,P){
  ...
  if(r.key==="corner"){
    ...
    ctx.beginPath();
    ctx.moveTo(at(.30)-M*1.5,fy);ctx.lineTo(at(.30)-M*1.2,fy-M*.86);
    ctx.lineTo(at(.30)+M*1.1,fy-M*.94);ctx.lineTo(at(.30)+M*1.5,fy);
    ctx.closePath();ctx.fill();
    ctx.fillStyle="rgba(255,255,255,.08)";
    ctx.beginPath();
    ctx.moveTo(at(.30)-M*1.2,fy-M*.86);ctx.lineTo(at(.30)+M*1.1,fy-M*.94);
    ctx.lineTo(at(.30)+M*.2,fy-M*.6);ctx.closePath();ctx.fill();
    ctx.strokeStyle="rgba(0,0,0,.3)";ctx.lineWidth=1;
    for(let i=0;i<5;i++){
