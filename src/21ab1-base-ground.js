...
/* гора: горб над базой, склон к равнине, плато вправо. Раньше высота считалась
   от камеры, и форма горы менялась, когда камера ехала вниз; потом — «до верха
   кадра при самой высокой камере», и на 760×475 со второго яруса (там строят)
   вершина уходила за кадр и съедала полосу неба с грядой (Контроль 26.09).
   Теперь вершина — в 22 px под верхом кадра при камере второго яруса, как у
   main в этом кадре: от размера кадра, не от камеры (выпечки ключуются W×H) */
function baseHump(wx){
  const bMidX=BASE_OX+BASE_COLS*BCELL_W*.5, bHalf=BCELL_W*BASE_COLS*.62;
  const mtnX0=BASE_GATE_X-95, mtnTop=Math.max(40,BASE_GY-Math.max(-120,cellY(1)-H/2)-22);
