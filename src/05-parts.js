...
function tierFromDanger(d,r){
  ...
  /* жила в поясе (M382, §15.1): трое суток находки здесь на тир крупнее — и
     туда летят все, включая пиратов. Держится это на летописи, а не на записи
     в мире: у всех одинаково и ничего не хранится */
  const vein=(typeof econTierBonus==="function")?econTierBonus():0;
  return clamp(Math.round(base)+vein,1,5);
}
