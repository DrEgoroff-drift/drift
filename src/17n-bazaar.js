...
function bazHulk(id,dk){
  ...
  const B=gpuBaked(BAZ_BAKE,key,side,side,g=>{g.setTransform(sb,0,0,sb,side/2,side/2);
    ...
    tracePoly(h.poly);g.fillStyle="rgba(20,18,16,.6)";g.fill();});
...
function drawBazaar(zx,zy,Z){
  ...
  domLabel("bz"+B.seed,x,y-48*s,"БАРАХОЛКА","bold "+Math.round(8*s)+"px ui-monospace,monospace","rgba(240,210,160,.9)","center");   /* .9: в 2D её поднимал проход сцены */
}
