...
function crewDamage(c,amount){
  ...
  if(c.hull<=0){
    ...
    logAdd("warn",c.name+" потерял «"+(S?S.ru:lost)+"» — корабль не вернулся");
...
function crewHaul(c,min,eff,danger,r){
  ...
  const pay=Math.round(min*(S?S.cargo:20)*.55*eff);
  if(pay>0){
    G.credits+=pay;
