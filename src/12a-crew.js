...
function crewTick(){
  ...
  for(const c of G.crew){
    ...
    if(!c.order||c.order.kind==="home"||(needsShip&&!c.shipId)||c.hull<=0){
      /* ── праздный человек виден ──
         Наёмник без корпуса или без приказа не работает вовсе — и молчал об
         этом вечно. Автор нанял такого, улетел и через много часов сказал:
         «наёмники никуда не летали, непонятно» (30.08.2026). И правда: найм
         списался, человек появился в списке — и на этом игра замолчала
         навсегда. Через несколько минут простоя он напоминает о себе ОДИН
         раз и снова замолкает; отметка снимается, как только он поехал, —
         значит, второй раз она придёт только после второго простоя. */
      c.idleMs=(c.idleMs||0)+dtMs;
      if(c.idleMs>5*60000&&!c.idleSaid){
        c.idleSaid=1;
        logAdd("warn",c.name+" сидит без дела: "+
          (!c.shipId?"ему не выдан корабль":(c.hull<=0?"его корпус разбит":"у него нет приказа"))+
          " · МЕНЮ → ЭКИПАЖ");
      }
...
function drawAllies(zx,zy,Z){
  ...
  for(const A of G.allies){
    ...
    /* корпус на видеокарте, как свой (17c2): тело светом звезды, факел шейдером, огни
       ...
       копии #c и двух отправок. 2D-пути нет (25.09) */
    allyHullGpu(A,x,y,Z,zx(0),zy(0));
