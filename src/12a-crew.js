...
function stationMercs(sys){
     ...
     нехорошо, — залётный без истории. Ставки и содержание рейсов не трогаем. */
  {
    ...
    if(rv>=2)for(let i=0;i<Math.min(out.length,rv>=4?2:1);i++)
      out[i].xp=Math.max(out[i].xp,40+(rv>=4?60:25));
    else if(rv<=-2)for(const m of out)m.xp=Math.floor(m.xp*(rv<=-4?.25:.6));
  }
