...
function hotelBake(sd,col){
  const ck=col.join();
  if(HOTEL_BAKE&&HOTEL_BAKE.sd===sd&&HOTEL_BAKE.ck===ck)return HOTEL_BAKE;
  ...
  const N=(HR_COLS+HR_SIDE)*HR_ROWS,A=hotelPaint(sd,"0".repeat(N),col),Z=hotelPaint(sd,"1".repeat(N),col);
  const ph=A.cv.height,L={};
  for(const k of ["cv","em","sh"]){const c=document.createElement("canvas");c.width=A.cv.width;c.height=ph*2+HR_GAP;
    const g=c.getContext("2d");g.drawImage(A[k],0,0);g.drawImage(Z[k],0,ph+HR_GAP);L[k]=c;}
  ...
  HOTEL_BAKE={sd,ck,cv:L.cv,em:L.em,sh:L.sh,ph,win,shY,ver:((HOTEL_BAKE&&HOTEL_BAKE.ver)|0)+1};return HOTEL_BAKE;
}
/* дом по маске окон — в три холста HR_BW×HR_BH (·HR_PX) */
function hotelPaint(sd,mask,col){
  ...
  e.shadowColor="rgba(255,176,96,.7)";e.shadowBlur=1.2;   /* в пикселях холста, мимо трансформа: ~десятая окна */
...
function drawHotel(zx,zy,Z){
  ...
  const B=hotelBake(sd,col),mask=hotelWinLit(sd,hotelLitFrac(Ht.by,hr),Math.floor(G.t/60/6));
  ...
  const pass=gpuScene();
  const cw=B.cv.width,ph=B.ph,lit=[];
  ...
  if(pass){
    ...
    const th=B.cv.height,sx=w/cw,sy=h/ph,L=gpuMipTex;
    ...
    /* 2D (без видеокарты): дом по маске собирается из атласа в свой холст, когда маска сменилась */
    let C=B.c2d;
    if(!C||C.mask!==mask){
      C=B.c2d=B.c2d||{};C.mask=mask;
      for(const k of ["cv","em","sh"]){
        const c=C[k]||(C[k]=document.createElement("canvas"));c.width=cw;c.height=ph;const g=c.getContext("2d");
        g.drawImage(B[k],0,0,cw,ph,0,0,cw,ph);
        for(const q of lit){g.clearRect(q[0],q[1],q[2]-q[0],q[3]-q[1]);g.drawImage(B[k],q[0],q[1]+ph+HR_GAP,q[2]-q[0],q[3]-q[1],q[0],q[1],q[2]-q[0],q[3]-q[1]);}
      }
    }
    ctx.drawImage(C.cv,ox,oy,w,h);
    ctx.save();ctx.globalCompositeOperation="lighter";ctx.drawImage(C.em,ox,oy,w,h);
    if(S){ctx.globalAlpha=HOTEL_SHEEN*fa;ctx.drawImage(C.sh,ox,oy,w,h);}
    ctx.restore();
  }
