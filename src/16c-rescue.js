...
function haulTick(dt,sh){
  ...
  /* трос в мировых единицах — от зума НАРОЧНО: корабль и баржа рисуются не в
     масштабе мира (shipScaleAt держит пол .35), и мировой трос постоянной длины
     на дальнем отъезде (.16) выходил короче полубаржи — корма ложилась на
     корабль. Трос держит постоянную длину на экране; при щипке баржа плавно
     едет вместе с масштабом, скачка нет — позиция пересчитывается каждый кадр */
  const reachW=haulReach()*shipScaleAt(G.zoom)/Math.max(.05,G.zoom);
