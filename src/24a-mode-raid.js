...
function genRaid(seed,level){
  ...
  return {seed,level,cells,kind,rooms};
}
...
function enterRaid(PB){
  ...
  for(const rm of R.rooms){
    ...
    for(let i=0;i<n;i++){
      ...
      const boss=rm.k==="bridge"&&i===0;
      foes.push({x,z,a:r()*TAU,hp:(boss?70:26)+PB.level*14,hpMax:(boss?70:26)+PB.level*14,
        boss,cool:0,aware:false,seed:hashi(PB.seed,foes.length*131,9),bob:r()*TAU});
    }
  ...
  G.raid={PB,R,foes,loot,x:sx0,z:sz0,a:0,vx:0,vz:0,suit:100,cool:0,bag:{parts:[],tech:0,data:0},
...
function updateRaid(dt){
  ...
  if(keys.fire&&S.cool<=0&&st.armed){
    S.cool=Math.max(7,st.cool*.5);S.flash=6;
  ...
  for(const f of S.foes){
    ...
    const see=d<720&&raidLineOfSight(R,f.x,f.z,S.x,S.z);
    if(see)f.aware=true;
    if(f.aware){
      f.a=Math.atan2(dx,dz);
      if(d>190&&see){
        const nfx=f.x+dx/d*1.5*dt, nfz=f.z+dz/d*1.5*dt;
        if(!raidSolidAt(R,nfx,f.z))f.x=nfx;
        if(!raidSolidAt(R,f.x,nfz))f.z=nfz;
      }
      if(f.cool>0)f.cool-=dt;
      if(see&&d<700&&f.cool<=0){
        f.cool=f.boss?26:46;
        S.shots.push({x:f.x,z:f.z,vx:dx/d*7,vz:dz/d*7,life:70,dmg:(f.boss?9:5)+S.level*2,mine:false});
        sfx("shot",{f:280,v:.28});
      }
    }
    f.bob+=.05*dt;
    ...
    if(sh.mine){
      for(const f of S.foes){
        ...
        if(Math.hypot(f.x-sh.x,f.z-sh.z)<34){
          ...
          if(f.hp<=0){
            logAdd("kill","На абордаже: "+(f.boss?"главарь":"пират")+" уничтожен");
      ...
      S.suit-=sh.dmg*st.suitWear;S.hurt=10;sfx("hit",{v:.45});
  ...
  if(near){
    ...
    G.prompt="СКАФАНДР "+Math.round(S.suit)+"% · ЖИВЫХ "+alive+
...
function drawRaid(){
  ...
  const cam=[S.x-Math.sin(S.a)*back, 78, S.z-Math.cos(S.a)*back];
  ...
  for(let rr=r0-rad;rr<=r0+rad;rr++)for(let c=c0-rad;c<=c0+rad;c++){
    ...
    /* затухание с расстоянием заменяет освещение: дальше — темнее */
    const dd=Math.hypot(x0+RCELL/2-S.x,z0+RCELL/2-S.z);
    const li=clamp(1.15-dd/(RCELL*9),.12,1);
    quad([x0,0,z0],[x1,0,z0],[x1,0,z1],[x0,0,z1],base,li*.9,false);
    ...
    if(raidSolid(R,c,rr-1))quad([x0,0,z0],[x0,RAID_H,z0],[x1,RAID_H,z0],[x1,0,z0],base,li,true);
    if(raidSolid(R,c,rr+1))quad([x1,0,z1],[x1,RAID_H,z1],[x0,RAID_H,z1],[x0,0,z1],base,li,true);
    if(raidSolid(R,c-1,rr))quad([x0,0,z1],[x0,RAID_H,z1],[x0,RAID_H,z0],[x0,0,z0],base,li*.92,true);
    if(raidSolid(R,c+1,rr))quad([x1,0,z0],[x1,RAID_H,z0],[x1,RAID_H,z1],[x1,0,z1],base,li*.92,true);
  }
  ...
  for(const f of S.foes){
    if(f.hp<=0)continue;
    const p=proj(f.x,40+Math.sin(f.bob)*3,f.z);if(!p)continue;
    marks.push({p,kind:"foe",o:f});
  }
  const me=proj(S.x,42,S.z);
  ...
  for(const m of marks){
    ...
    if(m.kind==="loot"){
      ...
      ctx.fillStyle=f.boss?"#ff6b57":"#c4694f";
      ctx.beginPath();ctx.ellipse(0,0,9,15,0,0,TAU);ctx.fill();
  ...
  for(const sh of S.shots){
    const p=proj(sh.x,40,sh.z),q=proj(sh.x-sh.vx*2,40,sh.z-sh.vz*2);
    if(!p||!q)continue;
  ...
  ctx.fillText("СКАФАНДР "+Math.round(S.suit)+"%",bx,by+18);
}
