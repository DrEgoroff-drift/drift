...
function drawStrata(tr,camx,camy,p,P){
  ...
  for(let k=0;k<G0.length;k++){
    ...
    LP.closePath();
    /* ── тело пласта — СВЕТЛОТОЙ (гризайль P4, M422) ──
       Слой отличался от слоя оттенком, и на мирах с тесной палитрой этой
       разницы не хватало: разрез читался заливкой — про это и написан абзац
       про контакт ниже. В гризайли слои различаются ТОНОМ, который виден на
       любой палитре, а цвет им всем разом даёт лессировка от неба и звезды.
       Собственный оттенок остаётся у жил: там он порода, а не украшение. */
    if(glazeIsForm()){
      /* ── слои РАЗВОДЯТСЯ ПО ТОНУ, а не по оттенку ──
         Раньше пласт от пласта отличался цветом, и разница читалась даже
         когда светлоты совпадали. В гризайли этого не осталось: два слоя с
         близкой светлотой сливаются в одно тело, и прибор кадра это увидел —
         «кромок» 7 → 3. Настоящий разрез так и устроен: светлая пачка, тёмная
         пачка, светлая. Разводим сильнее обычного (`STRATA_K`) и добавляем
         шаг от семени слоя, чтобы соседние не совпадали случайно. */
      const PT=p.T.pal[p.T.pal.length-1];
      const v0=lum3(L.col)*255/Math.max(1,lum3(PT));
      const step=((L.seed>>>5)&1)?1:-1;
      ctx.fillStyle=greyA(clamp(128+(v0-128)*STRATA_K+step*STRATA_STEP,40,225),1);
      ctx.fill(LP);
    }
    /* ── контакт пластов ──
    ...
    if(glazeIsForm()){
      ...
      /* светлая кромка контакта была ТЁПЛОЙ КОНСТАНТОЙ — теперь она чистая
         светлота, а тепло ей даёт звезда вместе со всем разрезом */
      ctx.strokeStyle="rgba(255,255,255,.16)";ctx.lineWidth=1.1;
    ...
    /* жилы своим минералом — ПОСЛЕ лессировки: их оттенок и есть порода */
    if(L.vein>.05&&glazeIsHue()){
