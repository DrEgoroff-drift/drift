...
/* ── насколько далёкий слой отстаёт от листа (M438) ──
   `d` — путь листа в пикселях, `k` — доля этого пути, положенная слою (чем
   слой дальше, тем меньше), `M` — предел, за который небу не уехать. tanh
   держит малое движение честным (на первых пикселях выходит ровно k·d) и
   мягко упирает большое: полоса Галактики не должна уползать с листа. */
function mapSkyShift(d,k,M){return M*Math.tanh(d*k/M);}
function drawMap(){
  const st=stat();
  ...
  /* ══ небо за листом (M438) ══
     Фон был прибит к экрану: тащишь карту — лист едет, а полоса Галактики,
     туманность и звёздная крошка стоят колом. Лист и небо читались одной
     плоскостью, и получалось «схема ползёт по обоям».
     Закон: НЕБО СТОИТ В МИРЕ, ЛИСТ СКОЛЬЗИТ ПЕРЕД НИМ. Небо привязано к
     кораблю — где вы в галактике, таким и виден рукав, — а протяжка карты
     сдвигает его лишь на долю пути листа, тем меньшую, чем слой дальше:
     крошка ~.4 листа, туманность ~.14, полоса ~.05. Сдвиг мягко упирается
     в предел (tanh): небо бесконечно далеко, уехать ему некуда. */
  const dvx=(vx-G.sx)*cell, dvy=(vy-G.sy)*cell;  /* насколько лист уехал от вас, в пикселях экрана */
  /* запас поля под сдвиг — по каждой оси свой и в одной доле кадра: иначе
     слой растягивается неравномерно и полоса Галактики полнеет на глазах */
  const SKX=W*.08, SKY=H*.08, sky=(v,k,M)=>mapSkyShift(v,k,M);
  const N=mapNebula(),ex=W*.35,ey=H*.35;
  ctx.globalAlpha=.55;
  ctx.drawImage(N,-ex/2-((G.sx*11)%ex)-sky(dvx,.14,SKX),-ey/2-((G.sy*11)%ey)-sky(dvy,.14,SKY),W+ex,H+ey);
  ctx.globalAlpha=1;
  /* ── полоса Галактики (леджер кадров: карта mass 0%, contrast .07) ──
     ...
     на размер экрана, кадру — один drawImage: слой берётся с запасом по кромке
     (лёгкая ровная растяжка градиента незаметна) и ездит внутри этого запаса.
     Яркость приглушена: полоса — то, ЗА чем лежат адреса, а не подсветка;
     на полной силе она спорила с сеткой и подписями. */
  ctx.globalAlpha=.62;
  ctx.drawImage(screenLayer("mapband|"+W+"|"+H,()=>mapBandPaint(ctx,W,H)),
    -sky(dvx,.05,SKX)-SKX,-sky(dvy,.05,SKY)-SKY,W+SKX*2,H+SKY*2);
  ctx.globalAlpha=1;
  /* ── сеть пеленгов (экспедиция-2, свод §14: портуланы) ──
     На морской карте каждая линия — пеленг, по которому можно идти; красота
     заработана управляемостью. Шестнадцать румбов из ТЕКУЩЕЙ системы — курс
     читается направлением с одного взгляда, главные четыре чуть громче.
     Румбы принадлежат ЛИСТУ, а не экрану: узел стоит на вашей системе и едет
     с картой 1:1 — по нему и видно, что лист скользит перед небом. Шестнадцать
     линий в кадре дешевле слоя, который всё равно пришлось бы двигать. */
  mapRhumbPaint(ctx,W,H,px,py);
  drawStars(G.sx*140+dvx*3048/W, G.sy*140+dvy*2286/H, .35);
...
function updateMap(dt){
  ...
  if(actEdge){
    ...
    /* выбран свой же сектор: подсказка обещает ПРЫЖОК, и молчать в ответ нельзя —
       у отказа есть голос (нашёл набор подсказки, когда карта стала шагать
       в updateMap, 0.441.0) */
    else say("Вы уже здесь — выберите другой сектор");
  }
}
function jump(cost){
  ...
  /* ── точка прибытия — у системы своя (P9) ──
     Был rnd()*TAU: в одну систему прилетаешь всякий раз с новой стороны, и у
     места нет «входа». Теперь угол от зерна системы — вход один и тот же, на
     нём строит свою полосу жизнь Ж1 (этап 2). Заодно из потока rnd() ушёл один
     вызов: сдвинулись все реплеи и наборы с одинаковым хешем — перед
     релизом сознательный test.ps1 -Accept, строкой в патчноуте. */
  const E=sysEntry(G.sx,G.sy),a=E.a;   /* вход системы — начало полосы Ж1 (17g) */
  ...
  /* прибытие без хода (боты 12.09): с .7 прямо на звезду брошенный корабль за
     ...
     накат, и тормозить было некому. Нос — к станции: прилетел, и видно, куда
     идти; станции нет — к звезде, как было */
  const st0=G.sys&&G.sys.station;
