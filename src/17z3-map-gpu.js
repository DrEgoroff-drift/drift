...
fn field(p:vec2f,uv:vec2f)->vec4f{
  ...
  /* ядро светится, а не упирается в потолок: широкий тёплый ореол балджа за пределом модели.
     Втрое тише, чем был, и краснее — иначе под кругом прыжка молочное пятно (Контроль 26.09) */
  c+=vec3f(1.,.68,.56)*(.016*exp(-r/5.)+.0175*exp(-sq(r/2.2)))*(1.-dust*.8);
  /* гребень перемычки: вытянутое тёплое тело через ядро — у ядра есть ось */
  c+=vec3f(1.,.72,.58)*.03*exp(-(sq(u/(BL*.95))+sq(v/.75)))*(1.-dust);
  ...
  {let lv=log2(7./V.z);let s0=exp2(floor(lv));let t=fract(lv);
    /* реже и тише, чем была: у ядра крошка сливалась в белёсую вуаль, искр не больше, чем в main */
    let dn=clamp(pow(glow,1.25)*1.6+.02,0.,.5)*(1.-dust*.9);
    let ms=mix(gMs(w0,s0,dn,V.z,${0x6D1}),gMs(w0,s0*2.,dn,V.z,${0x6D5}),t);
    c+=mix(col,vec3f(1.),.18)*ms*.15;}
...
function mapStarsGpu(pass,vis,gk){
  ...
  for(const v of vis){
    ...
    /* лестница блеска: класс звезды решает и ореол, и лучи; свечение — сложением в три масштаба.
       Карта — прибор (Контроль 26.09): ореолы и лучи ужаты вдвое, ядро галактики было
       молочным пятном (p99 182 против 123 у main) — метки и кольца читаются поверх неба */
    const br=clamp(t/2,0,1);
    SH.push([1,x,y,0,0,0,rr*(3.5+2.5*br),col[0],col[1],col[2],(.035+.035*br)*f],
      [1,x,y,0,0,0,rr*2,col[0],col[1],col[2],(.28+.14*br)*f]);
    if(spk.has(v)){
      SH.push([1,x,y,0,0,0,rr*8,col[0],col[1],col[2],.04*f]);
      /* лучи сужаются к концам: длинная тусклая капсула, короткая яркая, белая середина */
      const L=rr*(2.8+1.2*t);
      for(const [dx,dy] of [[1,0],[0,1]]){
        SH.push([2,x-dx*L,y-dy*L,x+dx*L,y+dy*L,.4,.8,col[0],col[1],col[2],.16*f],
          [2,x-dx*L*.5,y-dy*L*.5,x+dx*L*.5,y+dy*L*.5,.5,.8,col[0],col[1],col[2],.22*f],
          [2,x-dx*L*.2,y-dy*L*.2,x+dx*L*.2,y+dy*L*.2,.6,.6,255,255,255,.24*f]);
      }
    }
