...
/* черновики записи в буферы — растут до самого длинного вызова и живут дальше: writeBuffer копирует
   данные сразу, так что один массив годится на все вызовы кадра (ревью 25.09 №7: массив на вызов) */
...
@group(1) @binding(0) var itx:texture_2d<f32>;
@group(1) @binding(1) var ism:sampler;
...
@fragment fn fs(i:IO)->@location(0) vec4f{
  ...
  return c*i.k.x;}`;
/* раскладка картинки — своя, одна на устройство (живёт в GPU.lay): группа 0 — форма и арена, общая
   на кадр; группа 1 — текстура и сэмплер. Раскладка явная, поэтому группы годятся обоим смешениям,
   и группа текстуры кэшируется по виду (WeakMap): вид и есть текстура, умерла — ушла и группа.
   Раньше одна группа на всё и один слот на смешение — смена текстуры пересобирала её (~8 за кадр в системе) */
function gpuImgLayout(){
  const c=GPU.lay["kit.imgL"];if(c)return c.PL;
  const d=GPU.dev,S=GPUShaderStage,VF=S.VERTEX|S.FRAGMENT;
  const L0=d.createBindGroupLayout({entries:[{binding:0,visibility:VF,buffer:{type:"uniform"}},{binding:1,visibility:S.VERTEX,buffer:{type:"read-only-storage"}}]});
  const L1=d.createBindGroupLayout({entries:[{binding:0,visibility:S.FRAGMENT,texture:{sampleType:"float"}},{binding:1,visibility:S.FRAGMENT,sampler:{type:"filtering"}}]});
  const PL=d.createPipelineLayout({bindGroupLayouts:[L0,L1]});
  GPU.lay["kit.imgL"]={L0,L1,PL,g0:null,u:null,a:null,g1:new WeakMap()};return PL;
}
function gpuImgBind(pass,A,view,smp){
  const c=GPU.lay["kit.imgL"],u=gpuKitU(),d=GPU.dev;
  if(c.u!==u||c.a!==A.buf){c.u=u;c.a=A.buf;c.g0=d.createBindGroup({layout:c.L0,entries:[{binding:0,resource:{buffer:u}},{binding:1,resource:{buffer:A.buf}}]});}
  ...
  pass.setBindGroup(0,c.g0);pass.setBindGroup(1,g);
}
function gpuImage(pass,cv,rects,o){
  if(!pass||!rects.length)return;
  const blend=(o&&o.blend)||"over",P=gpuPipe("kit.img",GPU_IMG_WGSL,blend,gpuImgLayout());
  const n=rects.length,A=gpuArena("img",n*12,12),f=gpuScr("img",n*12),mip=!!cv.view,gs=mip?(o&&o.sharp?-GPU_MIP_GS:(o&&o.lod)||GPU_MIP_LOD):0;   /* <0 — с маской; o.lod — свой масштаб уровня */
  ...
  GPU.dev.queue.writeBuffer(A.buf,A.off*4,f,0,n*12);
  ...
  pass.setPipeline(P);
  gpuImgBind(pass,A,t.view,mip?gpuMipSmp():GPU.S.lin);
...
function gpuShapes(pass,items,o){
  ...
  const n=items.length,A=gpuArena("shp",n*12,12),f=gpuScr("shp",n*12);
