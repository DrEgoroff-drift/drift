...
try{
  ...
  TAB_CH.onmessage=e=>{
    ...
    if(d.k==="hello"&&TAB_LIVE){
      ...
      logShip("Игра открыта в другой вкладке — эта больше не сохраняется.");
    }
  };
...
function cloudCall(a,body){
  return fetch(CLOUD.api+"?a="+a,{method:"POST",
    ...
    body:JSON.stringify(netBody(body))}).then(r=>r.json()).catch(e=>{try{crashShip("net","облако "+a+": "+(e&&e.message||e),"");}catch(_){}throw e;});
}
...
function cloudMark(k){
  ...
  CLOUD_ST.said=k;
  if(k==="gone"){
    logShip("Облако: вход устарел — войдите заново на вкладке КОРАБЛЬ");
    say("ОБЛАКО\nвход устарел\nвойдите заново",220);
  }else if(k==="conflict"){
    logShip("Облако: там запись новее этой. Заберите её или выгрузите свою — вкладка КОРАБЛЬ");
    say("ОБЛАКО\nтам запись новее\nвкладка КОРАБЛЬ",240);
  }else if(k==="big"){
    logShip("Облако: запись слишком велика, она больше не отправляется");
  }
}
