...
/* ── одна строка из ста написана руками ──
   Девяносто девять кусков собраны из трёх банок, и в этом их сила: судовой
   журнал одинаково сух, и жуть берётся из сухости.

   Но табло вернувшихся (`11s-returners`) несёт среди просроченных строк
   позывной шлюпки — и эта строка не значит РОВНО НИЧЕГО, если игрок нигде
   позывного не встречал. Поэтому один кусок из ста не собран, а написан: он
   называет шлюпку по имени, и называет буднично, в ряду прочих записей.

   Дальше игра не делает ничего. Не подсвечивает, не связывает, не поздравляет
   за понимание. Кто прочтёт обе строки и сложит — сложит сам; кто нет — увидит
   ещё одну просроченную строку на табло, которое никто не чистит. */
const LORE_SHUTTLE="Тесло";
(function loreShuttle(){
  const own=LORE_BY_CHAP["tihonya"];
  if(!own||!own.length)return;
  const rec=own[Math.floor(own.length/2)];
  rec.ru="младший борт ушёл на тихой тяге, шлюпка «"+LORE_SHUTTLE+"», и это последняя запись";
  rec.give="addr";
  rec.word=null;
})();
/* ── собранное ── */
