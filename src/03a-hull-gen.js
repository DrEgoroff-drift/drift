function hullOf(id){
  ...
  /* hl/hw — ползунки СТАПЕЛЯ (M481); у всех прочих корпусов их нет, числа прежние */
  let nose=(16+r()*14)*K.len*(MB?MB.len:1)*(S.hl||1), tail=-(13+r()*13)*K.len*(MB?MB.len:1)*(S.hl||1), len=nose-tail;
  let bw=(3.8+r()*4.4)*K.bw*(MB?MB.bw:1)*(S.hw||1);
