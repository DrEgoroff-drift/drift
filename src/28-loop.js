...
function loopReset(){last=wallMs();lastDroneTick=0;capPrev=0;capN=0;quantAcc=0;
...
/* ══════════════ авторазрешение ══════════════
   ...
   ── и возвращается (M437) ──
   Прежде вверх она не шла никогда: «дрожание чётко — мыльно — чётко хуже
   ровной картинки». Цена этого правила оказалась выше самого дрожания: три
   тяжёлые секунды ОДИН раз — заход на планету, первая печь ломтей, чужая
   вкладка, укравшая кадр, — и весь остальной вечер игрок смотрит на мыло,
   включая карту, которая стоит копейки (автор, 09.09: «шрифт как-то
   размывает»). Дрожание лечится не запретом, а терпением: вниз — три
   секунды тяжёлого кадра, вверх — двадцать секунд подряд ЛЁГКОГО (≤13 мс,
   вдвое быстрее порога спуска), и не больше двух возвратов за сеанс. Между
   двумя порогами лежит вся обычная жизнь кадра: попадать в них по очереди
   случайно нечему, а это и есть дрожание. */
...
function resAuto(d){
  ...
  if(RES_AUTO>1){
    ...
    if(resBad>3000){
      resBad=0;resGood=0;resEma=16;
      RES_AUTO=RES_AUTO>1.5?1.5:1;
      if(DPR>RES_AUTO){resize();say("Разрешение снижено до ×"+RES_AUTO+"\nвернуть — в настройках, «Графика»");}
      return;
    }
  }
  /* и обратно, если кадр давно и уверенно лёгкий */
  if(RES_AUTO<2&&resUps<2){
    if(resEma<13)resGood+=d;else resGood=Math.max(0,resGood-d*2);
    if(resGood>20000){
      resGood=0;resBad=0;resEma=16;resUps++;
      RES_AUTO=RES_AUTO<1.5?1.5:2;
      if(Math.min(RES_AUTO,window.devicePixelRatio||1)>DPR){resize();say("Разрешение вернулось на ×"+RES_AUTO);}
    }
  }
}
