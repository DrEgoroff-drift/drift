...
function chipDomSnap(g,sc){
  /* подписи мира — первыми: на экране они под фишками */
...
/* свой слой — сразу ПЕРЕД #chips: подпись мира лежит под интерфейсом, фишки и плитки выше неё
   (в одном слое с фишками подпись, заведённая позже, ложилась поверх и склеивалась с ними) */
function labDomBox(){
  if(LABDOM.box||typeof document==="undefined"||!document.body)return LABDOM.box;
  const c=chipDomBox();if(!c)return null;
  const b=document.createElement("div");b.id="labels";
  b.style.cssText="position:fixed;inset:0;pointer-events:none;overflow:hidden";
  c.before(b);return LABDOM.box=b;
}
