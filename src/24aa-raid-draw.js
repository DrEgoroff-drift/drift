...
function drawRaid(){
  ...
  /* ── лут — ЯЩИК в мире, а не значок на экране (M180) ──
     ...
     стоит в проекции. Теперь это та же коробка box(), что и груз в ангаре:
     стоит на полу, ловит свет своей клетки, даёт контактную тень. Маячок
     остаётся — но крошечный и НА ящике, а не вместо него. */
  for(const L of S.loot){
    if(L.taken)continue;
    const fl0=raidFloorAt(R,L.x,L.z);
    const dd=Math.hypot(L.x-S.x,L.z-S.z);
    const li=clamp(.95-dd/(RCELL*9)+clamp(1-dd/620,0,1)*.4,.15,1.1);
  ...
  for(const m of marks){
    ...
    if(m.kind==="loot"){
      ...
      ctx.beginPath();ctx.arc(m.p.x,m.p.y,2.2*s,0,TAU);ctx.fill();
      if(on){
        const gg=ctx.createRadialGradient(m.p.x,m.p.y,0,m.p.x,m.p.y,9*s);
        gg.addColorStop(0,"rgba(127,230,216,.30)");gg.addColorStop(1,"rgba(127,230,216,0)");
        ctx.fillStyle=gg;ctx.beginPath();ctx.arc(m.p.x,m.p.y,9*s,0,TAU);ctx.fill();
      }
    }else if(m.kind==="pick"){
      ...
      ctx.save();ctx.translate(m.p.x,m.p.y);ctx.scale(s*.55,s*.55);
      ...
      /* страховка по вертикали: тень рисуется только НИЖЕ тела на экране.
         Враг, заспаунившийся с полом выше собственной головы (мезонин над
         залом), рисовал её на потолке */
      const pf=proj(f.x,raidFloorAt(R,f.x,f.z)+1,f.z);
      if(pf&&pf.y>m.p.y+4*s){
        ctx.fillStyle="rgba(0,0,0,.42)";
        ctx.beginPath();ctx.ellipse(pf.x,pf.y,15*s,4.2*s,0,0,TAU);ctx.fill();
      }
      ...
      if(pm&&pm.y>m.p.y+4*s){
        ctx.fillStyle="rgba(0,0,0,.45)";
        ctx.beginPath();ctx.ellipse(pm.x,pm.y,13*s,3.8*s,0,0,TAU);ctx.fill();
      }
