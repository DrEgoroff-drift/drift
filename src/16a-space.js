...
function drawStarSingle(ox,oy,R,col,heat){
  ...
  /* ядро — тоже в спрайт (Контроль, печь звезду, 18.09): рисунок гардиента
     постоянен, дышит только альфа (±.07 вокруг .82) — печём на полной альфе
     и дышим готовой картинкой через globalAlpha, а не пересчитываем градиент
     каждый кадр. Средний слой (.5, альфа .42) от этого тоже чуть дышит вместе
     с ядром вместо своей неизменной альфы — при амплитуде дыхания в восьмую
     долю разницы глаз её не ловит, а разница в кадре была первой по площади
     после туманности. */
  const CORE=glowSprite("core|"+col,()=>{
    const core=ctx.createRadialGradient(0,0,0,0,0,.85);
    core.addColorStop(0,"rgba(255,253,247,1)");
    core.addColorStop(.5,"rgba("+[0,1,2].map(i=>Math.round(lerp(c[i],255,.6))).join(",")+",.42)");
    core.addColorStop(1,"rgba("+c.join(",")+",0)");
    ctx.fillStyle=core;ctx.beginPath();ctx.arc(0,0,.85,0,TAU);ctx.fill();
  });
  ctx.globalAlpha=.82+.07*Math.sin(G.t*.04);
  ctx.drawImage(CORE,ox-R*.85,oy-R*.85,R*1.7,R*1.7);
  ctx.globalAlpha=1;
