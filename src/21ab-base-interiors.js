...
reactor(x0,y0,w,h,cx,fy,lit,seed,B,P){
  ...
  ctx.beginPath();ctx.ellipse(vx+vw/2-4,fy-1,vw*.60,3,0,0,TAU);ctx.fill();
  ctx.fillStyle="rgba(0,0,0,.15)";
  ctx.beginPath();ctx.roundRect(vx-7,vy+7,vw,vh-7,9);ctx.fill();
  /* теплоноситель уходит в потолок и вбок к соседям — база связана трубами */
  bPipe([[vx+10,vy+4],[vx+10,y0+10],[x0+w-6,y0+10]],5,"120,140,158",lit);
  bPipe([[vx+vw-10,vy+6],[vx+vw-10,y0+22],[x0+w-6,y0+22]],4,"120,140,158",lit);
  /* ── сосуд: скруглённый бак, а не окно (M232) ──
     Прежний корпус был рамой вокруг светового столба во всю высоту и читался
     тюремной решёткой. Реактор — это ЖЕЛЕЗО: скруглённая бочка с куполом,
     и только маленький смотровой люк выдаёт, что внутри горит. */
  ctx.fillStyle="rgba(38,48,60,.97)";
  ctx.beginPath();ctx.roundRect(vx,vy+6,vw,vh-6,9);ctx.fill();
  ctx.beginPath();ctx.ellipse(vx+vw/2,vy+8,vw/2,9,0,Math.PI,TAU);ctx.fill();  // купол
  const mg=ctx.createLinearGradient(vx,0,vx+vw,0);
  mg.addColorStop(0,"rgba(170,190,210,"+(.10+lit*.12).toFixed(3)+")");
  mg.addColorStop(.45,"rgba(0,0,0,0)");
  mg.addColorStop(1,"rgba(0,0,0,.30)");
  ctx.fillStyle=mg;
  ctx.beginPath();ctx.roundRect(vx,vy+6,vw,vh-6,9);ctx.fill();
  ctx.fillStyle="rgba(150,170,190,"+(.14+lit*.12).toFixed(3)+")";   // кромка купола
  ctx.beginPath();ctx.ellipse(vx+vw/2,vy+7,vw/2-1,8,0,Math.PI*1.05,Math.PI*1.95);
  ctx.lineWidth=1.4;ctx.strokeStyle=ctx.fillStyle;ctx.stroke();
  /* обручи корпуса */
  for(let i=0;i<3;i++){
    const by=vy+14+i*(vh-24)/2.6;
    bBox(vx-3,by,vw+6,6,"rgba(34,43,54,.95)",lit,"rgba(0,0,0,.5)");
    ctx.fillStyle="rgba(190,205,220,"+(.10+lit*.12).toFixed(2)+")";
    for(let j=0;j<4;j++)ctx.fillRect(vx+j*(vw/4)+5,by+2,2,2);
  }
  /* клапан на куполе: раз в десяток секунд — мягкий выдох пара. Цикл с
  ...
  ctx.fillStyle="rgba(150,170,190,.3)";ctx.fillRect(vx+vw/2+9,vy-4,4,1);
  {
    ...
    if(sph<.16){
      ...
      for(let i=0;i<3;i++){
        const tp=Math.min(1,sph/.16+i*.22);
        ctx.fillStyle="rgba(215,225,235,"+(k*(.20-i*.05)*lit).toFixed(3)+")";
        ctx.beginPath();
        ctx.arc(vx+vw/2+11+tp*7+i*2,vy-7-tp*11-i*3,1.4+tp*2.6+i*.8,0,TAU);ctx.fill();
      }
    }
  }
  ...
  cg.addColorStop(0,"rgba(235,254,255,"+(.55+heat*.4*pulse).toFixed(3)+")");
  cg.addColorStop(.6,"rgba("+BM_CORE+","+(.30+heat*.45*pulse).toFixed(3)+")");
  cg.addColorStop(1,"rgba("+BM_CORE+",.05)");
  ctx.fillStyle=cg;ctx.fillRect(hx-hr,hy-hr,hr*2,hr*2);
  ctx.fillStyle="rgba(10,16,22,"+(.55+lit*.2).toFixed(2)+")";        // стержни за стеклом
  for(let i=-1;i<=1;i++)ctx.fillRect(hx+i*5-1,hy-hr,2,hr*2);
  ctx.strokeStyle="rgba(220,240,250,.30)";ctx.lineWidth=1;           // блик стекла
...
solar(x0,y0,w,h,cx,fy,lit,seed,B,P){
  ...
  for(let i=0;i<nc;i++){
    ...
    for(let t=0;t<2;t++){
      const ty=fy-32+t*15;
      ctx.fillStyle="rgba(52,62,76,.9)";ctx.fillRect(bx+2,ty+11,cw2-9,1.6);   // полка
      for(let j=0;j<3;j++){
        const jx=bx+4+j*((cw2-13)/3);
        ctx.fillStyle="rgba(38,48,60,.95)";ctx.fillRect(jx,ty,(cw2-17)/3,11);
        ctx.fillStyle="rgba(150,170,190,"+(.12+lit*.12).toFixed(2)+")";
        ctx.fillRect(jx,ty,(cw2-17)/3,1.2);                                   // крышка ловит свет
        ctx.fillStyle="rgba("+BM_COOL+","+(.25+lit*.30).toFixed(2)+")";
        ctx.fillRect(jx+1.5,ty+3,1.4,1.4);                                    // глазок банки
      }
  ...
  /* техник с щупом у крайнего шкафа: он ДЕЛАЕТ замер, а не стоит в нише */
  {
    ...
    bWorker(tx2-8,fy,lit,false,G.t*.045+seed+4,1);
    ctx.strokeStyle="rgba(210,180,90,"+(.5+lit*.3).toFixed(2)+")";ctx.lineWidth=1;
    ctx.beginPath();ctx.moveTo(tx2-2.6,fy-14.6);
    ctx.quadraticCurveTo(tx2+2,fy-16,tx2+3,fy-37);ctx.stroke();     // щуп к клемме
...
drill(x0,y0,w,h,cx,fy,lit,seed,B,P){
  ...
  for(let i=0;i<2;i++){
    ...
    ctx.fillStyle="rgba(0,0,0,.30)";                     // портал стоит на башмаках
    ctx.beginPath();ctx.ellipse(px+4.5,fy-1,10,2.2,0,0,TAU);ctx.fill();
  ...
  if(on)for(let i=0;i<5;i++){
    ...
    let hop=0;
    for(let k=0;k<5;k++){
      const d=Math.abs(ox-(bx+7+k*((bw2-14)/4)));
      if(d<5)hop=Math.max(hop,(1-d/5)*1.8);
    }
    ctx.fillStyle="rgba(0,0,0,"+(.4-hop*.1).toFixed(2)+")";  // тень куска на полотне
    ctx.beginPath();ctx.ellipse(ox,by+2.6,rx2+.4,1,0,0,TAU);ctx.fill();
    ctx.fillStyle="rgba(146,116,84,"+(.55+lit*.3).toFixed(2)+")";
    ctx.beginPath();ctx.ellipse(ox,by+.6-hop,rx2,ry2,(hp&7),0,TAU);ctx.fill();
    ctx.fillStyle="rgba(206,178,140,"+(.18+lit*.2).toFixed(2)+")";  // блик на куске
    ctx.beginPath();ctx.ellipse(ox-.8,by-.3-hop,rx2*.42,ry2*.36,(hp&7),0,TAU);ctx.fill();
  }
