...
function arrive(){
  ...
  if(ap.kind==="station"){
    ...
    /* ── орбита стала неспешной ──
       Корабль облетал планету за считаные секунды: на такой скорости она не
       тело, вокруг которого висишь, а карусель. Виток теперь занимает минуты —
       у планеты появляется вес, а у игрока время осмотреться и выбрать, с
       какой стороны заходить (сторона захода теперь решает, куда он сядет). */
    const w=dirCcw*clamp(Math.min(.6/Math.sqrt(r),cruise*.28/Math.max(r,1)),.0006,.006);
...
/* одна функция с корпусом (16c shipScaleAt): здесь стояло своё «.55», а корпус
...
const wakeLanes=new Map();   /* дорожки кильватера — рисует gpuWake (16ga) */
...
function drawTrail(zx,zy,Z){
  gpuWake(zx,zy,Z);   /* кильватер под шлейфом и под корпусом (16ga) */
