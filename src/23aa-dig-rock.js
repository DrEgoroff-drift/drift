/* ══════════════ шахта: порода ══════════════
   ...
   Рисуется через тайловый кэш (`gpuDrawTiles`, 18c): порода под камерой не
   меняется, значит печётся один раз — на видеокарте, — а кадр кладёт текстуры. */
...
function digRockPass(D,p,camx,camy){
  ...
  /* материала планеты под землёй нет: цвет породы дают пласты. Так выглядит кадр main —
     его тайл печётся раньше, чем готов материал, и больше не перепекается; с материалом
     порода серела до S .13 и теряла оливу пластов (Контроль 26.09: «цвет main») */
