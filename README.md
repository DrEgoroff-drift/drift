...
- **Generative music** — no tracks, just six layers (drone, bass, motif, beacons, percussion, atmosphere) that drift in and out, so there are no seams and no loop to notice. Notes are long: they swell over a second or more, decay while still sounding, and trail off into a synthesised reverb and a feedback delay, so each one overlaps the next instead of being plucked. The **beacon** voice isn't an oscillator at all — it's brown noise squeezed through a very narrow band-pass filter, which turns noise into a pitch that breathes, like a navigation beacon heard across empty space. The melody is a real phrase — a line that walks the scale rather than picking notes at random, sometimes answered a beat later by a third or a fifth — repeated with shifts so it stays recognisable without wearing out. Sixteen modes are in play, including pentatonic and Japanese scales where overlapping sustained notes stay consonant. Each location has its own mode and tempo: lydian and unhurried in open space, whole-tone on the star map, locrian and sparse in caves. Planets pick their mode, key and tempo from the same genome that decides their plants and creatures, so every world has its own music and sounds the same each time you return. A single tension value — pirates closing in, a battered hull, mine depth — tightens the rhythmic grid and thickens the percussion on its own, then lets it all spread out again, instead of switching to a separate combat track.
