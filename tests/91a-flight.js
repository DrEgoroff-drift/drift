...
TEST_SUITES.push(()=>suite("станция ↔ экран корабля",{tier:"browser"},()=>{
  resetWorld();
...
TEST_SUITES.push(()=>suite("маяк-телепорт",{tier:"browser"},()=>{
  resetWorld();
  landOnTestPlanet();
...
TEST_SUITES.push(()=>suite("музыка меняется по местам",{tier:"browser"},()=>{
  resetWorld();
...
TEST_SUITES.push(()=>suite("система: по метке можно ткнуть, промах цель не отменяет",{tier:"browser"},()=>{
  resetWorld();
