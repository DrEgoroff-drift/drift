...
TEST_SUITES.push(()=>suite("гравитационный якорь возвращает, а не запирает",()=>{
  ...
  /* Две беды подряд. Первая: якорь тянул к звезде сильнее двигателя — корабль
     ...
     Теперь якорь мягко возвращает: ускорение к звезде вдвое слабее двигателя
     плюс потолок на ход «прочь». Ход остаётся, но всегда домой. Проверяем
     ровно это: далеко не уходит, размах хода на краю мал, обратный курс
     свободен, и полная остановка невозможна. */
  let dEnd=0,amp=0;
  for(let i=0;i<2500;i++){
    ...
    if(i>1200){
      amp=Math.max(amp,Math.abs(vr));
    }
    dEnd=d;
  }
  ok(dEnd<5200,"дальше края корабль не уходит");
  ok(amp<.6,"размах хода у стены мал — камере дрожать не от чего");
  /* стоять на равновесии, упершись тягой в гравитацию, — это не застревание:
     важно, что стоит перестать давить, и корабль сам едет домой */
  keys.thrust=false;
  const dPause=Math.hypot(G.ship.x,G.ship.y);
  for(let i=0;i<300;i++)updateSystem(1);
  ok(Math.hypot(G.ship.x,G.ship.y)<=dPause+2,"отпустил тягу — дальше не уносит");
  ok(Math.hypot(G.ship.vx,G.ship.vy)>.2,"и корабль не замирает: якорь стал тяготением, у кромки он ходит по дуге");
  keys.thrust=true;
