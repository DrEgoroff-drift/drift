...
TEST_SUITES.push(()=>suite("сводка M431: заметка есть у каждого события",{tier:"node"},()=>{
  resetWorld();
  const bad=[];
  const check=(L,what)=>{
    const s=newsOf(L);
...
TEST_SUITES.push(()=>suite("сводка M431: одна запись — одна заметка у всех",{tier:"node"},()=>{
  resetWorld();
