...
TEST_SUITES.push(()=>suite("план: комбинат не останавливается, изделие берётся и довозится до базы",()=>{
  ...
  /* ── и одной миллисекунды хватало, чтобы покраснеть (0.426) ──
     ...
     машина занята, паузы длиннее) стал падать вдвое чаще. Спрашиваем ПРАВИЛО —
     смена долита почти до потолка и уж точно не пуста, — а не попадание в такт. */
  eq(T.run,0,"наряда нет");T.last=Date.now();tinTick(T);
  ok(T.run>49.5,"а смена идёт — комбинат не останавливается ("+T.run.toFixed(3)+")");
