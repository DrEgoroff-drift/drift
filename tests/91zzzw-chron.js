...
TEST_SUITES.push(()=>suite("война M372: свежий хозяин берёт треть и поднимает цены",()=>{
  chWorld();
  /* смотрим на ЖИВОЕ состояние: `occPowerAt` спрашивает его же, и подставлять
     сюда состояние другой сводки — значит проверять не то, что работает */
  const st=chronState();
  let fresh=null;
  for(const k of chronKeys()){
    const S=st.systems[k];
...
/* ── кэш на диске: повтор от него равен повтору от нуля (0.419) ──
   Набор выше проверял кэш В ПАМЯТИ (`chronReplay(400,mid)`) — он и проходил
   всегда. Расходились клиенты на диске: `chronSave` не писал строк летописи, а
   по ним державы считают обиды (`chronGrudge`, сутки) и семьи механик читают
   происшествия (до сорока сводок). Поднявшийся с диска шагал дальше с пустой
   памятью и уезжал в свою историю — это и был разъезд хэшей на сводках 993–999.
   Здесь проверяется полный круг: записали, прочитали, пошли дальше. */
TEST_SUITES.push(()=>suite("летопись: повтор от кэша НА ДИСКЕ равен повтору от нуля",()=>{
  chWorld();
  const a=chRun(400);
  chWorld();
  const mid=chRun(200);
  ...
  ok(!!disk,"кэш прочитался");
  eq(disk?disk.N:-1,200,"и он на той же сводке");
  ok((disk.lines||[]).length>0,"строки летописи в кэше есть");
  /* всё, что повтор ещё может прочитать (сорок сводок — самый долгий срок,
     SEC_SPY), обязано пережить диск целиком */
  const win=L=>200-L.N<=40;
  eq((disk.lines||[]).filter(win).length,mid.lines.filter(win).length,"хвост строк пережил диск целиком");
  eq(chronHash(chronReplay(400,disk)),chronHash(a),"повтор от кэша на диске равен повтору от нуля");
  /* и через две посадки подряд: кэш пишется поверх кэша */
  const st2=chronReplay(300,chronLoad());
  chronSave(st2);
  eq(chronHash(chronReplay(400,chronLoad())),chronHash(a),"и через две посадки подряд");
}));


/* ── кэш живёт в общем ключе и не затирает соседей (0.419.2) ──
   В `drift_war_v1` лежат четверо: состояние летописи, ведомости, циркуляры и
   метки снимков. Хозяин у каждого свой, и тот, кто пишет своё, обязан перенести
   чужое. `prov` не переносилась — и метка снимка открытой сводки пропадала на
   первом же сохранении состояния, вместе со всем смыслом метки. */
TEST_SUITES.push(()=>suite("летопись: кэш не затирает соседей по ключу",()=>{
  chWorld();
  const st=chRun(50);
  localStorage.setItem(CHRON_KEY,JSON.stringify({off:123,led:{"7":{}},prov:{"7":1},circ:[{n:1}]}));
  chronSave(st);
  const o=JSON.parse(localStorage.getItem(CHRON_KEY)||"null");
  ok(o&&o.led&&o.led["7"],"ведомости на месте");
  ok(o&&o.prov&&o.prov["7"],"метка снимка открытой сводки на месте");
  ok(o&&Array.isArray(o.circ)&&o.circ.length===1,"циркуляры на месте");
  eq(o?o.off:0,123,"и смещение часов — у него один хозяин, и это не летопись");
}));
