...
/* ── 1. ночь темнее дня ── */
TEST_SUITES.push(() => suite("свет: ночь на самом деле темнее дня",{tier:"heavy"}, () => {
  const rows = [], bad = [];
...
TEST_SUITES.push(() => suite("свет: облако не ярче своей звезды",{tier:"node"}, () => {
...
TEST_SUITES.push(() => suite("свет: в кадре ничто не спорит со звездой за источник",{tier:"heavy"}, () => {
  const bad = [], rows = [];
  let checked = 0;
  for (const { s, p } of plWorlds(6)) {
    ...
    S.p.wx = { kind: null };
    /* и без облаков: кучевое, наплывшее на диск, гасит его вдвое, а соседнее,
       освещённое, светит ярче — и набор краснел по ветру и часу (лаборатория,
       первая ночь: Нейэль I 0.52 против 0.69, без облаков 0.74 против 0.69).
       Облака судятся по краске набором выше; здесь — небо и его источник */
    const SR = sunSpot(S.p);
    if (!SR.up || SR.alt < .15) continue;
    lgClean();
    CLOUDS_OFF = true;
    try { drawWorld(); } finally { CLOUDS_OFF = false; }
...
TEST_SUITES.push(() => suite("свет: у звезды и у фонаря яркость падает с расстоянием, а не обрывается",{tier:"browser"}, () => {
  const rows = [], bad = [];
  const halo = (ru) => {
...
TEST_SUITES.push(() => suite("свет: свечение дышит плавно, а не щёлкает между кадрами",{tier:"heavy"}, () => {
  const bad = [], rows = [];
...
TEST_SUITES.push(() => suite("свет: ни одна сцена не выжжена в белое",{tier:"heavy"}, () => {
  const bad = [], rows = [];
  let scenes = 0;
...
TEST_SUITES.push(() => suite("свет: пламя факела живёт, но не щёлкает между кадрами",{tier:"browser"}, () => {
  resetWorld();
...
/* заслоны планет (sysOcPush, 17-mode-system): тень — луч от звезды через тело; тело за краем кладётся, только
   если тень заходит в экран, и сдвигается к краю (f16 заслона, диск без тени вне экрана) */
TEST_SUITES.push(() => suite("свет: планета тенит корабли и из-за края экрана",{tier:"node"}, () => {
  const O=GPU.oc,n0=O.length;
  try{
    O.length=0;sysOcPush(W/2,H/2,40,W/2-300,H/2);
    eq(O.length,1,"планета на экране — заслон");
    ok(O[0][0]===W/2&&O[0][2]===40,"на месте и своего радиуса");
    O.length=0;sysOcPush(-400,H/2,60,-900,H/2);
    eq(O.length,1,"планета левее экрана, звезда ещё левее — тень идёт в экран, заслон есть");
    const [x,,r]=O[0];
    ok(x+r<=0&&x>-400,"сдвинут по лучу к краю, диск в экран не заходит: x "+x.toFixed(0));
    O.length=0;sysOcPush(-400,H/2,60,W+900,H/2);
    eq(O.length,0,"звезда за экраном справа — тень уходит влево, заслона нет");
    O.length=0;sysOcPush(-400,-400,60,-900,H/2);
    eq(O.length,0,"тень мимо экрана — заслона нет");
    O.length=0;for(let i=0;i<20;i++)sysOcPush(W/2,H/2,40,0,0);
    eq(O.length,15,"шестнадцатый — станции");
  }finally{O.length=n0;}
}));
