...
const EYES_SUITE=() => suite("глаза тестов: видеокарта есть",{tier:"browser"}, () => {
  ok(GPU.ok,"глаза тестов слепы: видеокарты нет"+(GPU.none?" (браузер без WebGPU)":
    " (не поднялась: опросов "+BOOT_GPU_POLL+", ~"+Math.round(BOOT_GPU_POLL*BOOT_GPU_MS/1000)+" с настоящих"+
    (GPU.lost?", устройство потеряно":GPU.busy?", адаптер не ответил":"")+")"));
});
BOOT_SUITE.pin="first";
let BOOT_SINK=0,BOOT_PIPE_POLL=0,BOOT_GPU_POLL=0;
/* опрос адаптера: ~20 мс настоящего счёта (2e7 шагов прогретого цикла) за 25 мс виртуальных */
const BOOT_GPU_MS=20;
...
(function boot(t0){
  ...
  /* видеокарта: мир рисует только она (08b). Под --virtual-time-budget часы
     ...
     Счёт — 2e7 шагов, не 2e6: прогретый цикл проходит 2e6 за 2 мс, а не за 10, и 14 с виртуальных
     давали ~0.6 с настоящих — при -Full -Jobs 3 три Хрома поднимают адаптер разом, и часть оставалась
     слепой (26.09). Теперь ~560 опросов по ~20 мс: до ~11 с настоящих на адаптер, потолок виртуальных тот же
     (14 с из бюджета 20). Настоящая «не поднялась» ловится как прежде — набором «глаза тестов» */
  if(!GPU.ok&&!GPU.none&&performance.now()-t0<14000){let s=0;for(let k=0;k<2e7;k++)s+=k&1;BOOT_SINK=s;BOOT_GPU_POLL++;setTimeout(()=>boot(t0),25);return;}
