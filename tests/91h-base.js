...
TEST_SUITES.push(()=>suite("смотритель: энергия, стройка и логистика",()=>{
  ...
  foundBase(p);
  const B=baseAt(G.sx,G.sy,p.idx);
  /* без погоды: буря выбивает верхний ряд, а весь этот набор меряет энергию
     верхнего ряда. С M390 бросок бури берётся от НОМЕРА смены — значит исход
     зависел бы от того, в котором часу запущен прогон. Ставим базу там, где не
     дует (`STORM_WORLDS.gas`=0), и меряем то, что собирались. */
  B.type="gas";
