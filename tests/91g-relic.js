...
TEST_SUITES.push(()=>suite("лаборатория: домен исследователя, а не воздух",{tier:"node"},()=>{
  resetWorld();
  G.credits=300000;
...
TEST_SUITES.push(()=>suite("артефакты: слот один, эффект глобальный",{tier:"node"},()=>{
  resetWorld();
  G.credits=300000;
  hireMgr(genMgr(31,["sci"]));
...
TEST_SUITES.push(()=>suite("оживлённые перки исследователя",{tier:"node"},()=>{
  resetWorld();
  G.credits=300000;
  hireMgr(genMgr(31,["sci"]));
  const sci=mgrOf("sci");
