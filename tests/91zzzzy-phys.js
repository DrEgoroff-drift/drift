...
TEST_SUITES.push(() => suite("физика: потолок скорости держится и на рваном кадре",{tier:"browser"}, () => {
...
TEST_SUITES.push(() => suite("физика: та же планета даёт тот же мир — рельеф, залежи, устье",{tier:"browser"}, () => {
  const bad = [];
  let worlds = 0;
...
TEST_SUITES.push(() => suite("физика: падение кончается грунтом, а не под ним, при любом шаге кадра",{tier:"browser"}, () => {
  const bad = [];
  let drops = 0;
...
TEST_SUITES.push(() => suite("физика: в пещере тело не проходит сквозь камень и на рваном кадре",{tier:"heavy"}, () => {
  const bad = [];
