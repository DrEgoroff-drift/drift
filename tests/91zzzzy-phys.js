...
TEST_SUITES.push(() => suite("физика: потолок скорости держится и на рваном кадре",{tier:"node"}, () => {
...
TEST_SUITES.push(() => suite("физика: та же планета даёт тот же мир — рельеф, залежи, устье",{tier:"node"}, () => {
  const bad = [];
  let worlds = 0;
...
TEST_SUITES.push(() => suite("физика: падение кончается грунтом, а не под ним, при любом шаге кадра",{tier:"node"}, () => {
  const bad = [];
  let drops = 0;
