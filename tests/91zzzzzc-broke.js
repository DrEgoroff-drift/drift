...
TEST_SUITES.push(() => suite("нищий: с нулём в кассе и пустым трюмом двойной тычок ничего не ломает",{tier:"browser"}, () => {
  resetWorld(); fuzzRich();
...
TEST_SUITES.push(() => suite("полный трюм: покупка и погрузка не пролезают сверх ёмкости",{tier:"heavy"}, () => {
  resetWorld(); fuzzRich();
...
TEST_SUITES.push(() => suite("устаревшая кнопка: ушедшая с экрана строка не платит второй раз",{tier:"heavy"}, () => {
...
TEST_SUITES.push(() => suite("новичок: экраны первого часа рисуются и жмутся на пустом мире",{tier:"browser"}, () => {
