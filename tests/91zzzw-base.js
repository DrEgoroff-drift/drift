...
/* ── номер смены для набора: ПРИШПИЛЕННЫЙ, а не с часов (M418) ──
   `baseShift()` считается от `Date.now()`. Набор, который берёт его и потом
   дёргает `palStep(B,n+3)` или `devStep(B,n+6)`, зависит от времени суток:
   стоит окну наехать на середину отчётного периода — приходит инспектор со
   своим штрафом, и «пеня 180» становится «300». Так и случилось 07.09.2026,
   на правке, которая к ПАЛАТЕ отношения не имела.

   `bShift(k)` даёт ТЕКУЩУЮ смену, сдвинутую вниз до кратной `k`: абсолютное
   число по-прежнему плывёт (иначе догон от `B.t0` до `Date.now()` пришлось бы
   считать за сотни смен), а вот МЕСТО В РАСПИСАНИИ пришпилено — и именно оно
   ломало наборы. `bShift(PAL_PERIOD)` даёт n%90===0, значит n+1…n+4 далеко и
   от проверки (середина периода), и от прогноза за четыре смены до неё.

   Правило простое: если набор считает от номера смены (`n+3`, `n-1`, `n%X`),
   он обязан взять его через `bShift(X)`. Голый `baseShift()` — только там, где
   номер никуда не идёт дальше журнальной записи. */
function bShift(k){
  const n=baseShift();
  return k?n-(n%k):n;
}
...
TEST_SUITES.push(()=>suite("база M391: выброс на пустой базе не отнимает воздуха",()=>{
  /* Найдено 09.09.2026: «за двадцать смен без людей воздух не тронут» падал —
     120 → 60, ровно половина, и это был директорский «выброс». Запас воздуха
     — запас ДЛЯ ЛЮДЕЙ; его отнимает только тот, кто дышит. То же правило уже
     стояло у течи атмосферы (`leak && baseCrewN`) и у срока пожара (0.409.1). */
  const B=bLife();
  const e={k:"vent"};
  G.crew=[];
  const a0=baseLife(B).air,log0=B.log.length;
  ok(!baseEventApply(B,e,baseShift()),"без людей выброса не случилось");
  eq(baseLife(B).air,a0,"и воздух цел");
  eq(B.log.length,log0,"и в журнал нечего писать");
  /* с людьми — половина, как и было задумано */
  bCrew(B,2);
  ok(baseEventApply(B,e,baseShift()),"с людьми выброс случается");
  eq(baseLife(B).air,(a0/2)|0,"и берёт половину: "+a0+" → "+baseLife(B).air);
  ok(B.log.some(x=>x.k==="vent"),"и журнал это говорит");
}));
...
TEST_SUITES.push(()=>suite("база M391: встала, но не умерла",()=>{
  ...
  baseSupply(B,"ice",5);
  /* один лёд — одна вода (разбор 0.409.1): он ложится НА СКЛАД и становится
     водой через ледоплавку. Прямо в шкалу его больше не льют — иначе единица
     давала две воды разом */
  eq(B.pool.ice|0,ice0+5,"лёд лёг на склад");
  eq(baseLife(B).water,w0,"а водой станет через машину, не сам собой");
}));
...
TEST_SUITES.push(()=>suite("база M394: сигнал решает, что слышно",()=>{
  ...
  /* дистанцию среднего сигнала ищем, а не угадываем: на восьми секторах он
     сильный, и проверка слов годами не шла вовсе (M442) */
  let W=null;
  for(let d=4;d<=80&&!(W&&W.lvl===2);d++)W=baseReport(B,B.sx+d,B.sy);
  if(ok(W&&W.lvl===2,"есть дистанция, на которой сигнал средний: "+(W&&W.lvl))){
    ok(W.head.indexOf("—")>0,"на среднем сигнале дают слова: "+W.head);
    /* позывной «БЗ-417» — трёхзначный всегда: проверка годами не шла и потому
       не знала, что судила бы его (M442) */
    ok(!/\d\d\d/.test(W.head.slice(baseCall(B).length)),"и ни одного длинного числа, кроме позывного");
  }
}));
...
TEST_SUITES.push(()=>suite("база M395: маяк приводит людей, и их берут за руку",{tier:"browser"},()=>{
  const B=bLife();
