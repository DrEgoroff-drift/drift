...
TEST_SUITES.push(()=>suite("холдинг: замер против §16 (отчёт и три правила)",()=>{
  ...
  note("торговля · «Стриж», без нужды: "+Math.round(tradeMin)+" кр/мин = "+Math.round(tradeMin*60)+" кр/час"+
  ...
  note("пай · лучший цех ×1: "+best.ru+" — "+Math.round(best.shareShift)+" кр/смену за "+best.inUnits+" ед = "+best.perUnit.toFixed(1)+" кр/ед · окупаемость "+best.payLoops.toFixed(1)+" круга");
  note("пай · худший цех ×1: "+worst.ru+" — "+Math.round(worst.shareShift)+" кр/смену за "+worst.inUnits+" ед = "+worst.perUnit.toFixed(1)+" кр/ед · окупаемость "+worst.payLoops.toFixed(1)+" круга");
  const avgPerUnit=rows.reduce((a,r)=>a+r.perUnit,0)/rows.length;
  const tradePerUnit=L40?(L40.sell-L40.buy):0;
  note("§16.2 · пай против торговли за ЕДИНИЦУ трюма: "+avgPerUnit.toFixed(1)+" кр/ед против "+tradePerUnit+" кр/ед на лучшем плече = ×"+(tradePerUnit?(avgPerUnit/tradePerUnit).toFixed(2):"—")+" (цель 1.3–1.8)");
  const shareHour=best.shareShift*3;                   /* три смены в час, если кормить и забирать */
  note("§16.1 · доход в час: торговля "+Math.round(tradeMin*60)+" → торговля + лучший пай "+Math.round(tradeMin*60+shareHour)+" = ×"+(tradeMin?(1+shareHour/(tradeMin*60)).toFixed(2):"—")+" (цель 1.5–2)");
  /* ── три правила ── */
  const pays=rows.filter(r=>r.payLoops>=4&&r.payLoops<=6).length;
  note("§16.3 · окупаемость ×1 в 4–6 кругов: "+pays+" из "+rows.length+" цехов · разброс "+rows.reduce((a,r)=>Math.min(a,r.payLoops),1e9).toFixed(1)+"–"+rows.reduce((a,r)=>Math.max(a,r.payLoops),0).toFixed(1));
  ...
  note("§16.9 · баржа «Вьюк»: "+vy.cargo+" ед на борту, за смену ссыпает до "+Math.min(vy.cargo,best.inUnits*HOLD_CAP_SHIFTS)+" ед в один цех ×1 · кр/мин = 0 ≤ фактор");
  ...
  for(const fam of ["C","D"]){
    ...
    note("ярус "+(fam==="C"?2:3)+" · средняя цена входа "+Math.round(cost/ids.length)+" кр · средний пай за смену "+Math.round(share/ids.length)+" кр · окупаемость "+(cost/ids.length/(share/ids.length*2)).toFixed(1)+" круга");
  }
  ...
  note("добыча · Реголитовая разработка ×1: выгода "+Math.round(srcVal)+" кр/смену (0.3 цены на "+Object.keys(src.makes).map(k=>src.makes[k]).join("+")+" ед) · окупаемость "+(srcCost/(srcVal*2)).toFixed(1)+" круга");
  ...
  note("аппетит · лучшая надбавка за смену в радиусе 7: "+appBest+" кр · против пая лучшего цеха "+Math.round(best.shareShift)+" кр");
}));
