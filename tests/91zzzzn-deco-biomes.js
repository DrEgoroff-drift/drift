...
TEST_SUITES.push(()=>suite("крупная форма: своя семья у каждого сухого мира",{tier:"browser"},()=>{
  resetWorld();
...
TEST_SUITES.push(()=>suite("крупная форма: рисуется на всех мирах, две-четыре на экран",{tier:"heavy"},()=>{
  resetWorld();
  const land=Object.keys(TYPES).filter(t=>t!=="gas");
  const fails=[];
  for(const t of land){
