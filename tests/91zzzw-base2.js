...
TEST_SUITES.push(()=>suite("база M397: погода вместо костей",{tier:"browser"},()=>{
  const B=bLife();
...
TEST_SUITES.push(()=>suite("база M397: у каждой погоды своё последствие",{tier:"browser"},()=>{
  ...
  B.vein=0;
  /* выброс: половина воздуха — но только пока есть кому дышать. Запас воздуха
     заперт тем же правилом, что течь атмосферы и срок пожара: на пустой базе
     он не тратится вовсе (набор «выброс на пустой базе не отнимает воздуха») */
  bCrew(B,2);
  baseLife(B).air=100;
...
TEST_SUITES.push(()=>suite("база M401: сведения покупаются",()=>{
  ...
  else ok(!w1&&!w2,"в эту смену прогноза нет вовсе — ни без сведений, ни с ними: «"+w1+"» / «"+w2+"»");
}));
