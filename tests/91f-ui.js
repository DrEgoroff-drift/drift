...
TEST_SUITES.push(()=>suite("интерфейс: приборы и кнопки не наезжают друг на друга",{tier:"browser"},()=>{
  ...
  /* кнопки борта набор показал сам: наблюдатель вёрстки скажет о них микрозадачей
     позже, а мерка --railw (27z) нужна сейчас — иначе подсказка стоит по борту прошлого набора */
  hud();hudFloorMeasure(true);
...
TEST_SUITES.push(()=>suite("интерфейс: кнопка называет действие, а не себя",{tier:"browser"},()=>{
  ...
  /* длинный глагол не прячется за безымянным ДЕЙСТВИЕ (R6, 0.448.0): два первых
     слова и число — кнопка называет действие всегда */
  G.prompt="ДЕЙСТВИЕ — СИНТЕЗ ТОПЛИВА ИЗО ЛЬДА (12)";hud();
  eq($act.textContent,"Синтез топлива 12","длинный глагол режется до двух слов и числа");
  G.prompt="ДЕЙСТВИЕ — ВЗЯТЬ НА БУКСИР";hud();
  eq($act.textContent,"Взять на","три слова — два первых");
...
TEST_SUITES.push(()=>suite("интерфейс: на чистом старте в кадре нет лишнего",{tier:"browser"},()=>{
  ...
  const OK=["c","g","slope","hud","ovl","labels","msg","prompt","console","rail","pads","menu"];
  const stray=[];
...
TEST_SUITES.push(() => suite("жёрдочка: птицу не рисуют каждую секунду",{tier:"node"}, () => {
  /* Пульт обновляется раз в секунду, и всё это время сюда рисовался ВЕСЬ
     ...
     секунду и до конца игры. Считаем ВЫЗОВЫ, а не миллисекунды: часов в
     стенде нет (см. CLAUDE.md), и время тут ничего не скажет. */
  resetWorld();
  G.parrot=null;parrotFind(7,"чужого борта");
  PERCH_AT=-1e9;                 /* холодный старт: соседний набор уже тикал пультом */
  const real=parrotDraw;
  let n=0;
  try{
    parrotDraw=function(){n++;return real.apply(null,arguments);};
    conT=0;consoleTick(1);
    eq(n,1,"первый раз — нарисовали");
    for(let i=0;i<8;i++){conT=0;consoleTick(1);}
    eq(n,1,"восемь обновлений пульта подряд — по-прежнему один рисунок");
    /* а через свой срок — снова */
    G.t+=PERCH_EVERY+1;conT=0;consoleTick(1);
    eq(n,2,"через пять секунд поза обновилась");
  }finally{parrotDraw=real;}
  resetWorld();
}));
