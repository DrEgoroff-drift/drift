...
TEST_SUITES.push(() => suite("имена: каждый звук, который зовут, есть в таблице звуков",{tier:"node"}, () => {
  ...
  note("звуков без единого вызова: " + idle.length + (idle.length ? " (" + idle.slice(0, 6).join(", ") + ")" : ""));
}));
...
TEST_SUITES.push(() => suite("имена: вкладки станции объявлены её типом",{tier:"browser"}, () => {
...
TEST_SUITES.push(() => suite("имена: у каждой вещи в таблицах есть код, который её читает", () => {
  ...
  /* ── исключений нет ──
     Последнее — артефакт «Карта чужой руки» — ушло с ответом (M342): «где торгуют
     редким» — это «Сорока», и карта показывает её стоянку (12v, relicOn("chart")).
     Новое исключение сюда вписывать нельзя без строки в PLAN.md. */
  const KNOWN = new Set([]);
  const mute = [];
