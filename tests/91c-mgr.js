...
TEST_SUITES.push(()=>suite("управляющий: найм, домен и доля",()=>{
  ...
  ok(G.credits>=before,"оклад больше не вычитается из вашей кассы — он гасится из доли (M152e)");
  ok((m.spent|0)>=0&&(m.pool|0)>=0,"оклад взят из пула доли, пул не ушёл в минус");
}));
...
TEST_SUITES.push(()=>suite("управляющий: уровни, перки, слоты",()=>{
  ...
  coopStamp();eq(crewCap(),1+techLv("license")+1,"место в экипаже прибавилось");
...
TEST_SUITES.push(()=>suite("репутация: у своих садятся стоящие",()=>{
  ...
  /* ── налёт наёмников судим ПО НЕСКОЛЬКИМ станциям (M337) ──
     Один бросок здесь ничего не доказывает: состав зала сеется двухсуточным
     ведром времени (`timeBucket`), и в иное ведро лучший наёмник у этой
     станции и без репутации, и с ней выходит один и тот же. Так тест и упал
     04.09.2026 в 03:00 по местному — ровно на смене ведра, ничего в игре не
     меняя. Свойство, которое проверяем, статистическое: у своих зал В ЦЕЛОМ
     крепче. Берём десяток станций и сравниваем суммы. */
  const stations=[];
  for(let dx=-8;dx<=8&&stations.length<10;dx++)for(let dy=-8;dy<=8&&stations.length<10;dy++){
    if(!starAt(dx,dy))continue;
    const s=getSystem(dx,dy);
    if(s.station)stations.push(s);
  }
  ok(stations.length>=5,"станций под сравнение: "+stations.length);
  let sum0=0,sum1=0,up=0;
  for(const s of stations){
    G.rep={};
    const a=best(stationMercs(s));
    repAdd(5,s);
    const b=best(stationMercs(s));
    sum0+=a;sum1+=b;
    if(b>a)up++;
  }
  G.rep={};repAdd(5,sys);
  ok(sum1>=sum0,"у своих зал не хуже по налёту: "+sum0+" → "+sum1+" (выросло у "+up+" из "+stations.length+")");
  ok(sum1>sum0||sum1>=100*stations.length*.2,"и репутация зал действительно поднимает");
