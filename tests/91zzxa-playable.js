...
TEST_SUITES.push(()=>suite("масштаб: корабль не мельче .7, потолок растёт с зумом, мир зумится до ×4.5, диск — физический",()=>{
  resetWorld();
  eq(shipScaleAt(.16),.7,"пол корабля .7");
  eq(shipScaleAt(1),.8,"на зуме ×1 корабль .8");
  near(shipScaleAt(4.5),SHIP_SCALE_MAX+.6,1e-9,"потолок мягко растёт у всех: на ×4.5 корабль 1.4 (автор 18.09, отменяет решение 12.09)");
