...
TEST_SUITES.push(()=>suite("слухи: область, не точка; образ, деталь, источник; пятнадцать процентов врут",()=>{
  ...
  /* «Сорока» (M342) кладёт третий слух, когда стоит рядом — считаем только слухи регионов */
  const L=rumoursHere().filter(q=>q.id!=="wander");eq(L.length,2,"два слуха на станции");
  for(const q of L){
