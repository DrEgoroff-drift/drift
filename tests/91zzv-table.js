...
TEST_SUITES.push(()=>suite("меню: пять дверей, журнал/отчёт/трепло ушли на стол и на пульт",{tier:"browser"},()=>{
  ...
  /* Шестая с 11.09 — ДОМОЙ: прыжок к дому откуда угодно (автор: «можно вызвать
     только со станции»), 16c-rescue */
  eq(ids.length,6,"в меню шесть дверей (M286: ЭКИПАЖ и ШТАБ свелись в ДЕЛО; 11.09 — ДОМОЙ)");
  ok(ids.indexOf("callbtn")>=0,"ДОМОЙ в меню");
...
TEST_SUITES.push(()=>suite("станция: ДОСКА у всех, очередь у стойки пишется в ЛЮДИ",{tier:"browser"},()=>{
  ...
  /* ── ДОСКА — РАЗДЕЛ, а не вкладка второго яруса (0.426) ──
     Набор искал кнопку в `#stTabs` и требовал, чтобы она была видна. Вторая
     полоса показывает вкладки ТЕКУЩЕГО раздела, а у доски раздел свой, из
     одной вкладки: кнопка на ней видна ровно тогда, когда игрок УЖЕ на доске.
     `openStation` вкладку не меняет — игрок возвращается туда, где был, — и
     проверка держалась на том, что предыдущий набор ушёл со станции именно
     с доски. Стоило поменять порядок наборов (прогон по частям, -Jobs 8), и
  ...
  if(!grp)return;
  grp.click();
  eq(tab,"board","раздел открывает доску");
  const btn=[...document.querySelectorAll("#stTabs button")].find(b=>b.dataset.tab==="board");
  ok(btn&&btn.style.display!=="none","и на второй полосе кнопка ДОСКА показана");
...
TEST_SUITES.push(()=>suite("стол: огонёк гаснет от визита, а не от чтения",{tier:"browser"},()=>{
  ...
  applySave(s);
  /* `|0` здесь и была поломка: `Date.now()` не влезает в 32 бита, и раз в
     49 суток срезанное значение выходит отрицательным — набор краснел по
     календарю, а не по коду. Сравниваем число целиком. */
  ok(G.tableSeen>0,"запись без отметки грузится как «стол видели»");
  /* и метка переживает круг «сохранил — загрузил» без потери разрядов */
  const mark=G.tableSeen;
  applySave(snapshot());
  eq(G.tableSeen,mark,"отметка вернулась из записи целой, а не срезанной");
  logBtnLabel();
