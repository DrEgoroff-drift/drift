...
/* ── 1. пустой бак на грунте ──
   ...
   денег, но с домом. */
TEST_SUITES.push(() => suite("сценарий: с пустым баком на планете всегда есть выход",{tier:"browser"}, () => {
  const land = () => {
    resetWorld();
...
TEST_SUITES.push(() => suite("сценарий: без топлива и без хода в космосе есть чем себе помочь",{tier:"browser"}, () => {
  resetWorld();
...
TEST_SUITES.push(() => suite("сценарий: круг «купил — продал» на одном прилавке всегда в минус",{tier:"browser"}, () => {
  resetWorld();
  const sys = G.sys;
  G.credits = 100000;
...
TEST_SUITES.push(() => suite("сценарий: после аварии игрок жив, с топливом и не в петле",{tier:"browser"}, () => {
  resetWorld();
...
TEST_SUITES.push(() => suite("сценарий: первый рейс с нуля выходит в плюс",{tier:"browser"}, () => {
  resetWorld();
  const w = plWorlds(1)[0];
  const cr0 = G.credits;
...
/* ── 7. автопилот доводит, а не наматывает круги ──
   ...
   во что это обошлось. */
TEST_SUITES.push(() => suite("сценарий: автопилот доводит до цели и не съедает бак",{tier:"browser"}, () => {
  const bad = [], rows = [];
  let runs = 0;
  const targets = [];
  resetWorld();
