...
TEST_SUITES.push(()=>suite("растр: грунт и свод рисуются ломтями, а не каждый кадр",{tier:"browser"},()=>{
  resetWorld();
  const p=landOnTestPlanet();
  const tr=G.surf.tr;
  /* ── материал печётся по кадрам (M418) ──
     Ломти делаются только когда материал готов, а он теперь приходит через
     полсекунды кадров, а не мгновенно. Набор про ЛОМТИ, а не про расписание
     выпечки: печём тайл сразу и меряем то, ради чего набор написан. */
  tr.mat=planetMatNow(p);
  drawSurface();
...
TEST_SUITES.push(()=>suite("дальняя гряда: свой рельеф, а не растянутый здешний",{tier:"browser"},()=>{
  resetWorld();
...
TEST_SUITES.push(()=>suite("поверхность: рост человека — доля кадра, а не пиксель",{tier:"browser"},()=>{
  resetWorld();
  landOnTestPlanet();
  const pW=W,pH=H;
  /* сама мерка: обе стороны кадра, тесная решает (M222) */
  W=1280;H=480;  eq(surfScale(),1,"низкое окно не ужимаем: мира и так мало");
  W=390; H=1120; eq(surfScale(),1,"и узкое тоже — телефону высота не помогает");
  W=1000;H=SURF_BASE*2; near(surfScale(),1,.001,"высота вдвое, а ширина в обрез — растём по тесной стороне");
  W=2000;H=SURF_BASE*2; near(surfScale(),2,.001,"обе стороны вдвое — и мир вдвое");
  W=9000;H=4000; eq(surfScale(),SURF_KMAX,"выше потолка не поднимаемся: кадр перестанет быть миром");
  /* и главное, ради чего всё: доля кадра одна и та же на любом экране */
  W=1280;H=720;  const s1=26*surfScale()/H;
  W=2560;H=1400; const s2=26*surfScale()/H;
  ok(Math.abs(s1-s2)<.01,"ходок занимает ту же долю кадра в 720 и в 1400");
  ok(s1>.04,"и эта доля больше прежних 3.6%");
  W=pW;H=pH;

  const _p=G.surf&&G.surf.p;if(_p)G.surf.tr.mat=planetMatNow(_p);
  drawSurface();
...
TEST_SUITES.push(()=>suite("интерфейс: растёт вместе с окном, но медленнее мира",{tier:"browser"},()=>{
  resetWorld();
  ...
  eq(uiScale(900,1400),1,"а в узком и высоком окне — единица: ширины и так нет (M222)");
...
TEST_SUITES.push(()=>suite("солнце светит оттуда, где нарисовано",{tier:"browser"},()=>{
  resetWorld();
  const p=landOnTestPlanet();
  let checked=0,bad=[];
  const t0=G.t;
...
TEST_SUITES.push(()=>suite("растр: материал грунта печётся по кадрам, а не одним куском",{tier:"node"},()=>{
  ...
  /* первый спрос материала не даёт — он ставит заказ */
  eq(planetMat(p),null,"пока не испечён — пусто, и это не ошибка");
  ok(typeof MAT_JOB==="object"&&MAT_JOB&&MAT_JOB.p===p,"заказ поставлен");
  /* порция — РОВНО столько строк, сколько заказано: это единственное, что
     здесь можно мерить без часов. Сколько порций уложится в кадр, решает
     машина, и проверять это набором значило бы мерить машину */
  const y0=MAT_JOB.y|0;
  matJobRows(MAT_JOB,1);
  eq(MAT_JOB.y,y0+1,"порция в одну строку двигает работу ровно на строку");
  ok(!p.mat,"и одной строкой дело не кончается");
  ok(MAT_ROWS<=4&&MAT_MS<=6,"бюджет мелкий: "+MAT_ROWS+" строк, "+MAT_MS+" мс");
  /* и доводим до конца тиками, как это делает кадр */
  let guard=0;
  while(!p.mat&&guard++<4000)matTick();
  ok(!!p.mat,"за конечное число тиков материал готов ("+guard+" тиков)");
  eq(MAT_JOB,null,"задание снято");
  /* второй спрос отдаёт готовое сразу */
  ok(planetMat(p)===p.mat,"готовый материал отдаётся без нового заказа");
}));

TEST_SUITES.push(()=>suite("растр: без материала грунт рисуется, а не падает",{tier:"browser"},()=>{
  resetWorld();
  const p=landOnTestPlanet();
  const tr=G.surf.tr;
  delete p.mat;delete p.matCn;tr.mat=null;tr.chunks=null;
  /* кадр без материала обязан пройти целиком: игрок в это время уже ходит */
  let err=null;
  try{drawSurface();}catch(e){err=e&&e.message||String(e);}
  eq(err,null,"кадр без материала прошёл: "+err);
  /* и стенду доступна выпечка разом — ради снимков и наборов */
  const m=planetMatNow(p);
  ok(!!m,"planetMatNow печёт целиком");
  ok(!!p.mat,"и кладёт результат планете");
}));
