...
TEST_SUITES.push(()=>suite("три пересмотренные сцены рисуются",{tier:"browser"},()=>{
  resetWorld();
  ...
  if(PB){
    enterRaid(PB);
    ok(T.ledger(drawRaid).calls>0,"абордаж рисуется");
  }
...
TEST_SUITES.push(()=>suite("крупная форма: поздний мир виден силуэтом",{tier:"heavy"},()=>{
  resetWorld();
  ...
  for(const t of late){
    ...
    ok(T.ledger(drawSurface).calls>20,t+": кадр с крупной формой рисуется");
  }
...
TEST_SUITES.push(()=>suite("посадочный корабль: масштаб и три точки опоры",{tier:"browser"},()=>{
  resetWorld();
  const ASTRO=24;
...
TEST_SUITES.push(()=>suite("пиратский корпус: сварен, а не покрашен",{tier:"browser"},()=>{
  resetWorld();
  const seen={};
  for(let i=0;i<40;i++){
...
/* ══════════════ порода читается силуэтом (M369, §19.4) ══════════════
   Свод §0 закон 7: чужой корпус называет класс первым, изготовителя вторым.
   Мерит это `makerRead()` (28y-look) — тот же прибор, что судит кадр: рисует
   корпус студией движка (17c2, как в полёте — со светом) в маленький кадр и по
   пикселям решает, чей он. 2D-двойника корпуса прибор не зовёт (п. h, 26.09). Порог — 90 %, и он
   не круглое число: ниже него на листе видно, как две породы сходятся.
   Быстрая проверка живёт в ярусе Хрома, полная (сотня семян на класс) — в
   -Full: шестьсот корпусов рисовать на каждую правку незачем. */
TEST_SUITES.push(()=>suite("изготовитель M369: порода читается силуэтом и краской",{tier:"browser"},()=>{
  resetWorld();
  const dh=drawHull;let nh=0;window.drawHull=function(){nh++;return dh.apply(this,arguments);};
  let r;try{r=makerRead(14);}finally{window.drawHull=dh;}
  eq(nh,0,"прибор смотрит на кадр движка: 2D-корпус не рисуется ни разу");
  ok(r.n>=80,"прибор посмотрел на корпуса: "+r.n);
  ok(r.acc>=90,"порода читается на "+r.acc+" % (нужно 90): "+
     r.rows.map(x=>x.ru+" "+x.acc).join(" · "));
  for(const row of r.rows)
    ok(row.acc>=70,row.ru+": "+row.acc+" % (ниже семидесяти — порода не своя)");
}));

TEST_SUITES.push(()=>suite("изготовитель M369: сотня семян на класс, силуэт держится",{tier:"heavy"},()=>{
  resetWorld();
  const r=makerRead(100);
  ok(r.acc>=90,"на "+r.n+" корпусах порода читается на "+r.acc+" %: "+
     r.rows.map(x=>x.ru+" "+x.acc).join(" · "));
}));
