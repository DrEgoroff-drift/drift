...
const DET_MUTE={
  ...
  /* база: клеть ходит по ярусам W/S, отсеки — A/D (21a-mode-base); сцена
     ставит клеть на второй ярус (28y-look), и W обязана поднимать — молчание
     снято (0.441.0; первая попытка упёрлась в набор подсказки, который не
     видел открывшееся меню, — починен он, не сцена). Холст базы рукой не
     трогают — у tap() её нет */
...
/* max — сколько кадров обычно хватает; пока печь не пуста (bakeIdle), кадры идут дальше, до 40:
   кабина пояса готова и проявлена только к ~22-му кадру, и эталон на шестом мерил пустое небо (26.09) */
function detSettle(max,floor){
  for(let i=0;i<max||(i<40&&!bakeIdle());i++){
    G.t++;
