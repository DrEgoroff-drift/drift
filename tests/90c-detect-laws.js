...
function detStuck(c){
  ...
  /* рука на руле, а в кадре и в мире — ничего: ни пикселя, ни поля.
     Молчание по праву (c.mute, таблица DET_MUTE в 91zzzzzzzz-detect) закон не
     судит: где клавиш полёта нет по замыслу — карту двигают рукой, зимовку
     тычком, — «ничего не сдвинулось» и есть правильный ответ. До 0.450.1 карта
     держалась зелёной случайно: под W/A её спасало дрожание пикселя (пульс
     кольца «ВЫ» по G.t), и когда дизайн-проход убрал движение из сетки проб,
     «карта · A» покраснела, не сломав в игре ничего. Сам закон остаётся
     строгим — ровно ноль, не «почти» (0.438.0). */
  if((c.gesture==="W"||c.gesture==="A")&&c.mode1===c.mode0&&!c.spoke&&!c.mute&&
  ...
  if(c.mode1===c.mode0){
    if(c.gesture==="W"&&c.mode0==="system"&&c.p0&&c.p1&&c.ship0){
      ...
      /* закон M447: на карте слой либо в мире (едет 1:1), либо бумага — долей
         пути не едет ничего. Пороги поставлены до замера; сверить на релизе */
      cls=p.deep<.08&&p.sheet>=.25;
      why="протяжка карты: небо не в мире — с листом "+Math.round(p.sheet*100)+"% блоков, с небом "+
