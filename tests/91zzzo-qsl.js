...
TEST_SUITES.push(()=>suite("карточки: на том конце только люди игры, ни одного живого",{tier:"node"},()=>{
  resetWorld();
...
TEST_SUITES.push(()=>suite("карточки: услышал — записал, послал — ждёшь неделями",{tier:"node"},()=>{
  resetWorld();
...
TEST_SUITES.push(()=>suite("карточки: стена собирается целиком и переживает сохранение",{tier:"node"},()=>{
  resetWorld();
