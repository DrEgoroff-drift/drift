...
/* ══════════════ кадр базы (M413) ══════════════
   Шесть замечаний соседнего захода были про КАДР, а не про правила. Кадр
   тестом не измеришь целиком, но то, что игрок читает буквами, — можно. */
TEST_SUITES.push(()=>suite("M413: подсказка отвечает на нажатие, а не пересказывает базу",{tier:"node"},()=>{
  const B=bLife();
  ...
  S.cur=0;S.row=0;
  updateBase(0);
  const P=(G.prompt||"").split("\n");
  ok(P.length<=3,"не больше трёх строк: "+P.length+" · «"+(G.prompt||"").replace(/\n/g," / ")+"»");
  ok((G.prompt||"").indexOf("ЭНЕРГИЯ")<0,"энергия уехала на приборную доску");
  ok((G.prompt||"").indexOf("НА СКЛАДЕ")<0,"склад тоже");
  ok(P[0].indexOf("РЕАКТОР")===0,"первая строка называет, что под курсором: "+P[0]);
  /* в стволе — две строки и ни одной цифры хозяйства */
  S.cur=-1;
  updateBase(0);
  ok((G.prompt||"").indexOf("СТВОЛ")===0,"в стволе подсказка про ствол: "+G.prompt);
  ok((G.prompt||"").split("\n").length<=3,"и она короткая");
}));

TEST_SUITES.push(()=>suite("M413: журнал встречает карточкой, а не поперёк базы",{tier:"browser"},()=>{
  const B=bLife();
  const n=bShift();
  ...
  enterBase(B.p||{idx:B.idx});
  if(W<BASE_NOTE_W){
    /* телефонное окно: карточке места нет, записи идут сообщением */
    ok((G.msg||"").indexOf("Сбор")>=0,"на узком экране записи в сообщении: «"+(G.msg||"")+"»");
    ok(!G.base.note,"и карточки нет");
    return;
  }
  ok(G.base&&G.base.note&&G.base.note.lines.length>0,
    "записи легли в карточку: "+((G.base.note&&G.base.note.lines.length)|0));
  ok((G.msg||"").indexOf("Сбор")<0,"и не в сообщение по центру: «"+(G.msg||"")+"»");
  ok((G.base.note.t|0)>0,"карточка гаснет сама, а не висит вечно");
  /* и гаснет: за свой срок она уходит */
  G.base.note.t=1;updateBase(2);
  ok(G.base.note.t<=0,"через свой срок — погасла");
}));

TEST_SUITES.push(()=>suite("M413: шкалы названы словами",()=>{
  /* трёхбуквенные обрубки «ВЗД/ВОД/ХРЧ» ушли: на доске слова целиком.
     Проверяем по исходнику — это единственный способ спросить у картинки */
  const src=drawBase.toString();
  ok(src.indexOf('"ВОЗДУХ"')>0,"воздух назван словом");
  ok(src.indexOf('"ВОДА"')>0,"вода тоже");
...
TEST_SUITES.push(()=>suite("зонд: нажатие забрано, даже когда денег нет",()=>{
  ...
  G.probed={};
  /* денег нет: зонд не куплен, но нажатия всё равно его. С R5 (12.09) покупка —
     вторым тапом: первый переспрашивает, второй говорит, сколько не хватает */
  G.credits=0;G.msg="";G._probeArm=null;
  eq(probeClaim(),true,"первое нажатие забрал зонд, а не цели");
  ok(/ЦЕЛЬ ещё раз/.test(G.msg||""),"и переспросил: «"+G.msg+"»");
  eq(probeClaim(),true,"второе — тоже он");
  ok(!probeHas(G.sx,G.sy,p.idx),"а куплен он не был");
  ok((G.msg||"").indexOf("Зонд стоит")>=0,"и сказано ровно одно: «"+G.msg+"»");
  /* с деньгами — вторым тапом покупается, и нажатия тоже его */
  G.credits=PROBE_COST+10;G._probeArm=null;
  eq(probeClaim(),true,"с деньгами первое нажатие — взвод");
  ok(!probeHas(G.sx,G.sy,p.idx),"первым тапом не куплен");
  eq(probeClaim(),true,"второе — покупка");
