...
TEST_SUITES.push(() => suite("память: вечер прыжков не копит растр без предела",{tier:"heavy"}, () => {
  ...
  if (ok(gb && gc, "видеокарта мерится (GPU.ok)")) {
    ...
    /* кэш холстов в текстурах ограничен GPU_CVTEX_CAP (ступень 1 подняла его под подписи и фишки) */
    ok(gc.tex <= GPU_CVTEX_CAP, "холстов в текстурах не больше " + GPU_CVTEX_CAP + ": " + gc.tex);
  }
