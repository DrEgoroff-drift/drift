...
| P4 | ~~**Grisaille chunks**~~ — **done, 0.420.0 (M432)**: grey form bake + one glaze per chunk, hue events in a third pass; the parity sheet across three palettes is almanac issue VI, the algebra is `91zzzw-glaze` | `18a1-glaze`, `18c-chunks`, `19-mode-landing` | done |
| P5 | ~~**Cast shadow masks**~~ — **done on the surface, 0.421.0 (M433)**: the terrain and its boulders shadow themselves along a ray to `celSun`, baked per chunk as grey under the P4 glaze (`19c1-cast`); the sheet is almanac issue VII, the geometry is `91zzzw-cast`. The row named the mine/base/raid first — underground there is no static illuminant, so the surface, which has one, went first; interiors keep their lamps | `19c1-cast`, `19-mode-landing` | done (bake-only by construction) |
