...
- **29e (home upstairs) waits for the kit's `Path2D`** (on `claude/gpu-kit`, not yet in the base). Until then
  `hinDrawShell` is drawn live on 2D between the two GPU layers; `hinDrawStair`, `hinDrawHole`, `hinUpStuff`
  and `hinUpWindow` already bake (no `Path2D` in them).
