...
const BIRD_C={
  ...
  viol:sRGB("#7a5ad2"),
  /* радужка — по фотографии автора: сливово-пурпурная, к краю уходит в
     маджентовый кант. Синему и золотому она не спорит, потому что стоит
     ровно между ними по кругу */
  iris:sRGB("#9b3f7d"),
  /* клюв не жёлтый, а костяной: на листе породы это слоновая кость с тёплой
     тенью, и именно поэтому он не спорит с янтарём хохла */
  /* по фото: кость у основания, тёплый янтарь ближе к концу, тёмный рог на
     самом крючке. Ровно-кремовый клюв читается пластиковым */
  beak:sRGB("#f7e0a8"), beakD:sRGB("#e08a30"), beakTip:sRGB("#7e4c22"),
...
const BIRD_SPINE=[
  ...
  [0.10, 0.64,-0.52, 0.132,0.122,0.114],
  [0.22, 0.72,-0.32, 0.188,0.174,0.164],
  [0.35, 0.80,-0.15, 0.246,0.270,0.204],
  [0.48, 0.99,-0.05, 0.264,0.314,0.220],
  [0.60, 1.19,-0.02, 0.234,0.254,0.210],
  [0.70, 1.36,-0.06, 0.204,0.204,0.214],
  [0.78, 1.50,-0.06, 0.196,0.196,0.220],
  [0.86, 1.66,-0.01, 0.306,0.316,0.312],
  [0.94, 1.80, 0.04, 0.318,0.318,0.308],
...
function bodyColor(t,a){
  ...
  /* спина не должна быть плоским тёмным полем: к хребту кобальт светлеет,
     иначе со спины птица читается чёрным пятном */
  let c=vLerp(BIRD_C.blueD,BIRD_C.blue,smooth(.0,.55,belly)*.8+.2);
  c=vLerp(c,BIRD_C.blue,smooth(.35,.0,belly)*smooth(.45,.10,side)*.55);
  ...
  const cream=smooth(.50,.68,belly+jit)*smooth(.70,.58,t)*smooth(.16,.30,t);
  c=vLerp(c,vLerp(BIRD_C.creamD,BIRD_C.cream,smooth(.08,.42,t)),cream);
  /* воротник: тонкая кобальтовая полоса между горлом и грудью. На листе она
     отделяет голову от корпуса, и без неё белое поле идёт от клюва до лап */
  const collar=smooth(.022,.004,Math.abs(t-.748)+jit*0.20)*smooth(.48,.72,belly);
  c=vLerp(c,BIRD_C.blue,clamp(collar,0,1)*.85);
  ...
  /* эполет — узкая капля на ПЛЕЧЕ, а не половина груди. Он требует настоящего
     ...
     и хорош */
  const ep=smooth(.62,.40,Math.abs(t-.560)*5.2)*smooth(.44,.74,side)
          *smooth(.22,.38,belly)*smooth(.80,.60,belly);
  ...
  const mask=headk*smooth(.60,.86,frontal+jit*0.6)*smooth(.945,.880,t);
  c=vLerp(c,vLerp(BIRD_C.cream,BIRD_C.creamD,.06),clamp(mask,0,1));
  /* переход от кремового лица к кобальтовому темени идёт КРАПОМ, а не
     линией: на листе там мелкие пёстрые перья, и именно они не дают маске
     выглядеть наклеенной */
  const edge=clamp(1-Math.abs(mask-0.5)*3.4,0,1);
  c=vLerp(c,hashf(Math.round(t*160),Math.round(a*26))>0.5?BIRD_C.blue:BIRD_C.cream,edge*0.35);
  /* бровь: тёмная дуга над глазом. Она делает взгляд, и на листе он есть */
  const brow=smooth(.020,.004,Math.abs(t-.950))*smooth(.50,.80,side)*smooth(.34,.60,frontal);
  c=vLerp(c,vLerp(BIRD_C.blueD,BIRD_C.body,.45),clamp(brow,0,1)*.8);
